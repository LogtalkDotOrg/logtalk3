...
function Write-Usage-Help() {
	...
	Write-Output "This script creates a JIProlog logtalk.jip file with the Logtalk compiler"
	Write-Output "and runtime and an optional application.jip file from an application source code"
...
Copy-Item -Path "$env:LOGTALKHOME\adapters\ji.pl" -Destination .
...
if (($s -eq "") -or ($s -eq "none")) {
	...
	jiplgt -g $GoalParam
	(Get-Content ji.pl) -replace 'settings_file, allow', 'settings_file, deny' | Set-Content ji.pl
	Get-Content -Path ji.pl,
		paths_*.pl,
		expanding*_lgt.pl,
		monitoring*_lgt.pl,
