...
while getopts "cxd:t:n:p:k:s:l:g:vh" option; do
	case $option in
		c) compile="true";;
		x) saved_state="true";;
		d) d_arg="$OPTARG";;
...
if [ "$compile" != "false" ] ; then
	yaplgt$extension -g "logtalk_load(expand_library_alias_paths(loader)),logtalk_compile('$paths',[hook(expand_library_alias_paths),scratch_directory('$temporary')]),halt"
else
...
if [ "$settings" != "" ] && [ "$settings" != "none" ] ; then
	if [ "$compile" != "false" ] ; then
		yaplgt$extension -g "logtalk_load(expand_library_alias_paths(loader)),logtalk_compile('$settings',[hook(expand_library_alias_paths),optimize(on),scratch_directory('$temporary')]),halt"
	else
		yaplgt$extension -g "logtalk_compile('$settings',[optimize(on),scratch_directory('$temporary')]),halt"
	fi
	sed_i "s/settings_file, allow/settings_file, deny/" yap.pl
	cat \
		yap.pl \
		paths*.yap \
		expanding*_lgt.yap \
...
if [ "$loader" != "" ] ; then
	...
	if test -n "$(find . -maxdepth 1 -name '*.yap' -print -quit)" ; then
		files="$(ls -rt ./*.yap)"
