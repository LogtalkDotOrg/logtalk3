...
if [ "$loader" != "" ] ; then
	...
	swilgt$extension -g "set_logtalk_flag(source_data, off),set_logtalk_flag(optimize, on),set_logtalk_flag(clean, off),set_logtalk_flag(context_switching_calls, deny),set_logtalk_flag(scratch_directory, '$directory/application'),logtalk_load('$loader')" -t "halt"
	cat $(ls -t $directory/application/*.pl) > application.pl
	swipl -g "consult('../logtalk'),set_logtalk_flag(reload,skip),qcompile(application)" -t "halt"
	mv application.qlf ..
	rm *.pl
fi
