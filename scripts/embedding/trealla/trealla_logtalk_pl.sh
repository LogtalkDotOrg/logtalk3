...
usage_help() {
	echo
	echo "This script creates a Trealla Prolog logtalk.pl file with the Logtalk compiler"
	echo "and runtime and an optional application.pl file from an application source code"
	echo "given its loader file. When embedding an application, this script also creates"
	echo "a loader.pl file for loading all generated Prolog and foreign library files,"
...
while getopts "cd:t:p:l:s:g:vh" option; do
	case $option in
		c) compile="true";;
		d) d_arg="$OPTARG";;
		t) t_arg="$OPTARG";;
...
cp "$LOGTALKHOME/adapters/trealla.pl" .
cp "$LOGTALKHOME/core/core.pl" .

tplgt$extension -g "logtalk_compile([core(expanding),core(monitoring),core(forwarding),core(user),core(logtalk),core(core_messages)],[optimize(on),scratch_directory('$temporary')]),halt"

if [ "$compile" != "false" ] ; then
	tplgt$extension -g "logtalk_load(expand_library_alias_paths(loader)),logtalk_compile('$paths',[hook(expand_library_alias_paths),scratch_directory('$temporary')]),halt"
else
...
if [ "$settings" != "" ] && [ "$settings" != "none" ] ; then
	if [ "$compile" != "false" ] ; then
		tplgt$extension -g "logtalk_load(expand_library_alias_paths(loader)),logtalk_compile('$settings',[hook(expand_library_alias_paths),optimize(on),scratch_directory('$temporary')]),halt"
	else
		tplgt$extension -g "logtalk_compile('$settings',[optimize(on),scratch_directory('$temporary')]),halt"
	fi
	sed_i "s/settings_file, allow/settings_file, deny/" trealla.pl
	cat \
		trealla.pl \
		paths*.pl \
		expanding*_lgt.pl \
...
if [ "$loader" != "" ] ; then
	...
	tpl -g "consult('$directory/logtalk'),set_logtalk_flag(clean,off),set_logtalk_flag(scratch_directory,'$temporary/application'),logtalk_load('$loader'),halt"
