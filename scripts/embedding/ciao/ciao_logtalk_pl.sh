...
usage_help() {
	echo
	echo "This script creates a Ciao Prolog logtalk.pl file with the Logtalk compiler"
	echo "and runtime and an optional application.pl file from an application source"
...
echo ":- discontiguous('\$lgt_current_protocol_'/5)." > header.pl
echo ":- discontiguous('\$lgt_current_category_'/6)." >> header.pl
echo ":- discontiguous('\$lgt_current_object_'/11)." >> header.pl
echo ":- discontiguous('\$lgt_entity_property_'/2)." >> header.pl
echo ":- discontiguous('\$lgt_predicate_property_'/3)." >> header.pl
echo ":- discontiguous('\$lgt_implements_protocol_'/3)." >> header.pl
echo ":- discontiguous('\$lgt_imports_category_'/3)." >> header.pl
echo ":- discontiguous('\$lgt_instantiates_class_'/3)." >> header.pl
echo ":- discontiguous('\$lgt_specializes_class_'/3)." >> header.pl
echo ":- discontiguous('\$lgt_extends_category_'/3)." >> header.pl
...
if [ "$settings" != "" ] && [ "$settings" != "none" ] ; then
	...
	sed_i "s/settings_file, allow/settings_file, deny/" ciao.pl
	cat \
		header.pl \
		ciao.pl \
		paths*.pl \
...
if [ "$loader" != "" ] ; then
	...
	ciaolgt$extension -e "set_logtalk_flag(clean,off),set_logtalk_flag(scratch_directory,'$temporary/application'),logtalk_load('$loader'),halt"
	echo ":- set_prolog_flag(multi_arity_warnings, off)." > "$directory"/header.pl
	echo ":- discontiguous('\$lgt_current_protocol_'/5)." >> "$directory"/header.pl
	echo ":- discontiguous('\$lgt_current_category_'/6)." >> "$directory"/header.pl
	echo ":- discontiguous('\$lgt_current_object_'/11)." >> "$directory"/header.pl
	echo ":- discontiguous('\$lgt_entity_property_'/2)." >> "$directory"/header.pl
	echo ":- discontiguous('\$lgt_predicate_property_'/3)." >> "$directory"/header.pl
	echo ":- discontiguous('\$lgt_implements_protocol_'/3)." >> "$directory"/header.pl
	echo ":- discontiguous('\$lgt_imports_category_'/3)." >> "$directory"/header.pl
	echo ":- discontiguous('\$lgt_instantiates_class_'/3)." >> "$directory"/header.pl
	echo ":- discontiguous('\$lgt_specializes_class_'/3)." >> "$directory"/header.pl
	echo ":- discontiguous('\$lgt_extends_category_'/3)." >> "$directory"/header.pl
