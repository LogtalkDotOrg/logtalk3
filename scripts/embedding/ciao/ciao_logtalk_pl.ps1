...
#############################################################################
## 
##   This script creates a Ciao Prolog logtalk.pl file with the Logtalk
...
function Write-Usage-Help() {
	...
	Write-Output "This script creates a Ciao Prolog logtalk.pl file with the Logtalk compiler"
	Write-Output "and runtime and an optional application.pl file from an application source"
...
Copy-Item -Path "$env:LOGTALKHOME\adapters\ciao.pl" -Destination .
Copy-Item -Path "$env:LOGTALKHOME\core\core.pl" -Destination .
$ScratchDirOption = ", scratch_directory('$($t.Replace('\','/'))')"

Set-Content -Path header.pl -Value ":- discontiguous('\$lgt_current_protocol_'/5)."
Add-Content -Path header.pl -Value ":- discontiguous('\$lgt_current_category_'/6)."
Add-Content -Path header.pl -Value ":- discontiguous('\$lgt_current_object_'/11)."
Add-Content -Path header.pl -Value ":- discontiguous('\$lgt_entity_property_'/2)."
Add-Content -Path header.pl -Value ":- discontiguous('\$lgt_predicate_property_'/3)."
Add-Content -Path header.pl -Value ":- discontiguous('\$lgt_implements_protocol_'/3)."
Add-Content -Path header.pl -Value ":- discontiguous('\$lgt_imports_category_'/3)."
Add-Content -Path header.pl -Value ":- discontiguous('\$lgt_instantiates_class_'/3)."
Add-Content -Path header.pl -Value ":- discontiguous('\$lgt_specializes_class_'/3)."
Add-Content -Path header.pl -Value ":- discontiguous('\$lgt_extends_category_'/3)."
...
if (($s -eq "") -or ($s -eq "none")) {
	...
	ciaolgt -e $GoalParam
	(Get-Content ciao.pl) -replace 'settings_file, allow', 'settings_file, deny' | Set-Content ciao.pl
	Get-Content -Path header.pl,
		ciao.pl,
		paths_*.pl,
		expanding*_lgt.pl,
...
if ($l -ne "") {
	...
	Set-Content -Path $d/header.pl -Value ":- set_prolog_flag(multi_arity_warnings, off)."
	add-Content -Path $d/header.pl -Value ":- discontiguous('\$lgt_current_protocol_'/5)."
	Add-Content -Path $d/header.pl -Value ":- discontiguous('\$lgt_current_category_'/6)."
	Add-Content -Path $d/header.pl -Value ":- discontiguous('\$lgt_current_object_'/11)."
	Add-Content -Path $d/header.pl -Value ":- discontiguous('\$lgt_entity_property_'/2)."
	Add-Content -Path $d/header.pl -Value ":- discontiguous('\$lgt_predicate_property_'/3)."
	Add-Content -Path $d/header.pl -Value ":- discontiguous('\$lgt_implements_protocol_'/3)."
	Add-Content -Path $d/header.pl -Value ":- discontiguous('\$lgt_imports_category_'/3)."
	Add-Content -Path $d/header.pl -Value ":- discontiguous('\$lgt_instantiates_class_'/3)."
	Add-Content -Path $d/header.pl -Value ":- discontiguous('\$lgt_specializes_class_'/3)."
	Add-Content -Path $d/header.pl -Value ":- discontiguous('\$lgt_extends_category_'/3)."
	...
	Get-Item *.pl | 
		...
		Get-Content |
		Set-Content $d/application0.pl
	Get-Content -Path $d/header.pl,
		$d/application0.pl | Set-Content "$d/application.pl"
	Remove-Item $d/header.pl
	Remove-Item $d/application0.pl
	Pop-Location
}
