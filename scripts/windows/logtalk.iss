...
Name: "prolog\trealla"; Description: "Trealla Prolog integration (2.84.29 or later)"; Types: full prolog custom; Flags: disablenouninstallwarning
...
Source: "{#MyBaseDir}\scripts\embedding\xvm\*.ps1"; DestDir: "{win}"; Components: base; Flags: ignoreversion; Check: IsAdminInstallMode
...
Source: "{#MyBaseDir}\tools\packs\*.ps1"; DestDir: "{win}"; Components: base; Flags: ignoreversion; Check: IsAdminInstallMode
...
begin
  if IsWin64 then
    if RegQueryStringValue(HKLM64, 'Software\IC-Parc\Eclipse\8.0\', 'ECLIPSEDIR', ECLIPSEDIR) then
      Result := ECLIPSEDIR + '\lib\x86_64_nt\eclipse.exe'
    else if RegQueryStringValue(HKLM32, 'Software\IC-Parc\Eclipse\8.0\', 'ECLIPSEDIR', ECLIPSEDIR) then
      Result := ECLIPSEDIR + '\lib\i386_nt\eclipse.exe'
    else if RegQueryStringValue(HKLM64, 'Software\IC-Parc\Eclipse\7.2\', 'ECLIPSEDIR', ECLIPSEDIR) then
      Result := ECLIPSEDIR + '\lib\x86_64_nt\eclipse.exe'
    ...
    if RegQueryStringValue(HKLM, 'Software\IC-Parc\Eclipse\8.0\', 'ECLIPSEDIR', ECLIPSEDIR) then
      Result := ECLIPSEDIR + '\lib\i386_nt\eclipse.exe'
    else if RegQueryStringValue(HKLM, 'Software\IC-Parc\Eclipse\7.2\', 'ECLIPSEDIR', ECLIPSEDIR) then
      Result := ECLIPSEDIR + '\lib\i386_nt\eclipse.exe'
...
begin
  if IsWin64 then
    if RegQueryStringValue(HKLM64, 'Software\SICS\SICStus4.10_win32\', 'SP_PATH', SP_PATH) or
       ...
       RegQueryStringValue(HKLM64, 'Software\SICS\SICStus4.6_win32\', 'SP_PATH', SP_PATH) or
       ...
       RegQueryStringValue(HKLM64, 'Software\SICS\SICStus4.3_win32\', 'SP_PATH', SP_PATH) or
    ...
    if RegQueryStringValue(HKLM, 'Software\SICS\SICStus4.10_win32\', 'SP_PATH', SP_PATH) or
       ...
       RegQueryStringValue(HKLM, 'Software\SICS\SICStus4.1_x86-win32-nt-4\', 'SP_PATH', SP_PATH)
    then
      Result := SP_PATH + '\bin\spwin.exe'
    else
      Result := 'prolog_compiler_not_installed'
  ...
  if IsWin64 then
    ...
    then
      if FileExists(Home + '\bin\plcon.exe') then
        ...
        Result := 'prolog_compiler_not_installed'
    else if FileExists('C:\Program Files\swipl\bin\swipl.exe') then
  ...
  if IsWin64 then
    ...
    then
      if FileExists(Home + '\bin\plwin.exe') then
        ...
        Result := 'prolog_compiler_not_installed'
    else if FileExists('C:\Program Files\swipl\bin\swipl-win.exe') then
