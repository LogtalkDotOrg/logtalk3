...
:- object(tests,
	...
	throws(threaded_engine_create_3_02, error(type_error(callable,1), logtalk(threaded_engine_create(_,_,_), _))) :-
		Goal = 1,	% delay error to runtime
		{threaded_engine_create(_, Goal, _)}.

	% engine names must be unique
	throws(threaded_engine_create_3_03, error(permission_error(create,engine,test_engine_1), logtalk(threaded_engine_create(none,_,test_engine_1), _))) :-
