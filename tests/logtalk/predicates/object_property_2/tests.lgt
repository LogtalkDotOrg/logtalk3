...
:- object(tests,
	...
	succeeds(object_property_2_07) :-
		...
		object_property(test_object, number_of_clauses(NC)), NC == 37,
		object_property(test_object, number_of_rules(NR)), NR == 26,
		object_property(test_object, number_of_user_clauses(NUC)), NUC == 35,
		object_property(test_object, number_of_user_rules(NUR)), NUR == 24,
	...
	succeeds(object_property_2_08) :-
		object_property(test_object, public(Public)), Public == [m/2, a/1],
	...
	succeeds(object_property_2_29) :-
		findall(
			Reference-Properties,
			object_property(test_object, references(Reference, Properties)),
			References
		),
		References == [
			test_object-[in(clause), lines(41, 42), line_count(41)]
		].

	succeeds(object_property_2_30) :-
		findall(
			Reference-Properties,
			object_property(multifile_object, references(Reference, Properties)),
			References
		),
		References == [
			(test_object::m/2)-[in(multifile), lines(34, 34), line_count(34)]
		].

	% modules compiled as objects

	succeeds(object_property_2_31) :-
		findall(Property, object_property(module,Property), Properties),
		ground(Properties), member(module, Properties).
	...
	fails(object_property_2_40) :-
		object_property(empty_object, references(_, _)),
		fail.
