...
:- object(tests,
	...
	test(sics_univ_2_18, error(representation_error(max_arity)), [condition(\+ current_prolog_flag(max_arity,unbounded))]) :-
	...
	:- if((
		...
		test(lgt_univ_2_33, true(L == [f,f(X,Y),Y])) :-
			{X = f(X,Y), X =.. L}.
	...
	:- else.
		...
		- test(lgt_univ_2_33, true(L == [f,f(X,Y),Y]), [note('STO')]) :-
			{X = f(X,Y), X =.. L}.
