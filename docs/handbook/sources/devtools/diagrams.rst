...
`../../apis/library_index.html#diagrams <../../apis/library_index.html#diagrams>`__
...
- *entity diagrams* showing entity public interfaces, entity inheritance
  relations, and entity predicate (or non-terminal) cross-reference
  relations
...
- *directory loading diagrams* showing which directories contain files
  that load files from other directories
...
Limitations in both the graph languages and UML force the invention of a
...
the ``diagrams`` tool. There's also preliminary support for Mermaid (its
...
This viewer currently requires starting a local web server in the
generated diagrams directory and manually entering the server address.

PlantUML output is also supported. The PlantUML backend generates
``.puml`` files using custom stereotypes with spotted characters to
distinguish Logtalk entity kinds (O/blue for objects, P/green for
protocols, T/orange for categories). All five Logtalk relationships
(implements, imports, extends, instantiates, specializes, complements)
are mapped to distinct labeled arrow styles. A shared ``logtalk.iuml``
include file defines all skinparam blocks, stereotype colors, and CSS
styles for consistent rendering. PlantUML version 1.2024.0 or later is
recommended. See https://plantuml.com for installation instructions.
...
links, which can be used for showing, e.g., entity types, relation
types, file paths, and for navigating to files and directories of files
...
It's also worth experimenting with different layouts to find the one
...
with a specific backend Prolog compiler, try, if possible, to use
another supported backend Prolog compiler when generating diagrams.

For printing large diagrams, you will need to either use a tool to slice
the diagram into page-sized pieces or, preferably, use software capable
of tiled printing (e.g., Adobe Reader). You can also hand-edit the
generated ``.dot`` files and play with settings such as aspect ratio for
...
``cx2``, and ``puml``. The non-parametric versions of the diagram
objects default to ``dot``.
...
libraries or files for which you want to generate diagrams.
...
using the predicates that generate library diagrams will also
automatically generate the entity and predicate cross-referencing
diagrams.
...
In this case, the ``DocPrefix`` argument should be the path to the
directory containing the HTML version of the application APIs.
...
a GitHub repository, and its documentation is published on the Logtalk
...
Caller graph diagrams can be generated for a specific predicate using
the ``caller_diagram`` object. For example, to generate a caller graph
for the ``list::member/2`` predicate:
...
``meta_compiler`` library is used to optimize meta-predicate calls, the
...
random crashes (segmentation faults usually), often requiring retrying
