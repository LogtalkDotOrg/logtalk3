...
(Gaussian) features with Laplace smoothing. Uses the same dataset
representation and similar API as the ``c45`` library to simplify
comparing learned classifiers.
...
- **Dataset Protocol**: Uses the same ``dataset_protocol`` as the
  ``c45`` library
...
Datasets are defined as Logtalk objects implementing the
``dataset_protocol`` protocol defined by the ``c45`` library. See
``test_files`` directory for examples.
