...
Open the
`../../apis/library_index.html#fp_growth <../../apis/library_index.html#fp_growth>`__
...
- **FP-tree Construction**: Builds a compact prefix tree from frequent
  items ordered by global support.
- **Header and Parent Links**: Stores parent links directly in tree
  nodes and derives header-table node chains from the final tree so
  conditional pattern bases are reconstructed by walking parent links
  instead of using a separate node index.
- **Pattern Growth**: Mines frequent itemsets recursively from
  conditional pattern bases without candidate generation.
...
1. Han, J., Pei, J., and Yin, Y. (2000) - "Mining frequent patterns
