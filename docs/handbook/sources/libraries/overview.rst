...
protocols. The currently available libraries can be grouped by theme as
follows (note that some libraries naturally span multiple themes):
...
- Types, collections and generic data processing: ``basic_types``,
  ...
  ``intervals``, ``lru_caches``, ``queues``, ``ripple_down_rules``,
  ``sets``, ``subsequences``, ``tries``, ``union_find``, and
  ``zippers``.
...
- Geospatial data: ``crs_projections``, ``geojson``, ``geohash``,
  ``geospatial``, ``gpx``, ``nmea``, ``tle_orbits``, and ``wkt_wkb``
...
- Text and NLP: ``character_sets``, ``grammars``, ``string_distance``,
  and ``strings``, ``language_detection``, ``lemmatization``,
  ``n_grams``, ``stemming``, ``stop_words``, ``text_normalization``,
  ``text_vectorization``, and ``tokenization``.
- Web and HTTP APIs:

  - High-level libraries: ``html``, ``mime_types``, ``uri_template``,
    ``url``, ``http_client``, ``http_server``, ``http_sse``,
    ``http_websocket``, ``gravatar``, ``open_api``, ``open_ai``,
    ``open_id``, ``rest``, and ``s3``.
...
- Machine learning:
  ...
  - Classification: ``classification_protocols``,
    ...
    ``logistic_regression_classifier``, ``mlp_classifier``,
    ``naive_bayes_classifier``, ``nearest_centroid_classifier``,
    ``qda_classifier``, ``random_forest_classifier``, and
    ``sgd_classifier``.
  - Anomaly detection: ``anomaly_detection_protocols``,
    ...
    ``one_class_svm_anomaly_detector``, and
  ...
  - Ranking: ``ranking_protocols``, ``borda_ranker``,
    ...
    ``regularized_bradley_terry_ranker``, ``schulze_ranker``,
  ...
  - Clustering: ``clustering_protocols``, ``agglomerative_clusterer``,
    ...
    ``kmodes_clusterer``, ``kprototypes_clusterer``,
  ...
  - Dimension reduction: ``dimension_reduction_protocols``,
    ...
    ``truncated_svd_projection``, ``tsne_projection``, and
    ``umap_projection``.
