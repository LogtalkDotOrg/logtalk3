...
- **``k/1``**: Number of nearest neighbors considered for each
  prediction. Smaller values make predictions more local; larger values
  smooth them by averaging over more training rows. The default is
  ``3``.
...
- **``feature_scaling/1``**: Controls z-score standardization of
  continuous attributes before storing rows and encoding prediction
  requests. Accepted values are ``on`` and ``off``. The default is
  ``on``.
