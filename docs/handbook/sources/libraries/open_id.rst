...
The library uses ``http_client(http_process_transport)`` for HTTP
requests. HTTPS requests are made using ``http_process_transport`` TLS
connections, which require the ``openssl`` command to be available
unless overridden with the ``openssl_executable/1`` option.
