.. _library_http_socket_process:

``http_socket_process``
=======================

The ``http_socket_process`` library provides an alternative
implementation of the ``http_socket_protocol`` protocol. Based on the
...
Open the
`../../apis/library_index.html#http_socket_process <../../apis/library_index.html#http_socket_process>`__
...
::
   ...
   | ?- logtalk_load(http_socket_process(tester)).
...
- the same public connection and pool handle shapes used by
  ``http_socket`` to keep downstream endpoint validation compatible
...
- ``type/1`` is accepted for drop-in compatibility with ``http_socket``
  but does not change the internal relay socket type used by the
...
the existing ``http_socket`` behavior.
...
still exposing real binary streams to ``http_server`` and the existing
WebSocket layers. The same listener shape is used for both plain TCP and
TLS listeners, which allows the higher-level HTTP and WebSocket layers
to serve HTTPS and WSS requests without depending on backend-specific
SSL predicates.
...
::
   ...
   | ?- logtalk_load(http_socket_process(tester_live)).
...
- local loopback server-side smoke tests for plain HTTP, HTTPS, and WSS
  using the ``http_socket_process`` listener path
