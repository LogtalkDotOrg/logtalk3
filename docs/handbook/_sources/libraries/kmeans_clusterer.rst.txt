...
- **Deterministic Initialization**: Supports ``first_k`` and
  ...
  farthest example from the centroids selected so far.
...
- **Rich Training Diagnostics**: Learned clusterers report training
  example count, convergence status, iteration count, and final centroid
  shift.
...
- ``maximum_iterations(Iterations)``: Maximum number of Lloyd
  iterations. Default is ``100``.
- ``tolerance(Tolerance)``: Maximum centroid shift threshold for
  convergence. Default is ``1.0e-6``.
- ``initialization(Initialization)``: Centroid initialization strategy.
...
2. Lloyd (1982) - "Least Squares Quantization in PCM". IEEE Transactions
