...
imported category implements ``http_handler_protocol``, providing the
``handle/2`` predicate expected by the server:
...
annotations, middleware, content negotiation, automatic ``OPTIONS``
handling, and OpenAPI derivation.
