...
Open the
`../../apis/library_index.html#dbscan <../../apis/library_index.html#dbscan>`__
...
- **Density-Based Clustering**: Learns density-connected clusters using
  epsilon neighborhoods and minimum point counts.
- **Adaptive Neighborhood Indexing**: Builds a low-dimensional
  epsilon-grid index when it is likely to be cheaper and otherwise falls
  back to a deterministic metric tree. The metric-tree path uses
  adaptive leaf sizing, balanced tie-aware subtree splits, selectable
  exact or heuristic pivot scoring, and lower-allocation range-query
  traversal.
...
- **Reachable-Core Prediction**: New instances are assigned to the
  cluster of the nearest reachable core point within the learned epsilon
  radius; otherwise the atom ``noise`` is returned.
...
- ``epsilon(Epsilon)``: Neighborhood radius used to determine density
  connectivity. Default is ``1.0``.
- ``minimum_points(MinimumPoints)``: Minimum number of points in an
  epsilon neighborhood for a core point. Default is ``2``.
...
- ``pivot_scoring(PivotScoring)``: Metric-tree pivot scoring strategy.
  ...
  that sorts each candidate profile).
...
- ``Encoders``: List of continuous attribute encoders storing attribute
  name, mean, and scale.
- ``Clusters``: List of ``cluster(Id, CorePoints, BorderPoints)`` terms
  in cluster-id order.
...
1. Ester, Kriegel, Sander, and Xu (1996) - "A Density-Based Algorithm
