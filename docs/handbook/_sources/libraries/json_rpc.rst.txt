...
Protocol (MCP). Each message is preceded by a
``Content-Length: N\r\n\r\n`` header where ``N`` is the byte length of
the JSON body:
...
- ``write_message(+Output, +Message)`` - Write a newline-delimited
  message
- ``read_message(+Input, --Message)`` - Read a newline-delimited message
- ``write_framed_message(+Output, +Message)`` - Write a message with
  Content-Length framing
- ``read_framed_message(+Input, --Message)`` - Read a message with
  Content-Length framing
