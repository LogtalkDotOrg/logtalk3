...
``http_websocket_session`` library.
...
- ``read_message/6`` for stateful message reads plus automatic
  close-handshake replies and optional auto-pong policy.
...
- ``run_session/3-4`` for higher-level callback-driven loops over
  ...
  optional auto-pong, keepalive, and idle-timeout policies.
- ``open/4-5`` in the ``http_websocket_client_session`` object as a
...
- ``serve_once/6-7`` in the ``http_websocket_server_session`` object as
...
- ``serve_until_shutdown/5-6`` and ``request_shutdown/1`` in the
  ``http_websocket_server_session`` object for registry-backed
...
- ``open/1``, ``close/1``, ``register/2``, ``unregister/2``, ``send/3``,
  ...
  ``http_websocket_session_registry`` object for managing active
...
Client sessions expect masked inbound frames and mask all outgoing
frames. Server sessions expect unmasked inbound frames and leave
outgoing frames unmasked.
...
- reuses the stateful ``read_message/6`` orchestration including
  automatic close replies and optional auto-pong policy,
- writes the handler replies using the stateful ``write_message/4``
  predicate,
...
- ``http_websocket_server_session::request_shutdown(Control)`` is
  called, or
...
- When client-side code should own the full upgraded connection
  lifecycle, call ``http_websocket_client_session::open/4-5`` and let
  the session layer handle the opening handshake, any configured initial
  outbound messages, and the final connection close.
- When a single callback object should own the connection lifecycle,
  pass the upgraded handle to ``run_session/3-4`` and let the session
...
- When server-side code needs multiple active sessions plus queued
  ...
  ``http_websocket_session_registry::open/1`` and call
  ``http_websocket_server_session::serve_until_shutdown/5-6``.
