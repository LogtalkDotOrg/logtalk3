...
fails, without producing any plaintext, if the tag does not match. Bare,
unauthenticated ChaCha20 is intentionally not exposed as a public
predicate: pairing a stream cipher with its own MAC by hand is a
well-known source of protocol bugs, so only the authenticated
construction is part of the public interface. These two predicates are
only available on backend Prolog compilers supporting unbounded integer
arithmetic, since Poly1305 requires an exact 130-bit accumulator.
