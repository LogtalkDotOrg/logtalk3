...
The current implementation provides nineteen public predicates, grouped
by responsibility:
...
- Server-side request serving: ``serve_once/3``,
  ``serve_websocket_once/5``, ``serve_listener/4``, and
  ``serve_listener/5``.
...
When the same listener, serving, or pool-management option is given
