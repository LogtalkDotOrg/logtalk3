...
- Pair it with `http <../http/NOTES.md>`__ when you need request or
  response contract validation against normalized HTTP terms or wire
  sources.
- Pair it with `http_router <../http_router/NOTES.md>`__ when router
  objects expose ``open_api_provider_protocol`` metadata derived from
  ``route/4``, ``route_metadata/2``, and ``route_produces/2``.
- Pair it with `rest <../rest/NOTES.md>`__ when you want the same
  validation and document derivation over higher-level endpoint
  descriptors built on top of ``http_router``.
