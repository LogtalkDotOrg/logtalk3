...
The current library provides one public object with two public
predicates:
...
- ``416 Range Not Satisfiable`` for malformed, unsupported multi-range,
  and unsatisfiable range requests
- ``If-Range`` fallback to full ``200 OK`` responses when the validator
  does not match or is only weakly equal
...
- precompressed ``.zst``, ``.br``, and ``.gz`` asset negotiation driven
  by ``Accept-Encoding``
...
- ``If-Range`` mismatches fall back to the full ``200 OK`` response
  before range validation, so malformed or unsupported range syntax is
  ignored in that case
- precompressed-asset negotiation currently recognizes ``.zst``,
...
- path sandboxing is based on canonicalized absolute-path prefix checks;
  no separate cross-backend symlink-policy layer has been added yet
