...
- Provide the internal ``http_origin_site_helpers`` category reused by
  ``http_cors`` and ``http_session`` for absolute URL parsing,
  ``Origin`` parsing, normalized endpoint derivation, and schemeful
  same-site comparison using a bundled public suffix rule set with
  exact, wildcard, and exception cases
...
- The internal schemeful same-site helper uses bundled public suffix
  rules to tighten site classification for multi-label public suffixes
  and hosted domains, but automatic synchronization with the full
  Mozilla Public Suffix List remains out of scope
