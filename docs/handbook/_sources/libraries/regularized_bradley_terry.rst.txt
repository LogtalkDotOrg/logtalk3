.. _library_regularized_bradley_terry:
...
Open the
`../../apis/library_index.html#regularized_bradley_terry <../../apis/library_index.html#regularized_bradley_terry>`__
...
::
   ...
   | ?- logtalk_load(regularized_bradley_terry(loader)).
...
::
   ...
   | ?- logtalk_load(regularized_bradley_terry(tester)).
...
directed win graph to be strongly connected, see the ``bradley_terry``
library.
...
::
   ...
   | ?- regularized_bradley_terry::learn(my_dataset, Ranker).
   ...

   % Learn with custom iteration and Gamma-prior options
   | ?- regularized_bradley_terry::learn(my_dataset, Ranker, [maximum_iterations(500), tolerance(1.0e-7), gamma_prior(gamma(3.0, 4.0))]).
   ...
...
- ``model(regularized_bradley_terry)`` identifies the learning algorithm
  that produced the ranker.
...
::
   ...
   | ?- regularized_bradley_terry::learn(my_dataset, Ranker),
        regularized_bradley_terry::rank(Ranker, [item_a, item_b, item_c], Ranking).
   Ranking = [...]
   ...
...
::
   ...
   | ?- regularized_bradley_terry::learn(my_dataset, Ranker),
        regularized_bradley_terry::export_to_clauses(my_dataset, Ranker, my_ranker, Clauses).
   Clauses = [my_ranker(regularized_bt_ranker(...))]
   ...

   % Export to a file
   | ?- regularized_bradley_terry::learn(my_dataset, Ranker),
        regularized_bradley_terry::export_to_file(my_dataset, Ranker, my_ranker, 'ranker.pl').
   ...
