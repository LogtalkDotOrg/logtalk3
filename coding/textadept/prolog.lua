-- Copyright 2006-2019 Mitchell mitchell.att.foicica.com. MIT License.
...
local dialects = setmetatable({gprolog = 'gprolog',swipl = 'swipl'}, {__index = function(_, _) return 'iso' end})
...
lex:add_rule('directive', token(lexer.WHITESPACE, lexer.starts_line(S(' \t'))^0) *
...
bifs.swipl = [[
  -- Collected automatically via current_arithmetic_function/1 with some cleanup.
...
local operators = {}
operators.iso = [[
  -- Collected automatically via current_op/3 with some cleanup and comparison to docs.
  rem div mod is
]]
...
lex:add_rule('operator', token(lexer.OPERATOR, word_match(operators[dialect]) + S('-!+\\|=:;&<>()[]{}/*^@?.')))

-- Variables.
lex:add_rule('variable', token(lexer.VARIABLE, (lexer.upper+'_') * ((lexer.word)^1 + (lexer.digit^1) + P'_'^1)^0))
