...
if [[ $1 == *.lgt ]] || [[ $1 == *.logtalk ]]; then
	...
	if [ -e "$(command -v rlwrap)" ] ; then
		exec rlwrap lvmpl -f "$LOGTALKHOME/integration/logtalk_lvm.pl" -g "logtalk_load('$LOADER_FILE')." "$@"
	else
		exec lvmpl -f "$LOGTALKHOME/integration/logtalk_lvm.pl" -g "logtalk_load('$LOADER_FILE')." "$@"
	fi
else
	if [ -e "$(command -v rlwrap)" ] ; then
		exec rlwrap lvmpl -f "$LOGTALKHOME/integration/logtalk_lvm.pl" "$@"
	else
		exec lvmpl -f "$LOGTALKHOME/integration/logtalk_lvm.pl" "$@"
	fi
fi
