...
$source = $env:LOGTALKHOME + '\integration\logtalk_swi.pl'

if ($args.Count -gt 2 -and $args[$args.Count-2] -eq "--%") {
    $n = $args.Count - 3
    swipl.exe -s $source $args[0..$n] -- (-Split $args[$args.Count-1])
} elseif ($args.Count -eq 2 -and $args[0] -eq "--%") {
    swipl.exe -s $source -- (-Split $args[$args.Count-1])
} else {
    swipl.exe -s $source $args
}
