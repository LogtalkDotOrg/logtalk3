...
$sourcen = $env:LOGTALKHOME + '\integration\logtalk_eclipse.pl'

if ($args.Count -gt 2 -and $args[$args.Count-2] -eq "--%") {
    $n = $args.Count - 3
    eclipse.exe -L iso -t user -f $sourcen $args[0..$n] -- $args[$args.Count-1]
} elseif ($args.Count -eq 2 -and $args[0] -eq "--%") {
    eclipse.exe -L iso -t user -f $sourcen -- $args[$args.Count-1]
} else {
    eclipse.exe -L iso -t user -f $sourcen $args
}
