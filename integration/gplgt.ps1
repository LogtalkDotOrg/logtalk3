...
$sourcen = '$LOGTALKHOME/integration/logtalk_gp.pl'

if ($args.Count -gt 2 -and $args[$args.Count-2] -eq "--%") {
    $n = $args.Count - 3
    gprolog.exe --entry-goal "['$sourcen']" $args[0..$n] -- $args[$args.Count-1]
} elseif ($args.Count -eq 2 -and $args[0] -eq "--%") {
    gprolog.exe --entry-goal "['$sourcen']" -- $args[$args.Count-1]
} else {
    gprolog.exe --entry-goal "['$sourcen']" $args
}
