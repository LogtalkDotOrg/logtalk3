...
$source = $env:LOGTALKHOME + '\integration\logtalk_arriba.pl'

if ($args.Count -gt 2 -and $args[$args.Count-2] -eq "--%") {
    $n = $args.Count - 3
    arriba.exe -f $source $args[0..$n] -- (-Split $args[$args.Count-1])
} elseif ($args.Count -eq 2 -and $args[0] -eq "--%") {
    arriba.exe -f $source -- (-Split $args[$args.Count-1])
} else {
    arriba.exe -f $source $args
}
