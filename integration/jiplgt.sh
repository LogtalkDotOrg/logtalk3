...
if [[ $1 == *.lgt ]] || [[ $1 == *.logtalk ]]; then
	LOADER_FILE=$1
	shift
	exec java -jar -DLOGTALKHOME="$LOGTALKHOME" -DLOGTALKUSER="$LOGTALKUSER" -DHOME="$HOME" "$JIP_HOME/jipconsole.jar" -c "$LOGTALKHOME/integration/logtalk_ji.pl" -n -g "logtalk_load('$LOADER_FILE')" "$@"
else
