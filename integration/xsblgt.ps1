...
$sourcen = $env:LOGTALKHOME + '\integration\logtalk_xsb.pl'

if ($args.Count -gt 2 -and $args[$args.Count-2] -eq "--%") {
    $n = $args.Count - 3
    xsb64.bat -l -e "['$sourcen']." $args[0..$n] -- $args[$args.Count-1]
} elseif ($args.Count -eq 2 -and $args[0] -eq "--%") {
    xsb64.bat -l -e "['$sourcen']." -- $args[$args.Count-1]
} else {
    xsb64.bat -l -e "['$sourcen']." $args
}
