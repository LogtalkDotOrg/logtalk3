...
* `{!}` - delete (cut) all intermediate files generated by the compilation of Logtalk source files
* `{File}` - compile a single Logtalk source file (using default flags)
* `{File1, File2, ...}` - compile several Logtalk source files (using default flags)
