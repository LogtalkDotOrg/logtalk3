...
The afapter files define shorthands to often used Logtalk built-in
predicates such as `logtalk_load/1` and `logtalk_make/1`:

* `{*}` - reload all modified Logtalk source files since last laoded
...
iterative development, are not part of the Logtalk language specification
and can be commented in case of conflict with backend Prolog compiler
features.
