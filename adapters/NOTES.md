...
setting globally the `prolog_compiler` flag:
	...
	?- set_logtalk_flag(prolog_compiler, [encrypt(true)]).
	true.

After, simply load your application code. The encrypted files (using the
`.plx` extension ) are saved at the same location as the Logtalk source
files. To load the encrypted files, you will need to modify your driver
files (typically, `loader.lgt` and `tester.lgt`), replacing the loading
goals with:
...
The argument of the call to the XVM `load_files/1` built-in predicate
will be the encrypted files (the extension can be omitted).
