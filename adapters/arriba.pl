%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  Adapter file for Arriba 8.0.0 and later versions
...
'$lgt_prolog_feature'(prolog_dialect, arriba).
'$lgt_prolog_feature'(prolog_version, v(Major, Minor, Patch)) :-
	current_prolog_flag(version_data, arriba(Major, Minor, Patch, _)).
'$lgt_prolog_feature'(prolog_compatible_version, '@>='(v(8, 0, 0))).
...
'$lgt_prolog_term_expansion'((:- Directive), Expanded) :-
	...
	catch('$lgt_arriba_directive_expansion'(Directive, Expanded), _, fail).

'$lgt_arriba_directive_expansion'(disk_predicate(Template,Mode,Database), [{:- disk_predicate(CTemplate,Mode,Database)}, (:- dynamic(Functor/Arity))]) :-
...
'$lgt_prolog_goal_expansion'(load_foreign_library(Path), '$lgt_arriba_load_foreign_library'(Path)).
...
'$lgt_prolog_goal_expansion'(
		open_db(DB, DBFile),
		{open_db(DB, DBFile), '$lgt_arriba_add_disk_predicate_ddefs'(DB, Prefix)}) :-
	logtalk_load_context(entity_type, object),
	!,
	logtalk_load_context(entity_prefix, Prefix).
'$lgt_prolog_goal_expansion'(
		open_db(DB, DBFile),
		(this(This), {open_db(DB, DBFile), '$lgt_current_object_'(This, Prefix, _, _, _, _, _, _, _, _, _), '$lgt_arriba_add_disk_predicate_ddefs'(DB, Prefix)})
	) :-
	logtalk_load_context(entity_type, category).


'$lgt_arriba_load_foreign_library'(Path) :-
...
'$lgt_arriba_add_disk_predicate_ddefs'(DB, Prefix) :-
	database_property(DB, predicates(CPIs)),
	atom_concat(Prefix, '_def', Def),
	atom_concat(Prefix, '_ddef', DDef),
	forall(
		member(CPI, CPIs),
		'$lgt_arriba_add_disk_predicate_ddef'(Def, DDef, CPI)
	).

'$lgt_arriba_add_disk_predicate_ddef'(Def, DDef, CFunctor/CArity) :-
...
'$lgt_arriba_add_disk_predicate_ddef'(_, _, _).
