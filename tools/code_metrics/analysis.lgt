...
:- category(analysis).
    ...
    :- info([
        ...
        remarks is [
            ...
            'Predicate Scope'-'This is meant for internal use by metrics only. As such, all \c
            provided predicates are private.'
        ],
        see_also is [metricp]
    ]).
    ...
    :- info(ancestor/4, [
        comment is 'True if Entity descends from Ancestor, and EntityKind and AncestorKind unify \c
        with their respective entity types.',
    ...
    :- info(defines_predicate/2, [
        comment is 'True if Entity defines an implementation of Predicate internally. Only \c
        internally defined predicates are valid here, therefore predicates with the \c
        \'auxiliary\' property are excluded. Only categories and objects are valid entities.',
        argnames is ['Entity', 'Predicate']
    ]).
    ...
    :- info(defines_predicate/3, [
        comment is 'Same as defines_predicate/2, except Property is unified with a corresponding \c
        property defined in the grammar.',
    ...
    :- info(entity_property/2, [
        comment is 'True if Property is a valid property of Entity. Entity can be either \c
        category, protocol, or object.',
    ...
    :- info(entity_updates/3, [
        comment is 'True if a predicate Updater within Entity makes a dynamic update to Update \c
        (by e.g. asserta/1, retract/1, etc.)',
