...
:- object(subunit_v2_report).

	:- info([
		version is 1:0:0,
		author is 'Paulo Moura',
		date is 2026-03-02,
		comment is 'Intercepts unit test execution messages and generates a ``subunit_v2_report.bin`` file using the Subunit v2 binary streaming format in the same directory as the tests object file.',
		remarks is [
	...
	message_hook(running_tests_from_object_file(_, File)) :-
		...
		atom_concat(Directory, 'subunit_v2_report.bin', ReportFile),
		(   stream_property(_, alias(subunit_v2_report)) ->
			true
		;   open(ReportFile, write, _, [type(binary), alias(subunit_v2_report)])
		).
	% stop
	message_hook(tests_ended) :-
		(   stream_property(_, alias(subunit_v2_report)) ->
			close(subunit_v2_report)
		;   true
		).
	% passed test
	...
	write_packet(Packet) :-
		write_bytes(Packet),
		flush_output(subunit_v2_report).

	write_bytes([]).
	write_bytes([Byte| Bytes]) :-
		put_byte(subunit_v2_report, Byte),
		write_bytes(Bytes).
