...
current directory (or sub-directories) contain one or more `tester.lgt`
files:
...
them will be reported. Second, skipped test sets are not reported. Third, when
using the `xunit` format, dates are reported as MM/DD/YYYY. Finally, when using
the `xunit_net_v2` format, tests are reported in a random order instead of their
run order and dates are displayed as "unknown" in the overview page.
