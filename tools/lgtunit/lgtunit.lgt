...
:- object(lgtunit,
	...
	passed_test(Test, File, Position, Flaky, Note, CPUStart, WallStart, Output) :-
		...
		print_message(information, lgtunit, passed_test(Object, Test, File, Position, Flaky, Note, CPUTime, WallTime)),
		set_output(Current).

	passed_test(Test, File, Position, CPUStart, WallStart, Output) :-
		passed_test(Test, File, Position, false, '', CPUStart, WallStart, Output).
	...
	skipped_test(Test, File, Position, Flaky, Note, Output) :-
		...
		% the test itself doesn't affect printing the test results
		current_output(Current), set_output(Output),
		print_message(information, lgtunit, skipped_test(Object, Test, File, Position, Flaky, Note)),
		set_output(Current).

	skipped_test(Test, File, Position, Output) :-
		skipped_test(Test, File, Position, false, '', Output).
	...
	check_for_valid_test_identifier(Test, File, Position, Type, Entity) :-
		self(Object),
		(	var(Test) ->
			print_message(error, lgtunit, non_instantiated_test_identifier(Object, File, Position, Type, Entity))
		;	\+ callable(Test) ->
			print_message(error, lgtunit, non_callable_test_identifier(Object, Test, File, Position, Type, Entity))
		;	\+ ground(Test) ->
			print_message(error, lgtunit, non_ground_test_identifier(Object, Test, File, Position, Type, Entity))
		;	test_(Test, _) ->
			print_message(error, lgtunit, repeated_test_identifier(Object, Test, File, Position, Type, Entity))
		;	true
		).
	...
	parse_test_options([Option| Options], Test, Condition, Setup, Cleanup, Flaky, Note, NoteFlag, File, Position, Type, Entity) :-
		(	var(Option) ->
			print_message(warning, lgtunit, non_instantiated_test_option(Test, File, Position, Type, Entity))
		...
		;	% ignore but warn non-recognized options
			print_message(warning, lgtunit, invalid_test_option(Test, Option, File, Position, Type, Entity))
		),
		parse_test_options(Options, Test, Condition, Setup, Cleanup, Flaky, Note, NoteFlag, File, Position, Type, Entity).
	...
	extend_quick_check_closure(':'(Module,Closure), (':'(Module,Closure))-(':'(Module,Closure))) :-
		!.
	extend_quick_check_closure(Closure, (Sender<<call(Closure))-Closure) :-
		% the call/1 wrapper is required when the Closure is a lambda expression
		% as otherwise we would try to extend the (<</2) control construct
		sender(Sender).
