...
:- object(ctrf_report).
	...
	:- info([
		...
		date is 2026-02-27,
		comment is 'Intercepts unit test execution messages and generates a ``ctrf_report.json`` file using the CTRF JSON format in the same directory as the tests object file.',
		remarks is [
	...
	message_hook(running_tests_from_object_file(Object, File)) :-
		...
		atom_concat(Directory, 'ctrf_report.json', ReportFile),
		( stream_property(_, alias(ctrf_report)) ->
			true
		;	open(ReportFile, write, _, [alias(ctrf_report)])
		),
	...
	generate_json_report :-
		...
		write(ctrf_report, '{'), nl(ctrf_report),
		write(ctrf_report, '  "reportFormat": "CTRF",'), nl(ctrf_report),
		write(ctrf_report, '  "specVersion": "1.0.0",'), nl(ctrf_report),
		write(ctrf_report, '  "results": {'), nl(ctrf_report),
		write(ctrf_report, '    "tool": {"name": "lgtunit"},'), nl(ctrf_report),
		write(ctrf_report, '    "summary": {'), nl(ctrf_report),
		write(ctrf_report, '      "tests": '), write(ctrf_report, Tests), write(ctrf_report, ','), nl(ctrf_report),
		write(ctrf_report, '      "passed": '), write(ctrf_report, Passed), write(ctrf_report, ','), nl(ctrf_report),
		write(ctrf_report, '      "failed": '), write(ctrf_report, Failed), write(ctrf_report, ','), nl(ctrf_report),
		write(ctrf_report, '      "skipped": '), write(ctrf_report, Skipped), write(ctrf_report, ','), nl(ctrf_report),
		write(ctrf_report, '      "pending": 0,'), nl(ctrf_report),
		write(ctrf_report, '      "other": 0,'), nl(ctrf_report),
		write(ctrf_report, '      "start": '), write(ctrf_report, Start), write(ctrf_report, ','), nl(ctrf_report),
		write(ctrf_report, '      "stop": '), write(ctrf_report, Stop), write(ctrf_report, ','), nl(ctrf_report),
		write(ctrf_report, '      "duration": '), write(ctrf_report, Duration), nl(ctrf_report),
		write(ctrf_report, '    },'), nl(ctrf_report),
		write(ctrf_report, '    "tests": ['), nl(ctrf_report),
		write_test_entries,
		write(ctrf_report, '    ]'), nl(ctrf_report),
		write(ctrf_report, '  }'), nl(ctrf_report),
		write(ctrf_report, '}'), nl(ctrf_report).
	...
	write_test_entries([Entry]) :-
		!,
		write(ctrf_report, '      '),
		write_test_entry(Entry), nl(ctrf_report).
	write_test_entries([Entry| Entries]) :-
		write(ctrf_report, '      '),
		write_test_entry(Entry), write(ctrf_report, ','), nl(ctrf_report),
		write_test_entries(Entries).
	...
	write_test_entry(test(Object, Name, passed_test(File, Position, _Flaky, _Note, _, WallTime))) :-
		...
		write(ctrf_report, '{'),
		write_json_key_string('name', TestName), write(ctrf_report, ', '),
		write_json_key_string_array('suite', [Suite]), write(ctrf_report, ', '),
		write_json_key_string('status', passed), write(ctrf_report, ', '),
		write_json_key_number('duration', Duration), write(ctrf_report, ', '),
		write_json_key_string('filePath', Short), write(ctrf_report, ', '),
		write_json_key_number('line', Line), write(ctrf_report, ', '),
		write_json_key_string('message', ''), write(ctrf_report, ', '),
	...
	write_test_entry(test(Object, Name, failed_test(File, Position, Reason, _Flaky, _Note, _, WallTime))) :-
		...
		write_json_key_string('name', TestName), write(ctrf_report, ', '),
		write_json_key_string_array('suite', [Suite]), write(ctrf_report, ', '),
		write_json_key_string('status', failed), write(ctrf_report, ', '),
		write_json_key_number('duration', Duration), write(ctrf_report, ', '),
		write_json_key_string('filePath', Short), write(ctrf_report, ', '),
		write_json_key_number('line', Line), write(ctrf_report, ', '),
		write_json_key_string('message', Message), write(ctrf_report, ', '),
	...
	write_test_entry(test(Object, Name, skipped_test(File, Position, _Flaky, _Note))) :-
		...
		write_json_key_string('name', TestName), write(ctrf_report, ', '),
		write_json_key_string_array('suite', [Suite]), write(ctrf_report, ', '),
		write_json_key_string('status', skipped), write(ctrf_report, ', '),
		write_json_key_number('duration', 0), write(ctrf_report, ', '),
		write_json_key_string('filePath', Short), write(ctrf_report, ', '),
		write_json_key_number('line', Line), write(ctrf_report, ', '),
		write_json_key_string('message', 'Skipped test'), write(ctrf_report, ', '),
		write_json_key_string('trace', ''),
		write(ctrf_report, '}').
	...
	write_json_string(Value) :-
		...
		write(ctrf_report, '"'), write(ctrf_report, Escaped), write(ctrf_report, '"').
