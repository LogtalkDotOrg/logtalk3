...
<xsl:template match="/">
	...
	<body>
		<div class="header">
			...
			<xsl:if test="logtalk_entity/entity/comment or logtalk_entity/entity/parameters">
			<blockquote>
		...
		<xsl:if test="arguments">
			<dd class="value"><ul class="arguments">
                <xsl:for-each select="arguments/argument">
                	<li><code><xsl:value-of select="name" /></code><xsl:text disable-output-escaping="yes"> &amp;ndash; </xsl:text><span class="comment">
                		<xsl:call-template name="process-inline"><xsl:with-param name="text" select="description" /></xsl:call-template>
                	</span></li>
            	</xsl:for-each></ul></dd>
		</xsl:if>
		<xsl:if test="meta">
