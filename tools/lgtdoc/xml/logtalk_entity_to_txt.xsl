...
<xsl:template match="logtalk_entity/relations">
	<xsl:choose>
		...
		<xsl:otherwise>
			...
			<xsl:value-of select="$tab" /><xsl:text>(none)</xsl:text><xsl:value-of select="$nl" />
		</xsl:otherwise>
	</xsl:choose>
	<xsl:value-of select="$nl" />
</xsl:template>
...
<xsl:template match="logtalk_entity/predicates">
	...
	<xsl:choose>
		...
		<xsl:when test="/logtalk_entity/relations/*">
			<xsl:text>(no local declarations; see entity ancestors if any)</xsl:text><xsl:value-of select="$nl2" />
		</xsl:when>
		<xsl:otherwise>
			<xsl:text>(none)</xsl:text><xsl:value-of select="$nl2" />
		</xsl:otherwise>
...
<xsl:template match="*/predicate">
	...
	<xsl:if test="since">
		<xsl:text>since:</xsl:text><xsl:value-of select="$nl" />
			<xsl:value-of select="$tab" /><xsl:value-of select="since" /><xsl:value-of select="$nl" />
	</xsl:if>
