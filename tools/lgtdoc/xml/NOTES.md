...
formats such as PDF, ePub, (X)HTML, HTML 5, reStructuredText, Markdown, and
plain text files using XSL style sheets. The documenting files may also be
viewed directly on a web browser that supports the W3C standards XML,
XSLT, CSS, and HTML 4, XHTML 1.0, or HTML 5.
...
- `logtalk_index_to_xml.xsl`  
	XSLT files for viewing XML files directly in a browser. The links
...
- `logtalk_index_to_html.xsl`  
	XSLT files to output HTML 4.01 files from the XML files. The links in
...
- `logtalk_index_to_xhtml.xsl`  
	XSLT files to output XHTML 1.1 files from the XML files. The links in
	the generated XHTML files point to related XHTML files. UTF-8 encoding
	is assumed. Edit the file if you use a different encoding. This XSLT
	file adds anchors to each predicate section using as name the predicate
	indicator.

- `logtalk_entity_to_html_5.xsl`  
- `logtalk_index_to_html_5.xsl`  
	XSLT files to output HTML 5 files from the XML files. The links in
	the generated HTML files point to related HTML files. UTF-8 encoding
	is assumed. Edit the file if you use a different encoding. This XSLT
	file adds anchors to each predicate section using as name the predicate
	indicator.
...
- `logtalk_index_to_md.xsl`  
	XSLT files to output Markdown text files from the XML files. UTF-8 encoding
	is assumed. Edit the file if you use a different encoding.

- `logtalk_entity_to_rst.xsl`  
- `logtalk_index_to_rst.xsl`  
	XSLT files to output reStructuredText files (for use with Sphinx) from the
...
- `logtalk_index_to_txt.xsl`  
	XSLT files to output simple text files from the XML files. UTF-8 encoding
	is assumed. Edit the file if you use a different encoding.
...
- `logtalk_entity.rng`  
- `logtalk_index.rng`  
	RELAX NG files describing the structure of the entity and index XML
	files by Logtalk documenting tools.

- `logtalk_entity.xsd`  
- `logtalk_index.xsd`  
	XML Schema files describing the structure of the entity and index XML
...
- `lgt2html.sh` and `lgt2html.ps1`  
	Sample scripts to batch convert XML files to XHTML, HTML 4.01, or HTML 5
	files. These scripts also generate an `index.html` file which contains
	links to all the converted files.
