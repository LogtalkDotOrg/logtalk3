...
`diagrams` tool. There's also preliminary support for Mermaid (its current
version lacks several required features for parity with d2 and Graphviz,
notably support for links on edges).

JSON output format is also supported for use with Cytoscape. The Cytoscape
backend generates `.cx2` files using the Cytoscape Exchange Format version
2 (CX2), including core aspects such as `attributeDeclarations`,
`networkAttributes`, `nodes`, `edges`, `visualProperties`, and `status`.
Node and edge styling is exported using CX2 visual property mappings based
...
are provided for converting a directory of `.d2` or `.dot` files to `.svg`
files:
