...
Open the [../../apis/library_index.html#memcached](../../apis/library_index.html#memcached)
...
Note: Tests require a running Memcached server on `localhost` port `11211`
(the default). To install and start Memcached:
...
This library implements the Memcached text (ASCII) protocol as documented
...
If another client modifies the value between `gets` and `cas`, the `cas`
command will throw `error(memcached_error(exists), _)`.
...
Note: The item must already exist and contain a decimal representation of
a 64-bit unsigned integer. Decrementing below 0 yields 0.
...
Most predicates throw `error(memcached_error(Reason), Context)` on failure.
...
Retrieval predicates (`get`, `gets`, `gat`, `gats`) fail (rather than
throw) when the requested key is not found. The `delete`, `incr`, `decr`,
and `touch` predicates also fail when the key is not found.
