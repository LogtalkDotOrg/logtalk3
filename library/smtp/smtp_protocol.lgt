...
:- protocol(smtp_protocol).
	...
	:- info(send/5, [
		...
		exceptions is [
			...
			'``Body`` is neither an atom nor a list of 7-bit character codes' - domain_error(smtp_body, 'Body'),
			'``Code`` in the message body is not a 7-bit character code' - domain_error(smtp_body_code, 'Code'),
			...
			'The SMTP server closes the connection while a response is expected' - smtp_error(connection_closed),
