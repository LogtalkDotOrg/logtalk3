...
:- object(bfgs(_Problem_),
	...
	:- info([
		...
		see_also is [
			local_optimization_problem_protocol, local_optimization_solver(_), gradient_descent(_), conjugate_gradient(_)
		]
	]).
	...
	update_inverse_hessian(_Iter, Dim, _Restart, S, Y, H0, H1) :-
		...
		;	% curvature condition not (comfortably) satisfied: this
			...
			% happen. Resetting to the identity, rather than keeping
			% the stale approximation, avoids repeatedly recomputing
			% the same near-zero-progress direction from an
			% approximation that is no longer consistent with the
			% local curvature.
			identity_matrix(Dim, H1)
		).
