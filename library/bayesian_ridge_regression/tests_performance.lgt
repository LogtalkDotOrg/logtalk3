...
:- object(tests_performance,
	...
	:- info([
		...
		date is 2026-05-03,
		comment is 'Performance and reference-fit benchmarks for the "bayesian_ridge_regression" library.'
	]).
	...
	test(simple_line_reference_fit, true, [note(metrics(train_seconds-TrainTime, rmse-RMSE, mae-MAE))]) :-
		reference_fit(simple_line, [feature_scaling(false), maximum_iterations(500), tolerance(1.0e-10)], 0.05, 0.05, TrainTime, RMSE, MAE).

	test(plane_reference_fit, true, [note(metrics(train_seconds-TrainTime, rmse-RMSE, mae-MAE))]) :-
		reference_fit(plane, [feature_scaling(false), maximum_iterations(500), tolerance(1.0e-10)], 0.10, 0.10, TrainTime, RMSE, MAE).

	test(mixed_signal_reference_fit, true, [note(metrics(train_seconds-TrainTime, rmse-RMSE, mae-MAE))]) :-
		reference_fit(mixed_signal, [], 1.50, 1.50, TrainTime, RMSE, MAE).

	test(wide_mixed_signal_reference_fit, true, [note(metrics(train_seconds-TrainTime, rmse-RMSE, mae-MAE))]) :-
		reference_fit(wide_mixed_signal, [], 0.50, 0.50, TrainTime, RMSE, MAE).
	...
	reference_fit(Dataset, Options, MaximumRMSE, MaximumMAE, TrainTime, RMSE, MAE) :-
		benchmark(bayesian_ridge_regression::learn(Dataset, _Regressor, Options), TrainTime),
		bayesian_ridge_regression::learn(Dataset, Regressor, Options),
	...
	evaluate_examples([], _Regressor, SumSquaredError, SumSquaredError, SumAbsoluteError, SumAbsoluteError).
	evaluate_examples([Target-AttributeValues| Examples], Regressor, SumSquaredError0, SumSquaredError, SumAbsoluteError0, SumAbsoluteError) :-
		bayesian_ridge_regression::predict(Regressor, AttributeValues, Prediction),
