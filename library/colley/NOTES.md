...
Open the [../../apis/library_index.html#colley](../../apis/library_index.html#colley)
...
- **Colley Matrix Fidelity**: Solves the standard Colley linear system with
  diagonal regularization `2 + games_i` and off-diagonal entries `-games_ij`.
...
- **Conservative Ratings**: Produces ratings in the interval `[0,1]` with the
  neutral prior centered at `0.5`.
...
boundary noise at `0.0` or `1.0`; materially invalid out-of-range values are
rejected instead of being silently accepted.

The resulting ratings remain close to `0.5` when evidence is weak and move
toward `1.0` or `0.0` only when repeated pairwise results justify doing so.
