...
:- object(tests_performance,
	...
	:- info([
		...
		date is 2026-04-22,
		comment is 'Performance and reference-fit benchmarks for the "gradient_boosting_regression" library.'
	]).
	...
	test(step_signal_reference_fit, true, [note(metrics(train_seconds-TrainTime, rmse-RMSE, mae-MAE))]) :-
		reference_fit(step_signal, [number_of_estimators(5), learning_rate(1.0), maximum_depth(2)], 0.01, 0.01, TrainTime, RMSE, MAE).

	test(mixed_signal_reference_fit, true, [note(metrics(train_seconds-TrainTime, rmse-RMSE, mae-MAE))]) :-
		reference_fit(mixed_signal, [number_of_estimators(3), learning_rate(1.0), maximum_depth(4), feature_scaling(false)], 0.01, 0.01, TrainTime, RMSE, MAE).

	test(wide_mixed_signal_reference_fit, true, [note(metrics(train_seconds-TrainTime, rmse-RMSE, mae-MAE))]) :-
		reference_fit(wide_mixed_signal, [number_of_estimators(20), learning_rate(0.3), maximum_depth(3), minimum_samples_leaf(1), feature_scaling(false)], 0.75, 0.60, TrainTime, RMSE, MAE).

	test(intercept_only_reference_fit, true, [note(metrics(train_seconds-TrainTime, rmse-RMSE, mae-MAE))]) :-
		reference_fit(intercept_only, [number_of_estimators(5), learning_rate(1.0)], 0.01, 0.01, TrainTime, RMSE, MAE).

	reference_fit(Dataset, Options, MaximumRMSE, MaximumMAE, TrainTime, RMSE, MAE) :-
		benchmark(gradient_boosting_regression::learn(Dataset, _Regressor, Options), TrainTime),
		gradient_boosting_regression::learn(Dataset, Regressor, Options),
	...
	evaluate_examples([Target-AttributeValues| Examples], Regressor, SumSquaredError0, SumSquaredError, SumAbsoluteError0, SumAbsoluteError) :-
		gradient_boosting_regression::predict(Regressor, AttributeValues, Prediction),
