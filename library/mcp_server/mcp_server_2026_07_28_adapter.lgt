...
:- object(mcp_server_2026_07_28_adapter,
	...
	handle_request(Message, _Input, Output, Options) :-
		...
		(	valid_request_id(Id) ->
			true
		;	% Cannot respond without a valid id; ignore
			!
		),
		...
		;	% Validation already wrote an error response
			true
		).
	...
	build_server_capabilities(ApplicationCapabilities, Capabilities) :-
		Base = [tools-{}],
		(	member(prompts, ApplicationCapabilities) -> C1 = [prompts-{}| Base] ; C1 = Base),
		(	member(resources, ApplicationCapabilities) -> C2 = [resources-{}| C1] ; C2 = C1),
	...
	format_complete_result(error(Error), tool, _, Id, Output, _) :-
		!,
		(	atom(Error) -> ErrorText = Error ; write_to_atom(Error, ErrorText)),
	...
	make_progress_closure(Token, RequestId, Output, Progress) :-
		Progress = {Token, RequestId, Output}/[ProgressValue, Total, Message]>>(
			mcp_server_2026_07_28_adapter::emit_progress(Token, RequestId, Output, ProgressValue, Total, Message)
		).

	:- public(emit_progress/6).
	:- mode(emit_progress(+term, +term, +stream, +number, +number, +atom), one).
	:- info(emit_progress/6, [
		comment is 'Emits a notifications/progress notification for the current request.',
		argnames is ['Token', 'RequestId', 'Output', 'ProgressValue', 'Total', 'Message']
	]).

	emit_progress(Token, _RequestId, Output, ProgressValue, Total, Message) :-
		Params = {
			progressToken-Token,
		...
		write_locked(Output, Notification).
	...
	handle_subscriptions_listen(Params, Id, Output) :-
		...
		% Generate subscription id from request id
		(	atom(Id) -> SubId = Id
		...
		write_locked(Output, Ack),
		% Register; lives until notifications/cancelled for this request id or EOF
	...
	event_matches(Event, Filters) :-
		...
		(	^^has_pair(Filter, type, Type) -> true
		;	Filter == Type -> true
		;	fail
		), !.
	...
	send_error(Id, Code, Message, Output) :-
		...
		error_response(Code, Msg, Id, Response),
		write_locked(Output, Response).

	send_error_data(Id, Code, Message, Data, Output) :-
		(	atom(Message) -> Msg = Message ; write_to_atom(Message, Msg)),
		ErrorObj = {code-Code, message-Msg, data-Data},
