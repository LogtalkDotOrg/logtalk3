...
specification (tools, prompts, and elicitation capabilities).
...
descriptors that declare which predicates are exposed as MCP tools:
...
object, collects output-mode arguments (`-` and `--`), and returns them
as text content.
...
To use the server with an MCP client (e.g., VSCode or Claude Desktop), configure
it as a stdio server. Example `claude_desktop_config.json`:
...
- `capabilities/1` — returns the list of additional capabilities needed
  by the application (e.g. `[elicitation]`, `[prompts]`, or
