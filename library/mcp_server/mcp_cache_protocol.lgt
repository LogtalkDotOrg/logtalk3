...
:- protocol(mcp_cache_protocol).
	...
	:- info([
		...
		date is 2026-08-14,
		comment is 'Optional protocol for per-operation cache policy used by the MCP 2026-07-28 specification. An application may implement ``cache_policy/4`` to choose TTL (milliseconds) and scope (``public`` or ``private``) for complete results of ``server/discover``, list operations (``tools/list``, ``prompts/list``, ``resources/list``), and ``resources/read``. Cache fields are never attached to ``input_required`` results or MRTR retries. When the predicate is not defined, or for an operation it does not cover, the server falls back to the options ``cache_ttl(0)`` and ``cache_scope(private)``.',
		remarks is [
