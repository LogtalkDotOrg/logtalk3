...
:- protocol(mcp_resource_protocol).
	...
	:- info([
		...
		remarks is [
			...
			'Resource template descriptors' - 'Parameterized resources are optionally described by ``resource_template(URITemplate, Name, Description, MimeType)`` or ``resource_template(URITemplate, Name, Title, Description, MimeType)`` terms. ``URITemplate`` is an RFC 6570 URI template atom. The server advertises templates without expanding them; applications validate, authorize, and read concrete expanded URIs using ``resource_read/3``.',
