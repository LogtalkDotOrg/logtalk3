...
decode text strings into lists of chars or codes, use the `json/1` with the
...
Some tests, notably `parse_simple_valid_files` and `roundtrip_hexadecimals`,
fail on backends such as ECLiPSe and GNU Prolog that don't support Unicode.
