...
:- object(tests,
	...
	test(knn_valid_classifier_1, deterministic(knn_classifier::valid_classifier(Classifier))) :-
		knn_classifier::learn(iris_small, Classifier).

	test(knn_invalid_classifier_1, fail) :-
		knn_classifier::valid_classifier(knn_classifier([sepal_length], [numeric], [[wide]-setosa])).

	% Test predict/3 with iris dataset (continuous features)
	test(knn_predict_3_iris_setosa, true(Prediction == setosa)) :-
		knn_classifier::learn(iris_small, Classifier),
		knn_classifier::predict(Classifier, [sepal_length-5.0, sepal_width-3.3, petal_length-1.4, petal_width-0.2], Prediction).

	test(knn_predict_3_iris_versicolor, true(Prediction == versicolor)) :-
		knn_classifier::learn(iris_small, Classifier),
		knn_classifier::predict(Classifier, [sepal_length-6.5, sepal_width-3.0, petal_length-4.6, petal_width-1.4], Prediction).

	test(knn_predict_3_iris_virginica, true(Prediction == virginica)) :-
		knn_classifier::learn(iris_small, Classifier),
		knn_classifier::predict(Classifier, [sepal_length-6.7, sepal_width-3.1, petal_length-5.6, petal_width-2.4], Prediction).

	% Test predict/4 with different options
	test(knn_predict_4_different_k, true(ground(Prediction))) :-
		knn_classifier::learn(iris_small, Classifier),
		knn_classifier::predict(Classifier, [sepal_length-5.0, sepal_width-3.3, petal_length-1.4, petal_width-0.2], Prediction, [k(5)]).

	test(knn_predict_4_different_metric, true(ground(Prediction))) :-
		knn_classifier::learn(iris_small, Classifier),
		knn_classifier::predict(Classifier, [sepal_length-5.0, sepal_width-3.3, petal_length-1.4, petal_width-0.2], Prediction, [distance_metric(manhattan)]).
	...
	test(knn_predict_probabilities_4_uniform, true(check(list(pair(atom,probability)),Probabilities))) :-
		knn_classifier::learn(iris_small, Classifier),
		knn_classifier::predict_probabilities(Classifier, [sepal_length-5.0, sepal_width-3.3, petal_length-1.4, petal_width-0.2], Probabilities, [weight_scheme(uniform)]).

	test(knn_predict_probabilities_4_distance, true(check(list(pair(atom,probability)),Probabilities))) :-
		knn_classifier::learn(iris_small, Classifier),
		knn_classifier::predict_probabilities(Classifier, [sepal_length-5.0, sepal_width-3.3, petal_length-1.4, petal_width-0.2], Probabilities, [weight_scheme(distance)]).

	test(knn_predict_probabilities_4_gaussian, true(check(list(pair(atom,probability)),Probabilities))) :-
		knn_classifier::learn(iris_small, Classifier),
		knn_classifier::predict_probabilities(Classifier, [sepal_length-5.0, sepal_width-3.3, petal_length-1.4, petal_width-0.2], Probabilities, [weight_scheme(gaussian)]).

	% Test learn/2 with categorical weather dataset
	test(knn_learn_2_weather, true(ground(Classifier))) :-
		knn_classifier::learn(weather, Classifier).

	% Test predict/3 with weather dataset (categorical features)
	test(knn_predict_3_weather_01, true(Prediction == yes)) :-
		knn_classifier::learn(weather, Classifier),
		knn_classifier::predict(Classifier, [outlook-overcast, temperature-hot, humidity-normal, wind-weak], Prediction).

	% Test learn/2 with mixed dataset (categorical + continuous)
	test(knn_learn_2_mixed, true(ground(Classifier))) :-
		knn_classifier::learn(mixed, Classifier).

	% Test predict/3 with mixed dataset
	test(knn_predict_3_mixed_01, true(ground(Prediction))) :-
		knn_classifier::learn(mixed, Classifier),
		knn_classifier::predict(Classifier, [age-26, income-36000, student-yes, credit_rating-fair], Prediction).

	% Test export_to_clauses/4 - verify exported clause works with predict/3
	test(knn_export_to_clauses_4, true(ground(Prediction))) :-
		knn_classifier::learn(iris_small, Classifier),
		knn_classifier::export_to_clauses(_Dataset, Classifier, classifier, [classifier(ExportedClassifier)]),
		knn_classifier::predict(ExportedClassifier, [sepal_length-5.0, sepal_width-3.3, petal_length-1.4, petal_width-0.2], Prediction).

	% Test export_to_file/4 - verify file is written
	test(knn_export_to_file_4_written, deterministic(os::file_exists(File))) :-
		^^file_path('test_output.pl', File),
		knn_classifier::learn(iris_small, Classifier),
		knn_classifier::export_to_file(iris_small, Classifier, classify, File).
	...
	test(knn_export_to_file_4_loaded, true(ground(Prediction))) :-
		^^file_path('test_output.pl', File),
		knn_classifier::learn(iris_small, Classifier),
		knn_classifier::export_to_file(iris_small, Classifier, classifier, File),
		logtalk_load(File),
	...
	test(knn_print_classifier_1, true) :-
		...
		knn_classifier::learn(iris_small, Classifier),
		knn_classifier::print_classifier(Classifier).

	% Test different distance metrics
	test(knn_distance_metric_euclidean, true(ground(Prediction))) :-
		knn_classifier::learn(iris_small, Classifier),
		knn_classifier::predict(Classifier, [sepal_length-5.0, sepal_width-3.3, petal_length-1.4, petal_width-0.2], Prediction, [distance_metric(euclidean)]).
	...
	test(knn_distance_metric_chebyshev, true(ground(Prediction))) :-
		knn_classifier::learn(iris_small, Classifier),
		knn_classifier::predict(Classifier, [sepal_length-5.0, sepal_width-3.3, petal_length-1.4, petal_width-0.2], Prediction, [distance_metric(chebyshev)]).

	test(knn_distance_metric_minkowski, true(ground(Prediction))) :-
		knn_classifier::learn(iris_small, Classifier),
		knn_classifier::predict(Classifier, [sepal_length-5.0, sepal_width-3.3, petal_length-1.4, petal_width-0.2], Prediction, [distance_metric(minkowski)]).
