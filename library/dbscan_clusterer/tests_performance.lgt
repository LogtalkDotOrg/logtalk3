...
:- object(tests_performance,
	...
	:- info([
		...
		date is 2026-05-06,
		comment is 'Performance and reference-fit benchmarks for the "dbscan_clusterer" library.'
	]).
	...
	test(two_blobs_reference_fit, true, [note(metrics(train_seconds-TrainTime, cluster_count-ClusterCount, noise_count-NoiseCount, min_core_distance-MinCoreDistance))]) :-
		reference_fit(two_blobs, [epsilon(0.6), minimum_points(2), feature_scaling(off)], 2, 0, 5.5, TrainTime, ClusterCount, NoiseCount, MinCoreDistance).
	...
	test(iris_unlabeled_reference_fit, true, [note(metrics(train_seconds-TrainTime, cluster_count-ClusterCount, noise_count-NoiseCount, min_core_distance-MinCoreDistance))]) :-
		reference_fit(iris_unlabeled, [epsilon(1.25), minimum_points(2)], 2, 3, 3.0, TrainTime, ClusterCount, NoiseCount, MinCoreDistance).
	...
	reference_fit(Dataset, Options, ExpectedClusterCount, MaximumNoiseCount, MinimumCoreDistance, TrainTime, ClusterCount, NoiseCount, MinCoreDistance) :-
		benchmark(dbscan_clusterer::learn(Dataset, _Clusterer, Options), TrainTime),
		dbscan_clusterer::learn(Dataset, Clusterer, Options),
	...
	clusterer_metrics(dbscan_clusterer(_Encoders, Clusters, Noise, _Options), ClusterCount, NoiseCount, MinCoreDistance) :-
