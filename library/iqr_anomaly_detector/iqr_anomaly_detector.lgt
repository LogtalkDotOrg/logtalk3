...
:- object(iqr_anomaly_detector,
	...
	:- info([
		...
		see_also is [anomaly_dataset_protocol, anomaly_detector_protocol, modified_z_score_anomaly_detector, z_score_anomaly_detector, isolation_forest_anomaly_detector, knn_distance_anomaly_detector, lof_anomaly_detector]
	]).
	...
	build_encoders([Attribute| Attributes], Examples, [iqr_anomaly_detector(Attribute, FirstQuartile, ThirdQuartile, Scale)| Encoders]) :-
	...
	known_iqr_scores([iqr_anomaly_detector(_Attribute, FirstQuartile, ThirdQuartile, Scale)| Encoders], [Value| Values], Options, IQRScores, KnownCount0, KnownCount, PositiveCount0, PositiveCount) :-
		(	var(Value) ->
			IQRScores = Tail,
	...
	encoder_attribute_names([iqr_anomaly_detector(Attribute, _FirstQuartile, _ThirdQuartile, _Scale)| Encoders], [Attribute| AttributeNames]) :-
		encoder_attribute_names(Encoders, AttributeNames).
	...
	valid_iqr_encoders_([iqr_anomaly_detector(Attribute, FirstQuartile, ThirdQuartile, Scale)| Encoders], SeenAttributes) :-
		atom(Attribute),
	...
	valid_detector_diagnostics(Dataset, Encoders, Diagnostics) :-
		...
		memberchk(model(iqr_anomaly_detector), Diagnostics),
