...
:- object(tests,
	...
	:- info([
		...
		date is 2026-05-01,
		comment is 'Unit tests for the "cusum" library.'
	]).
	...
	test(cusum_invalid_anomaly_detector_1, error(domain_error(anomaly_detector, cusum_detector(cusum_shift_sequences, attribute_schema([t2], [t2-1]), [cusum_encoder(t1, 0.0, 1.0)], [model(cusum), training_dataset(cusum_shift_sequences), attribute_names([t1]), feature_count(1), example_count(0), options([anomaly_threshold(0.8333333333333334), allowance(0.5), baseline_class_values([normal]), baseline_selection_policy(reject), decision_interval(5.0)])])))) :-
		check_anomaly_detector(cusum_detector(cusum_shift_sequences, attribute_schema([t2], [t2-1]), [cusum_encoder(t1, 0.0, 1.0)], [model(cusum), training_dataset(cusum_shift_sequences), attribute_names([t1]), feature_count(1), example_count(0), options([anomaly_threshold(0.8333333333333334), allowance(0.5), baseline_class_values([normal]), baseline_selection_policy(reject), decision_interval(5.0)])])).

	test(cusum_diagnostics_2, deterministic((memberchk(model(cusum), Diagnostics), memberchk(training_dataset(cusum_shift_sequences), Diagnostics), memberchk(feature_count(6), Diagnostics), memberchk(example_count(8), Diagnostics)))) :-
		learn_filtered_shift_sequences(Detector),
		diagnostics(Detector, Diagnostics).
