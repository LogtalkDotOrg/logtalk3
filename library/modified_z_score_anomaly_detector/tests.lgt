...
:- object(tests,
	...
	:- uses(modified_z_score_anomaly_detector, [
	...
	cover(modified_z_score_anomaly_detector).
	...
	test(modified_z_score_learn_3_empty_dataset_error, error(domain_error(non_empty_dataset, modified_z_score_empty_anomalies))) :-
		learn(modified_z_score_empty_anomalies, _Detector, []).

	test(modified_z_score_learn_2_featureless_dataset_error, error(domain_error(non_empty_features, modified_z_score_featureless_anomalies))) :-
	...
	test(modified_z_score_score_3_singleton_dataset_matching_point, true(Score =:= 0.0)) :-
		learn(modified_z_score_singleton_anomalies, Detector),
	...
	test(modified_z_score_predict_4_score_mode_override_ignored, true(Prediction == normal)) :-
		learn(modified_z_score_high_dimensional_anomalies, Detector),
		predict(Detector, [x1-6.0, x2-0.0, x3-0.0, x4-0.0, x5-0.0, x6-0.0, x7-0.0, x8-0.0, x9-0.0, x10-0.0], Prediction, [score_mode(any_feature_extreme)]).

	test(modified_z_score_predict_3_singleton_far_point, true(Prediction == anomaly)) :-
		learn(modified_z_score_singleton_anomalies, Detector),
		predict(Detector, [x-7.00], Prediction).

	test(modified_z_score_predict_3_high_dimensional_training_normal, true(Prediction == normal)) :-
		learn(modified_z_score_high_dimensional_anomalies, Detector),
		predict(Detector, [x1-1.0, x2-1.0, x3-1.0, x4-1.0, x5-1.0, x6-1.0, x7-1.0, x8-1.0, x9-1.0, x10-1.0], Prediction).

	test(modified_z_score_predict_3_high_dimensional_anomaly, true(Prediction == anomaly)) :-
		learn(modified_z_score_high_dimensional_anomalies, Detector),
		predict(Detector, [x1-6.0, x2-6.0, x3-6.0, x4-6.0, x5-6.0, x6-6.0, x7-6.0, x8-6.0, x9-6.0, x10-6.0], Prediction).

	test(modified_z_score_predict_3_high_dimensional_sparse_anomaly_dense_mode, true(Prediction == normal)) :-
		learn(modified_z_score_high_dimensional_anomalies, Detector),
		predict(Detector, [x1-6.0, x2-0.0, x3-0.0, x4-0.0, x5-0.0, x6-0.0, x7-0.0, x8-0.0, x9-0.0, x10-0.0], Prediction).

	test(modified_z_score_predict_3_high_dimensional_sparse_anomaly_any_feature_extreme, true(Prediction == anomaly)) :-
		learn(modified_z_score_high_dimensional_anomalies, Detector, [score_mode(any_feature_extreme)]),
		predict(Detector, [x1-6.0, x2-0.0, x3-0.0, x4-0.0, x5-0.0, x6-0.0, x7-0.0, x8-0.0, x9-0.0, x10-0.0], Prediction).

	test(modified_z_score_score_3_missing_value_dimension_normalization, true(Delta < 1.0e-12)) :-
		learn(modified_z_score_high_dimensional_anomalies, Detector),
