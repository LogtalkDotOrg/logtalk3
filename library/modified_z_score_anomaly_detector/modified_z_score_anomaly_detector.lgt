...
:- object(modified_z_score_anomaly_detector,
	...
	:- info([
		...
		see_also is [anomaly_dataset_protocol, anomaly_detector_protocol, z_score_anomaly_detector, isolation_forest_anomaly_detector, knn_distance_anomaly_detector, lof_anomaly_detector]
	]).
	...
	build_diagnostics(Dataset, AttributeNames, ExampleCount, Options, Diagnostics) :-
		...
		Diagnostics = [
			model(modified_z_score_anomaly_detector),
	...
	valid_detector_diagnostics(Dataset, Encoders, Diagnostics) :-
		...
		memberchk(model(modified_z_score_anomaly_detector), Diagnostics),
