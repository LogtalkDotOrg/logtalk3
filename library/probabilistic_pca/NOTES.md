...
Open the [../../apis/library_index.html#probabilistic_pca](../../apis/library_index.html#probabilistic_pca)
...
To load this library, load the `loader.lgt` file:
	...
	| ?- logtalk_load(probabilistic_pca(loader)).
...
To test this library predicates, load the `tester.lgt` file:
	...
	| ?- logtalk_load(probabilistic_pca(tester)).
...
- `n_components/1`: Number of latent dimensions to extract. Requests that exceed the structural PPCA limit `min(FeatureCount, SampleCount - 1)` raise `domain_error(component_count, Requested-Maximum)`. The default is `2`.
...
### Learning a reducer
	...
	| ?- probabilistic_pca::learn(correlated_plane, DimensionReducer, [n_components(1), feature_scaling(false), shortfall_policy(error)]).

### Transforming new instances

	| ?- probabilistic_pca::learn(correlated_plane, DimensionReducer, [n_components(2)]),
	     probabilistic_pca::transform(DimensionReducer, [x-2.0, y-4.0, z-6.0], ReducedInstance).

### Exporting and reusing the reducer

	| ?- probabilistic_pca::learn(correlated_plane, DimensionReducer, [n_components(1)]),
	     probabilistic_pca::export_to_file(correlated_plane, DimensionReducer, reducer, 'probabilistic_pca_reducer.pl').
...
- `Diagnostics`: Learned metadata including the effective training options, sample count, retained explained variances, estimated noise variance, preprocessing details, and optional truncate-mode shortfall details.
