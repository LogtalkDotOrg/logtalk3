...
:- object(tests,
	...
	test(spade_diagnostics_2, deterministic((memberchk(model(spade_pattern_miner), Diagnostics), memberchk(search_strategy(lattice_depth_first_growth), Diagnostics), memberchk(candidate_generation(equivalence_class_temporal_joins), Diagnostics), memberchk(support_layout(vertical_occurrence_lists), Diagnostics)))) :-
		spade_pattern_miner::mine(same_event_vs_next_event_sequences, PatternMiner, [minimum_support_count(2), maximum_pattern_length(2)]),
