...
To learn an isolation forest model from a dataset with default options:
  ...
  | ?- isolation_forest::learn(gaussian_anomalies, Model, [baseline_selection_policy(filter)]).
...
level to 0:
  ...
  | ?- isolation_forest::learn(gaussian_anomalies, Model, [baseline_selection_policy(filter), extension_level(0)]).
