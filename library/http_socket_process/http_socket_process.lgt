...
:- object(http_socket_process,
	...
	resolve_listener_executable(Executable, _ResolvedExecutable, _ListenerExecutableKind) :-
		domain_error(http_socket_process_listener_helper_executable, Executable).
	...
	startup_listener_outcome(ncat, Line, _Context, _BoundPort, continue) :-
		...
		!.
	startup_listener_outcome(_ListenerExecutableKind, end_of_file, Context, _BoundPort, error(error(resource_error(http_socket_process_listener), Context))).
	...
	listener_id(Listener, _ListenerId) :-
		(	var(Listener) ->
			instantiation_error
		;	domain_error(http_socket_listener, Listener)
		).
	...
	parse_connection_pool_options(Options, MinSize, MaxSize, ConnectionOptions) :-
		...
		(	MinSize =< MaxSize ->
			true
		;	domain_error(http_socket_process_connection_pool_options, [min_size(MinSize), max_size(MaxSize)])
		).
	...
	check_connection_option(Option) :-
		...
		!.
	check_connection_option(Option) :-
		domain_error(http_socket_process_connection_option, Option).
	...
	check_listener_socket_option(Option) :-
		...
		!.
	check_listener_socket_option(Option) :-
		domain_error(http_socket_process_listener_option, Option).
	...
	check_connection_pool_option(connection_options(ConnectionOptions)) :-
		...
		!.
	check_connection_pool_option(Option) :-
		domain_error(http_socket_process_connection_pool_option, Option).
	...
	request_shutdown_impl(_Control) :-
		throw(error(resource_error(threads), http_socket_process::request_shutdown(_))).

	serve_listener_with_workers_impl(per_connection, _Listener, _Handler, _Count, _ClientInfos) :-
		throw(error(resource_error(threads), http_socket_process::serve_listener(_, _, _, _, [workers(per_connection)]))).
	serve_listener_with_workers_impl(pool(_Size), _Listener, _Handler, _Count, _ClientInfos) :-
		throw(error(resource_error(threads), http_socket_process::serve_listener(_, _, _, _, [workers(pool(_))]))).
	serve_listener_with_workers_impl(pool(_Size, rolling), _Listener, _Handler, _Count, _ClientInfos) :-
		throw(error(resource_error(threads), http_socket_process::serve_listener(_, _, _, _, [workers(pool(_, rolling))]))).
