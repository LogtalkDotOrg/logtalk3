...
:- object(tests,
	...
	test(ccsds_tm_frames_parse_2_01, true(Frames == [tm_transfer_frame(0, 42, 3, 1, 16, 32, 0, 0, 0, 3, 0, none, [1,2,3,4,5,6], ocf([0xDE,0xAD,0xBE,0xEF]), none)])) :-
		parse(bytes([0x02, 0xA7, 0x10, 0x20, 0x18, 0x00, 1, 2, 3, 4, 5, 6, 0xDE, 0xAD, 0xBE, 0xEF]), Frames).

	test(ccsds_tm_frames_parse_2_02, error(domain_error(ccsds_frame_byte_sequence, _))) :-
		parse(bytes([0x00, 0x01, 0x02]), _).

	test(ccsds_tm_frames_generate_2_01, true(Bytes == [0x02, 0xA7, 0x10, 0x20, 0x18, 0x00, 1, 2, 3, 4, 5, 6, 0xDE, 0xAD, 0xBE, 0xEF])) :-
		generate(bytes(Bytes), [tm_transfer_frame(0, 42, 3, 1, 16, 32, 0, 0, 0, 3, 0, none, [1,2,3,4,5,6], ocf([0xDE,0xAD,0xBE,0xEF]), none)]).

	test(ccsds_tm_frames_generate_3_01, true(Bytes == [0x02, 0xA7, 0x10, 0x20, 0x18, 0x00, 1, 2, 3, 4, 5, 6, 0xDE, 0xAD, 0xBE, 0xEF, 0xFF])) :-
	...
	test(ccsds_tm_frames_version_2_01, deterministic(Value == 0)) :-
		parse(bytes([0x02, 0xA7, 0x10, 0x20, 0x18, 0x00, 1, 2, 3, 4, 5, 6, 0xDE, 0xAD, 0xBE, 0xEF]), [Frame]),
		version(Frame, Value).
	...
	test(ccsds_tm_frames_parse_fecf_01, true(Frames == [tm_transfer_frame(0, 42, 3, 0, 16, 32, 1, 0, 0, 3, 0, secondary_header([0xAA,0xBB]), [1,2,3,4,5,6,7,8], none, fecf([0x28,0x97]))])) :-
		ccsds_tm_frames(18, 2, true)::parse(bytes([0x02, 0xA6, 0x10, 0x20, 0x98, 0x00, 0xAA, 0xBB, 1, 2, 3, 4, 5, 6, 7, 8, 0x28, 0x97]), Frames).

	test(ccsds_tm_frames_parse_fecf_02, error(domain_error(ccsds_frame_byte_sequence, _))) :-
		ccsds_tm_frames(18, 2, true)::parse(bytes([0x02, 0xA6, 0x10, 0x20, 0x98, 0x00, 0xAA, 0xBB, 1, 2, 3, 4, 5, 6, 7, 8, 0x12, 0x34]), _).
	...
	test(ccsds_tc_frames_parse_2_01, true(Frames == [tc_transfer_frame(0, 1, 0, 42, 3, 7, none, [1,2,3], fecf([0x44,0x6D]))])) :-
		parse_tc_fecf(bytes([0x20, 0x2A, 0x0C, 0x09, 0x07, 1, 2, 3, 0x44, 0x6D]), Frames).

	test(ccsds_tc_frames_parse_2_03, error(domain_error(ccsds_frame_byte_sequence, _))) :-
		parse_tc_fecf(bytes([0x20, 0x2A, 0x0C, 0x09, 0x07, 1, 2, 3, 0x12, 0x34]), _).

	test(ccsds_tc_frames_parse_2_02, true(Frames == [tc_transfer_frame(0, 0, 1, 42, 3, 7, segment_header([0xAA,0xBB]), [1,2,3,4,5], none)])) :-
		parse_tc_segment(bytes([0x10, 0x2A, 0x0C, 0x0B, 0x07, 0xAA, 0xBB, 1, 2, 3, 4, 5]), Frames).

	test(ccsds_tc_frames_generate_2_01, true(Bytes == [0x20, 0x2A, 0x0C, 0x09, 0x07, 1, 2, 3, 0x44, 0x6D])) :-
		generate_tc_fecf(bytes(Bytes), [tc_transfer_frame(0, 1, 0, 42, 3, 7, none, [1,2,3], none)]).

	test(ccsds_tc_frames_generate_3_01, true(Bytes == [0x20, 0x2A, 0x0C, 0x09, 0x07, 1, 2, 3, 0x44, 0x6D, 0xFF])) :-
	...
	test(ccsds_tc_frames_bypass_flag_2_01, deterministic(Value == 1)) :-
		parse_tc_fecf(bytes([0x20, 0x2A, 0x0C, 0x09, 0x07, 1, 2, 3, 0x44, 0x6D]), [Frame]),
	...
	test(ccsds_tc_frames_segment_header_2_01, deterministic(Value == segment_header([0xAA,0xBB]))) :-
		parse_tc_segment(bytes([0x10, 0x2A, 0x0C, 0x0B, 0x07, 0xAA, 0xBB, 1, 2, 3, 4, 5]), [Frame]),
	...
	test(ccsds_aos_frames_parse_2_01, true(Frames == [aos_transfer_frame(1, 42, 3, 0x123456, signaling_field(1, 0, 0, 0), none, [1,2,3,4,5,6], none, none)])) :-
		parse_aos_basic(bytes([0x4A, 0x83, 0x12, 0x34, 0x56, 0x80, 1, 2, 3, 4, 5, 6]), Frames).
	...
	test(ccsds_aos_frames_parse_2_02, true(Frames == [aos_transfer_frame(1, 42, 3, 0x1123456, signaling_field(0, 1, 0, 1), insert_zone([0xAA,0xBB]), [1,2,3,4], ocf([0xDE,0xAD,0xBE,0xEF]), fecf([0xD1,0x7D]))])) :-
		parse_aos_mission(bytes([0x4A, 0x83, 0x12, 0x34, 0x56, 0x41, 0xAA, 0xBB, 1, 2, 3, 4, 0xDE, 0xAD, 0xBE, 0xEF, 0xD1, 0x7D]), Frames).
	...
	test(ccsds_aos_frames_parse_2_04, true(Frames == [aos_transfer_frame(1, 513, 3, 0x123456, signaling_field(0, 0, 2, 0), none, [1,2,3,4,5,6], none, none)])) :-
		parse_aos_basic(bytes([0x40, 0x43, 0x12, 0x34, 0x56, 0x20, 1, 2, 3, 4, 5, 6]), Frames).

	test(ccsds_aos_frames_generate_2_01, true(Bytes == [0x4A, 0x83, 0x12, 0x34, 0x56, 0x80, 1, 2, 3, 4, 5, 6])) :-
		generate_aos_basic(bytes(Bytes), [aos_transfer_frame(1, 42, 3, 0x123456, signaling_field(1, 0, 0, 0), none, [1,2,3,4,5,6], none, none)]).

	test(ccsds_aos_frames_generate_2_02, true(Bytes == [0x4A, 0x83, 0x12, 0x34, 0x56, 0x41, 0xAA, 0xBB, 1, 2, 3, 4, 0xDE, 0xAD, 0xBE, 0xEF, 0xD1, 0x7D])) :-
	...
	test(ccsds_aos_frames_generate_3_01, true(Bytes == [0x4A, 0x83, 0x12, 0x34, 0x56, 0x80, 1, 2, 3, 4, 5, 6, 0xFF])) :-
	...
	test(ccsds_aos_frames_version_2_01, deterministic(Value == 1)) :-
		parse_aos_basic(bytes([0x4A, 0x83, 0x12, 0x34, 0x56, 0x80, 1, 2, 3, 4, 5, 6]), [Frame]),
	...
	test(ccsds_aos_frames_spacecraft_id_2_02, deterministic(Value == 513)) :-
		parse_aos_basic(bytes([0x40, 0x43, 0x12, 0x34, 0x56, 0x20, 1, 2, 3, 4, 5, 6]), [Frame]),
		aos_spacecraft_id(Frame, Value).
	...
	test(ccsds_aos_frames_ocf_2_02, deterministic(Value == ocf([0xDE,0xAD,0xBE,0xEF]))) :-
		parse_aos_mission(bytes([0x4A, 0x83, 0x12, 0x34, 0x56, 0x41, 0xAA, 0xBB, 1, 2, 3, 4, 0xDE, 0xAD, 0xBE, 0xEF, 0xD1, 0x7D]), [Frame]),
		aos_ocf(Frame, Value).
	...
	test(ccsds_frames_types_check_07, deterministic) :-
		type::check(ccsds_aos_frame(18, 2, true, true), [0x4A, 0x83, 0x12, 0x34, 0x56, 0x41, 0xAA, 0xBB, 1, 2, 3, 4, 0xDE, 0xAD, 0xBE, 0xEF, 0xD1, 0x7D]).
