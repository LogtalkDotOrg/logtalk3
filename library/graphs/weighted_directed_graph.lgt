...
:- object(weighted_directed_graph(_Dictionary_),
	...
	add_edge(Graph, Vertex1, Vertex2, Weight, NewGraph) :-
		...
		dict_insert(Graph, Vertex1, NewWNs, G1),
	...
	add_edges(Graph, [(Vertex1-Vertex2)-W|Edges], NewGraph) :-
		add_edge(Graph, Vertex1, Vertex2, W, G1),
	...
	transpose(Graph, NewGraph) :-
		...
		add_transposed_wedges(Pairs, G1, NewGraph).
	...
	add_transposed_wedges([], Graph, Graph).
	add_transposed_wedges([V-WNs|Pairs], Graph, NewGraph) :-
		add_reversed_wneighbors(WNs, V, Graph, G1),
		add_transposed_wedges(Pairs, G1, NewGraph).

	add_reversed_wneighbors([], _, Graph, Graph).
	add_reversed_wneighbors([N-W|WNs], V, Graph, NewGraph) :-
		(	dict_lookup(N, NWNs, Graph) ->
			winsert_neighbor(NWNs, V, W, NewNWNs),
			dict_insert(Graph, N, NewNWNs, G1)
		;	dict_insert(Graph, N, [V-W], G1)
		),
		add_reversed_wneighbors(WNs, V, G1, NewGraph).

	% --- Warshall for weighted ---

	wwarshall([], Graph, Graph).
	wwarshall([V|Vs], Graph, Closure) :-
		dict_lookup(V, VWNeighbors, Graph),
		dict_as_list(Graph, Pairs),
		wwarshall_update_pairs(Pairs, V, VWNeighbors, NewPairs),
		dict_as_dictionary(NewPairs, NewGraph),
		wwarshall(Vs, NewGraph, Closure).

	wwarshall_update_pairs([], _, _, []).
	wwarshall_update_pairs([X-XWNs|Pairs], V, VWNs, [X-NewXWNs|NewPairs]) :-
		(	wfind_weight(XWNs, V, W) ->
			wadd_through(VWNs, W, XWNs, NewXWNs)
		;	NewXWNs = XWNs
		),
		wwarshall_update_pairs(Pairs, V, VWNs, NewPairs).

	wfind_weight([V0-W0|_], V, W) :-
		V0 == V,
		!,
		W = W0.
	wfind_weight([_|Rest], V, W) :-
		wfind_weight(Rest, V, W).
	...
	strip_wpairs([], []).
	strip_wpairs([V-WNs|WPairs], [V-Ns|Pairs]) :-
		strip_weights(WNs, Ns),
	...
	add_vertices_from_dist([], T, T).
	add_vertices_from_dist([V-_| DistPairs], T0, T) :-
		(	dict_lookup(V, _, T0) ->
			add_vertices_from_dist(DistPairs, T0, T)
		;	dict_insert(T0, V, [], T1),
			add_vertices_from_dist(DistPairs, T1, T)
		).
