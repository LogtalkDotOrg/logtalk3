...
:- object(tests,
	...
	:- info([
		...
		date is 2026-04-28,
		comment is 'Unit tests for the "nmf" library.'
	]).
	...
	test(nmf_learn_3_custom_options, deterministic) :-
		nmf::learn(parts_based_measurements, nmf_reducer(_Encoders, Components, Diagnostics), [n_components(1), center(false), feature_scaling(true), maximum_iterations(250), tolerance(1.0e-7)]),
		...
		assertion(memberchk(model(nmf), Diagnostics)).

	test(nmf_check_dimension_reducer_1, deterministic) :-
		nmf::learn(parts_based_measurements, DimensionReducer, [n_components(2)]),
		nmf::check_dimension_reducer(DimensionReducer).

	test(nmf_valid_dimension_reducer_1_invalid_negative_component, fail) :-
		nmf::learn(parts_based_measurements, nmf_reducer(Encoders, [_Component| Components], Diagnostics), [n_components(2)]),
		nmf::valid_dimension_reducer(nmf_reducer(Encoders, [[-1.0, 0.0, 0.0, 0.0]| Components], Diagnostics)).

	test(nmf_diagnostics_2, deterministic) :-
		nmf::learn(parts_based_measurements, DimensionReducer, [n_components(2)]),
		nmf::diagnostics(DimensionReducer, Diagnostics),
		assertion(memberchk(model(nmf), Diagnostics)),
	...
	test(nmf_diagnostics_2_maximum_iterations, deterministic) :-
		nmf::learn(parts_based_measurements, DimensionReducer, [n_components(2), maximum_iterations(1), tolerance(1.0e-12)]),
		nmf::diagnostics(DimensionReducer, Diagnostics),
	...
	test(nmf_diagnostics_2_tolerance_convergence, deterministic) :-
		nmf::learn(parts_based_measurements, DimensionReducer, [n_components(2), maximum_iterations(25), tolerance(10.0)]),
		nmf::diagnostics(DimensionReducer, Diagnostics),
	...
	test(nmf_diagnostics_2_respects_training_iterations, deterministic) :-
		nmf::learn(parts_based_measurements, DimensionReducer, [n_components(2), maximum_iterations(2), tolerance(1.0e-30)]),
		nmf::diagnostics(DimensionReducer, Diagnostics),
		assertion(memberchk(convergence(maximum_iterations_exhausted), Diagnostics)),
	...
	test(nmf_transform_3_distinguishes_parts, deterministic) :-
		nmf::learn(parts_based_measurements, DimensionReducer, [n_components(2)]),
		nmf::transform(DimensionReducer, [f1-3.0, f2-0.0, f3-1.5, f4-0.0], [component_1-FirstA, component_2-SecondA]),
		nmf::transform(DimensionReducer, [f1-0.0, f2-3.0, f3-0.0, f4-1.5], [component_1-FirstB, component_2-SecondB]),
		assertion(FirstA > SecondA),
		assertion(FirstB < SecondB).

	test(nmf_learn_2_singleton_dataset, deterministic) :-
		nmf::learn(singleton_measurement, nmf_reducer(_Encoders, _Components, Diagnostics), [n_components(1)]),
		assertion(memberchk(model(nmf), Diagnostics)),
	...
	test(nmf_export_to_clauses_4, deterministic(Clause == reduced(DimensionReducer))) :-
		nmf::learn(parts_based_measurements, DimensionReducer, [n_components(2)]),
		nmf::export_to_clauses(parts_based_measurements, DimensionReducer, reduced, [Clause]).

	test(nmf_export_to_file_4, deterministic) :-
		^^file_path('test_output.pl', File),
		nmf::learn(parts_based_measurements, DimensionReducer, [n_components(2)]),
		nmf::export_to_file(parts_based_measurements, DimensionReducer, reducer, File),
		logtalk_load(File),
		{reducer(Reducer)},
	...
	test(nmf_transform_3_feature_scaling_true_exported_functor, deterministic) :-
		^^file_path('test_output.pl', File),
		nmf::learn(parts_based_measurements, DimensionReducer, [n_components(2), feature_scaling(true)]),
		nmf::transform(DimensionReducer, [f1-3.0, f2-0.0, f3-1.5, f4-0.0], ReducedInstance),
		nmf::export_to_file(parts_based_measurements, DimensionReducer, reducer, File),
		logtalk_load(File),
		{reducer(Reducer)},
		nmf::transform(Reducer, [f1-3.0, f2-0.0, f3-1.5, f4-0.0], ExportedReducedInstance),
