...
:- object(tests_performance,
	...
	:- info([
		...
		date is 2026-04-17,
		comment is 'Performance and reference-fit benchmarks for the "logistic_regression" library.'
	]).
	...
	reference_fit(Dataset, MinimumAccuracy, MaximumLogLoss, TrainTime, Accuracy, LogLoss) :-
		benchmark(logistic_regression::learn(Dataset, _), TrainTime),
		logistic_regression::learn(Dataset, Classifier),
	...
	evaluate_examples([Class-AttributeValues| Examples], Classifier, Correct0, Correct, LogLoss0, LogLoss) :-
		logistic_regression::predict(Classifier, AttributeValues, Prediction),
		(   Prediction == Class ->
			Correct1 is Correct0 + 1
		;   Correct1 = Correct0
		),
		logistic_regression::predict_probabilities(Classifier, AttributeValues, Probabilities),
