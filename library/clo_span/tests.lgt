...
:- object(tests,
	...
	test(clo_span_mine_3_same_event_and_next_event_extensions, deterministic((memberchk(sequence_pattern([[a, b]], 3), Patterns), memberchk(sequence_pattern([[a], [b]], 3), Patterns)))) :-
		clo_span::mine(same_event_vs_next_event_sequences, clo_span_pattern_miner(_ItemDomain, Patterns, _Options), [minimum_support_count(2), maximum_pattern_length(2)]).

	test(clo_span_mine_3_minimum_pattern_length_filter, deterministic(Patterns == [sequence_pattern([[a], [b]], 4)])) :-
		clo_span::mine(prefix_ladder_sequences, clo_span_pattern_miner(_ItemDomain, Patterns, _Options), [minimum_support_count(4), maximum_pattern_length(2), minimum_pattern_length(2)]).
	...
	test(clo_span_matches_prefix_span_closed_subset, deterministic(ClosedPatterns == FilteredPatterns)) :-
		...
		clo_span::mine(closure_sequences, clo_span_pattern_miner(_ClosedDomain, ClosedPatterns, _ClosedOptions), Options),
		prefix_span::mine(closure_sequences, prefix_span_pattern_miner(_PrefixDomain, PrefixPatterns, _PrefixOptions), Options),
	...
	test(clo_span_matches_prefix_span_dense_overlap_closed_subset, deterministic(ClosedPatterns == FilteredPatterns)) :-
		...
		clo_span::mine(dense_overlap_sequences, clo_span_pattern_miner(_ClosedDomain, ClosedPatterns, _ClosedOptions), Options),
		prefix_span::mine(dense_overlap_sequences, prefix_span_pattern_miner(_PrefixDomain, PrefixPatterns, _PrefixOptions), Options),
		filter_closed_patterns(PrefixPatterns, FilteredPatterns).

	test(clo_span_diagnostics_2, deterministic((memberchk(model(clo_span), Diagnostics), memberchk(backward_pruning(projected_database_equivalence), Diagnostics), memberchk(closure_filter(projected_database_equivalence_and_same_support_frontier_pruning), Diagnostics), memberchk(support_layout(projected_database), Diagnostics)))) :-
		clo_span::mine(closure_sequences, PatternMiner, [minimum_support_count(2), maximum_pattern_length(3)]),
		clo_span::diagnostics(PatternMiner, Diagnostics).

	test(clo_span_valid_pattern_miner_1, deterministic) :-
		clo_span::mine(closure_sequences, PatternMiner, [minimum_support_count(2), maximum_pattern_length(3)]),
		clo_span::valid_pattern_miner(PatternMiner).
