...
:- object(tests,
	...
	test(lda_predict_scores_3, deterministic(memberchk(setosa-_, Scores))) :-
		lda_classifier::learn(iris_small, Classifier),
		lda_classifier::predict_scores(Classifier, [sepal_length-5.0, sepal_width-3.4, petal_length-1.4, petal_width-0.2], Scores).

	test(lda_learn_3_custom_options, deterministic((memberchk(feature_scaling(false), Options), memberchk(regularization(1.0e-4), Options)))) :-
	...
	test(lda_diagnostics_2, deterministic((memberchk(model(lda_classifier), Diagnostics), memberchk(options(_), Diagnostics)))) :-
		lda_classifier::learn(iris_small, Classifier),
