...
:- object(tests_performance,
	...
	:- info([
		...
		date is 2026-05-02,
		comment is 'Performance and reference-fit benchmarks for the "elastic_net_regression" library.'
	]).
	...
	test(sparse_signal_regularized_reference_fit, true, [note(metrics(train_seconds-TrainTime, rmse-RMSE, mae-MAE))]) :-
		reference_fit(sparse_signal, [feature_scaling(false), maximum_iterations(8000), tolerance(1.0e-10), regularization(0.01), l1_ratio(0.5)], 0.25, 0.25, TrainTime, RMSE, MAE).
	...
	test(mixed_signal_regularized_reference_fit, true, [note(metrics(train_seconds-TrainTime, rmse-RMSE, mae-MAE))]) :-
		reference_fit(mixed_signal, [maximum_iterations(8000), tolerance(1.0e-9), regularization(0.01), l1_ratio(0.5)], 1.0, 1.0, TrainTime, RMSE, MAE).
	...
	test(sparse_mixed_signal_regularized_reference_fit, true, [note(metrics(train_seconds-TrainTime, rmse-RMSE, mae-MAE))]) :-
		reference_fit(sparse_mixed_signal, [maximum_iterations(8000), tolerance(1.0e-9), regularization(0.01), l1_ratio(0.5)], 1.0, 1.0, TrainTime, RMSE, MAE).
	...
	test(grouped_categorical_signal_regularized_reference_fit, true, [note(metrics(train_seconds-TrainTime, rmse-RMSE, mae-MAE))]) :-
		reference_fit(grouped_categorical_signal, [feature_scaling(false), maximum_iterations(8000), tolerance(1.0e-10), regularization(0.01), l1_ratio(0.5)], 0.25, 0.25, TrainTime, RMSE, MAE).

	test(wide_mixed_signal_reference_fit, true, [note(metrics(train_seconds-TrainTime, rmse-RMSE, mae-MAE))]) :-
		reference_fit(wide_mixed_signal, [maximum_iterations(12000), tolerance(1.0e-8), regularization(0.01), l1_ratio(0.5)], 0.50, 0.50, TrainTime, RMSE, MAE).
	...
	reference_fit(Dataset, Options, MaximumRMSE, MaximumMAE, TrainTime, RMSE, MAE) :-
		benchmark(elastic_net_regression::learn(Dataset, _Regressor, Options), TrainTime),
		elastic_net_regression::learn(Dataset, Regressor, Options),
	...
	evaluate_examples([Target-AttributeValues| Examples], Regressor, SumSquaredError0, SumSquaredError, SumAbsoluteError0, SumAbsoluteError) :-
		elastic_net_regression::predict(Regressor, AttributeValues, Prediction),
