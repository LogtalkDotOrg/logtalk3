...
`http` core library. It does not replace the low-level multipart wire support
already implemented in `http`; instead it wraps the normalized multipart body
and part terms, adds form-data field and file helpers, and delegates raw
parsing and generation to the existing `http` predicates.
...
- `parse/4` and `generate/3` as multipart-specific wrappers around the
  existing `http::parse_body/4` and `http::generate_body/3` predicates.
...
- Multipart wire parsing and generation still require a boundary parameter in
  ...
  exactly as in the underlying `http` library.
