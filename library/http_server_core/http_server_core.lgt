...
:- object(http_server_core,
	...
	:- info(accept_websocket/3, [
		...
		exceptions is [
			'``Options`` contains an invalid WebSocket acceptance option' - domain_error(http_server_core_websocket_option, 'Option'),
			'``Options`` contains reserved WebSocket handshake headers' - domain_error(http_server_core_websocket_headers, 'Headers'),
			'``Options`` contains reserved WebSocket handshake properties' - domain_error(http_server_core_websocket_properties, 'Properties'),
			'``Request`` is not a valid normalized WebSocket opening-handshake request' - domain_error(http_server_core_websocket_request, 'Request'),
			'``Options`` selects a subprotocol not offered by ``Request``' - domain_error(http_server_core_websocket_protocol, 'Protocol'),
	...
	websocket_handler_error_response(error(domain_error(http_server_core_websocket_request, Request), _Context), _OriginalRequest, Response) :-
		unsupported_websocket_version(Request),
		!,
		upgrade_required_response(Response).
	websocket_handler_error_response(error(domain_error(http_server_core_websocket_request, _Request), _Context), _OriginalRequest, Response) :-
		bad_request_response(http_server_core_websocket_request, Response).
