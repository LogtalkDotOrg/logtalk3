...
- Geospatial data:
	`crs_projections`, `geojson`, `geohash`, `geospatial`, `nmea`, `tle_orbits`, and `wkt_wkb`
...
- Web and HTTP APIs:
	`html`, `mime_types`, `url`, `http_core`, `http_authenticate`, `http_client_core`, `http_client`,
	`http_cookies`, `http_cors`, `http_digest`, `http_directory_listing`,
	`http_htmx`, `http_multipart`, `http_parameters`, `http_router`,
	`http_server_core`, `http_server`, `http_session`, `http_socket_transport`, `http_process_transport`,
