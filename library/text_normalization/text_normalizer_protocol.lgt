...
:- protocol(text_normalizer_protocol).
	...
	:- info(normalize_unicode/3, [
		...
		exceptions is [
			...
			'The ``Profile`` parameter neither a variable nor an object identifier' - type_error(object_identifier, 'Profile'),
			'The ``Profile`` parameter is an object identifier but not an object conforming to  ``text_normalization_profile_protocol``' - domain_error(text_normalization_profile, 'Profile'),
