...
:- object(tests,
	...
	cover(cusum_anomaly_detector).

	cleanup :-
		^^clean_file('test_output.pl').

	learn_filtered_shift_sequences(Detector) :-
		learn(cusum_shift_sequences, Detector, [baseline_selection_policy(filter)]).

	learn_filtered_shift_sequences(ExtraOptions, Detector) :-
		learn(cusum_shift_sequences, Detector, [baseline_selection_policy(filter)| ExtraOptions]).

	test(cusum_learn_2_high_dimensional_sequences, true(ground(Detector))) :-
		learn(cusum_high_dimensional_sequences, Detector).

	test(cusum_learn_2_non_baseline_dataset_error, error(domain_error(baseline_only_training_data, cusum_shift_sequences))) :-
		learn(cusum_shift_sequences, _Detector).
	...
	test(cusum_learn_2_invalid_dataset, error(domain_error(continuous_attribute, student))) :-
		learn(mixed_anomalies, _Detector).

	test(cusum_learn_3_empty_dataset_error, error(domain_error(non_empty_dataset, cusum_empty_anomalies))) :-
		learn(cusum_empty_anomalies, _Detector, []).

	test(cusum_learn_2_featureless_dataset_error, error(domain_error(non_empty_features, cusum_featureless_anomalies))) :-
		learn(cusum_featureless_anomalies, _Detector).

	test(cusum_score_3_shift_anomaly_higher_than_normal, true(AnomalyScore > NormalScore)) :-
		learn_filtered_shift_sequences(Detector),
	...
	test(cusum_score_3_shift_score_range, true((Score >= 0.0, Score < 1.0))) :-
		learn_filtered_shift_sequences(Detector),
		score(Detector, [t1-1.25, t2-1.35, t3-1.45, t4-1.55, t5-1.65, t6-1.75], Score).

	test(cusum_score_3_missing_values, true((Score >= 0.0, Score < 1.0))) :-
		learn_filtered_shift_sequences(Detector),
		score(Detector, [t1-1.20, t2-_, t3-1.30, t4-_, t5-1.40, t6-_], Score).

	test(cusum_score_3_all_missing_error, error(domain_error(non_empty_known_values, [t1, t2, t3, t4, t5, t6]))) :-
		learn_filtered_shift_sequences(Detector),
		score(Detector, [t1-_, t2-_, t3-_, t4-_, t5-_, t6-_], _Score).

	test(cusum_score_3_singleton_dataset_matching_point, true(Score =:= 0.0)) :-
		learn(cusum_singleton_anomalies, Detector),
		score(Detector, [t1-1.00], Score).

	test(cusum_predict_3_shift_normal, true(Prediction == normal)) :-
		learn_filtered_shift_sequences(Detector),
		predict(Detector, [t1-0.08, t2-0.02, t3- -0.03, t4-0.05, t5-0.00, t6- -0.01], Prediction).

	test(cusum_predict_3_shift_anomaly, true(Prediction == anomaly)) :-
		learn_filtered_shift_sequences(Detector),
		predict(Detector, [t1-1.25, t2-1.35, t3-1.45, t4-1.55, t5-1.65, t6-1.75], Prediction).

	test(cusum_predict_4_threshold_override, true(Prediction == anomaly)) :-
		learn_filtered_shift_sequences([anomaly_threshold(0.99)], Detector),
		predict(Detector, [t1-1.25, t2-1.35, t3-1.45, t4-1.55, t5-1.65, t6-1.75], Prediction, [anomaly_threshold(0.8333333333333334)]).

	test(cusum_predict_4_allowance_override_ignored, true(Prediction == normal)) :-
		learn(cusum_high_dimensional_sequences, Detector),
		predict(Detector, [t1-5.0, t2-0.0, t3-0.0, t4-0.0, t5-0.0, t6-0.0, t7-0.0, t8-0.0, t9-0.0, t10-0.0], Prediction, [allowance(0.0)]).

	test(cusum_predict_3_singleton_far_point, true(Prediction == anomaly)) :-
		learn(cusum_singleton_anomalies, Detector),
		predict(Detector, [t1-7.00], Prediction).

	test(cusum_predict_3_high_dimensional_in_control_sequence, true(Prediction == normal)) :-
		learn(cusum_high_dimensional_sequences, Detector),
		predict(Detector, [t1-0.0, t2-0.0, t3-0.0, t4-0.0, t5-0.0, t6-0.0, t7-0.0, t8-0.0, t9-0.0, t10-0.0], Prediction).

	test(cusum_predict_3_high_dimensional_anomaly, true(Prediction == anomaly)) :-
		learn(cusum_high_dimensional_sequences, Detector),
		predict(Detector, [t1-4.0, t2-4.0, t3-4.0, t4-4.0, t5-4.0, t6-4.0, t7-4.0, t8-4.0, t9-4.0, t10-4.0], Prediction).

	test(cusum_predict_3_high_dimensional_sparse_shift, true(Prediction == normal)) :-
		learn(cusum_high_dimensional_sequences, Detector),
		predict(Detector, [t1-4.0, t2-0.0, t3-0.0, t4-0.0, t5-0.0, t6-0.0, t7-0.0, t8-0.0, t9-0.0, t10-0.0], Prediction).

	test(cusum_predict_4_high_dimensional_sparse_threshold_override, true(Prediction == anomaly)) :-
		learn(cusum_high_dimensional_sequences, Detector),
		predict(Detector, [t1-4.0, t2-0.0, t3-0.0, t4-0.0, t5-0.0, t6-0.0, t7-0.0, t8-0.0, t9-0.0, t10-0.0], Prediction, [anomaly_threshold(0.70)]).

	test(cusum_score_all_3_shift_count, true(length(Scores, 12))) :-
		learn_filtered_shift_sequences(Detector),
		score_all(cusum_shift_sequences, Detector, Scores),
		length(Scores, 12).

	test(cusum_score_all_3_shift_sorted, true(FirstScore >= SecondScore)) :-
		learn_filtered_shift_sequences(Detector),
		score_all(cusum_shift_sequences, Detector, [_-_-FirstScore, _-_-SecondScore| _]).

	test(cusum_score_all_3_shift_top_anomalies, true(AnomalyCount >= 4)) :-
		learn_filtered_shift_sequences(Detector),
		score_all(cusum_shift_sequences, Detector, Scores),
		take(4, Scores, TopScores),
		count_class(TopScores, anomaly, AnomalyCount).

	test(cusum_valid_anomaly_detector_1, deterministic(valid_anomaly_detector(Detector))) :-
		learn_filtered_shift_sequences(Detector).
	...
	test(cusum_export_to_clauses_4, true(ground(Clauses))) :-
		learn_filtered_shift_sequences(Detector),
		export_to_clauses(cusum_shift_sequences, Detector, detect, Clauses).

	test(cusum_export_to_file_4, deterministic(os::file_exists(File))) :-
		^^file_path('test_output.pl', File),
		learn_filtered_shift_sequences(Detector),
		export_to_file(cusum_shift_sequences, Detector, detect, File).

	test(cusum_export_to_file_4_loadable, deterministic(Prediction == anomaly)) :-
		^^file_path('test_output.pl', File),
		learn_filtered_shift_sequences(Detector),
		export_to_file(cusum_shift_sequences, Detector, detector, File),
		logtalk_load(File),
		{detector(LoadedDetector)},
		predict(LoadedDetector, [t1-1.25, t2-1.35, t3-1.45, t4-1.55, t5-1.65, t6-1.75], Prediction).

	test(cusum_print_anomaly_detector_1, deterministic) :-
