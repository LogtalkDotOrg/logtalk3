...
:- object(tests,
	...
	test(geospatial_valid_coordinate_1_01, deterministic) :-
		valid_coordinate(geographic(0.0, 0.0)).

	test(geospatial_valid_coordinate_1_02, fail) :-
		valid_coordinate(geographic(91.0, 0.0)).

	test(geospatial_valid_coordinate_1_03, fail) :-
		valid_coordinate(geographic(0.0, -181.0)).

	test(geospatial_normalize_coordinate_2_01, deterministic(Coordinate == geographic(10.0, -170.0))) :-
		normalize_coordinate(geographic(10.0, 190.0), Coordinate).

	test(geospatial_normalize_coordinate_2_02, deterministic(Coordinate == geographic(85.0, -170.0))) :-
		normalize_coordinate(geographic(95.0, 10.0), Coordinate).

	test(geospatial_equirectangular_projection_4_01, deterministic((X > 111.19, X < 111.20, abs(Y) < 1.0e-9))) :-
		equirectangular_projection(geographic(0.0, 1.0), 0.0, X, Y).

	test(geospatial_equirectangular_inverse_4_01, deterministic((abs(Latitude) < 1.0e-9, Longitude > 0.99, Longitude < 1.01))) :-
		equirectangular_inverse(111.195, 0.0, 0.0, geographic(Latitude, Longitude)).

	test(geospatial_haversine_distance_3_01, deterministic((Distance > 111.19, Distance < 111.20))) :-
		haversine_distance(geographic(0.0, 0.0), geographic(0.0, 1.0), Distance).

	test(geospatial_haversine_distance_3_02, fail) :-
		haversine_distance(geographic(0.0, 0.0), geographic(95.0, 1.0), _).

	test(geospatial_vincenty_distance_3_01, deterministic((Distance > 111.31, Distance < 111.33))) :-
		vincenty_distance(geographic(0.0, 0.0), geographic(0.0, 1.0), Distance).

	test(geospatial_vincenty_distance_3_02, deterministic((Distance > 272.0, Distance < 278.0))) :-
		vincenty_distance(geographic(38.7223, -9.1393), geographic(41.1579, -8.6291), Distance).

	test(geospatial_rhumb_distance_3_01, deterministic((Distance > 111.19, Distance < 111.20))) :-
		rhumb_distance(geographic(0.0, 0.0), geographic(0.0, 1.0), Distance).

	test(geospatial_rhumb_bearing_3_01, deterministic((Bearing > 44.99, Bearing < 45.01))) :-
		rhumb_bearing(geographic(0.0, 0.0), geographic(1.0, 1.0), Bearing).

	test(geospatial_rhumb_destination_point_4_01, deterministic((abs(Latitude) < 0.01, Longitude > 0.99, Longitude < 1.01))) :-
		rhumb_destination_point(geographic(0.0, 0.0), 90.0, 111.195, geographic(Latitude, Longitude)).

	test(geospatial_interpolate_rhumb_4_01, deterministic((abs(Latitude) < 0.01, Longitude > 0.99, Longitude < 1.01))) :-
		interpolate_rhumb(geographic(0.0, 0.0), geographic(0.0, 2.0), 0.5, geographic(Latitude, Longitude)).

	test(geospatial_interpolate_rhumb_4_02, fail) :-
		interpolate_rhumb(geographic(0.0, 0.0), geographic(0.0, 2.0), -0.1, _).

	test(geospatial_rhumb_midpoint_3_01, deterministic((abs(Latitude) < 0.01, Longitude > 0.99, Longitude < 1.01))) :-
		rhumb_midpoint(geographic(0.0, 0.0), geographic(0.0, 2.0), geographic(Latitude, Longitude)).

	test(geospatial_distance_4_01, deterministic((Distance > 111.19, Distance < 111.20))) :-
		distance(geographic(0.0, 0.0), geographic(0.0, 1.0), haversine, Distance).

	test(geospatial_distance_4_03, deterministic((Distance > 111.19, Distance < 111.20))) :-
		distance(geographic(0.0, 0.0), geographic(0.0, 1.0), rhumb, Distance).

	test(geospatial_distance_4_02, fail) :-
		distance(geographic(0.0, 0.0), geographic(0.0, 1.0), unsupported, _).

	test(geospatial_distance_5_01, deterministic((Distance > 111194.9, Distance < 111195.1))) :-
		distance(geographic(0.0, 0.0), geographic(0.0, 1.0), haversine, meters, Distance).

	test(geospatial_distance_5_02, deterministic((Distance > 69.09, Distance < 69.10))) :-
		distance(geographic(0.0, 0.0), geographic(0.0, 1.0), haversine, miles, Distance).

	test(geospatial_distance_5_04, fail) :-
		distance(geographic(0.0, 0.0), geographic(0.0, 1.0), haversine, mi, _).

	test(geospatial_distance_5_05, fail) :-
		distance(geographic(0.0, 0.0), geographic(0.0, 1.0), haversine, mile, _).

	test(geospatial_distance_5_03, fail) :-
		distance(geographic(0.0, 0.0), geographic(0.0, 1.0), haversine, unsupported_unit, _).

	test(geospatial_initial_bearing_3_01, deterministic((Bearing > 89.99, Bearing < 90.01))) :-
		initial_bearing(geographic(0.0, 0.0), geographic(0.0, 1.0), Bearing).

	test(geospatial_final_bearing_3_01, deterministic((Bearing > 89.99, Bearing < 90.01))) :-
		final_bearing(geographic(0.0, 0.0), geographic(0.0, 1.0), Bearing).

	test(geospatial_midpoint_3_01, deterministic((abs(Latitude) < 0.01, Longitude > 0.99, Longitude < 1.01))) :-
		midpoint(geographic(0.0, 0.0), geographic(0.0, 2.0), geographic(Latitude, Longitude)).

	test(geospatial_destination_point_4_01, deterministic((abs(Latitude) < 0.01, Longitude > 0.99, Longitude < 1.01))) :-
		destination_point(geographic(0.0, 0.0), 90.0, 111.195, geographic(Latitude, Longitude)).

	test(geospatial_interpolate_great_circle_4_01, deterministic((abs(Latitude) < 0.01, Longitude > 0.99, Longitude < 1.01))) :-
		interpolate_great_circle(geographic(0.0, 0.0), geographic(0.0, 2.0), 0.5, geographic(Latitude, Longitude)).

	test(geospatial_interpolate_great_circle_4_02, fail) :-
		interpolate_great_circle(geographic(0.0, 0.0), geographic(0.0, 2.0), 1.5, _).

	test(geospatial_cross_track_distance_4_01, deterministic((abs(Distance) > 111.1, abs(Distance) < 111.3))) :-
		cross_track_distance(geographic(1.0, 1.0), geographic(0.0, 0.0), geographic(0.0, 2.0), Distance).

	test(geospatial_along_track_distance_4_01, deterministic((Distance > 111.1, Distance < 111.3))) :-
		along_track_distance(geographic(1.0, 1.0), geographic(0.0, 0.0), geographic(0.0, 2.0), Distance).

	test(geospatial_within_distance_4_01, deterministic) :-
		within_distance(geographic(0.0, 0.0), geographic(0.0, 1.0), 112.0, haversine).

	test(geospatial_within_distance_4_02, fail) :-
		within_distance(geographic(0.0, 0.0), geographic(0.0, 1.0), 100.0, haversine).

	test(geospatial_nearest_coordinate_5_01, deterministic((Nearest == geographic(0.0, 1.0), Distance > 111.19, Distance < 111.20))) :-
		nearest_coordinate(
			geographic(0.0, 0.0),
			[geographic(0.0, 2.0), geographic(0.0, 1.0), geographic(0.0, 3.0)],
			haversine,
			Nearest,
			Distance
		).
	...
	test(geospatial_nearest_coordinate_5_02, fail) :-
		nearest_coordinate(geographic(0.0, 0.0), [], haversine, _, _).

	test(geospatial_mean_center_2_01, deterministic((Latitude =~= 2.0, Longitude =~= 2.0))) :-
		mean_center([geographic(0.0, 0.0), geographic(2.0, 2.0), geographic(4.0, 4.0)], geographic(Latitude, Longitude)).

	test(geospatial_mean_center_2_02, fail) :-
		mean_center([], _).

	test(geospatial_minimum_enclosing_circle_3_01, deterministic((Center = geographic(10.0, -20.0), Radius =~= 0.0))) :-
		minimum_enclosing_circle([geographic(10.0, -20.0)], Center, Radius).
	...
	test(geospatial_minimum_enclosing_circle_3_02, deterministic((abs(Latitude) < 0.01, Longitude > 0.99, Longitude < 1.01, Radius > 111.19, Radius < 111.20))) :-
		minimum_enclosing_circle([geographic(0.0, 0.0), geographic(0.0, 2.0)], geographic(Latitude, Longitude), Radius).
	...
	test(geospatial_minimum_enclosing_circle_3_03, deterministic((Latitude > 0.49, Latitude < 0.51, Longitude > 0.49, Longitude < 0.51, Radius > 78.62, Radius < 78.64))) :-
		minimum_enclosing_circle([geographic(0.0, 0.0), geographic(0.0, 1.0), geographic(1.0, 1.0), geographic(1.0, 0.0)], geographic(Latitude, Longitude), Radius).
	...
	test(geospatial_coordinates_bounding_box_2_01, deterministic((MinLatitude =~= -1.0, MinLongitude =~= -2.0, MaxLatitude =~= 1.0, MaxLongitude =~= 1.0))) :-
		coordinates_bounding_box([geographic(1.0, -2.0), geographic(0.0, 1.0), geographic(-1.0, -1.0)], bbox(geographic(MinLatitude, MinLongitude), geographic(MaxLatitude, MaxLongitude))).
	...
	test(geospatial_bbox_from_coordinates_2_01, deterministic((MinLatitude =~= -1.0, MinLongitude =~= -2.0, MaxLatitude =~= 1.0, MaxLongitude =~= 1.0))) :-
		bbox_from_coordinates([geographic(1.0, -2.0), geographic(0.0, 1.0), geographic(-1.0, -1.0)], bbox(geographic(MinLatitude, MinLongitude), geographic(MaxLatitude, MaxLongitude))).
	...
	test(geospatial_point_in_polygon_2_01, deterministic) :-
		point_in_polygon(
			geographic(0.5, 0.5),
			[geographic(0.0, 0.0), geographic(0.0, 1.0), geographic(1.0, 1.0), geographic(1.0, 0.0)]
		).
	...
	test(geospatial_point_in_polygon_2_03, deterministic) :-
		point_in_polygon(
			geographic(0.0, 0.5),
			[geographic(0.0, 0.0), geographic(0.0, 1.0), geographic(1.0, 1.0), geographic(1.0, 0.0), geographic(0.0, 0.0)]
		).
	...
	test(geospatial_polygon_area_2_02, fail) :-
		polygon_area([geographic(0.0, 0.0), geographic(0.0, 1.0)], _).

	test(geospatial_polygon_centroid_2_01, deterministic((Latitude > 0.49, Latitude < 0.51, Longitude > 0.49, Longitude < 0.51))) :-
		polygon_centroid([geographic(0.0, 0.0), geographic(0.0, 1.0), geographic(1.0, 1.0), geographic(1.0, 0.0)], geographic(Latitude, Longitude)).

	test(geospatial_polygon_centroid_2_02, fail) :-
		polygon_centroid([geographic(0.0, 0.0), geographic(0.0, 1.0)], _).
	...
	test(geospatial_polygon_bounding_box_2_01, deterministic((MinLatitude =~= -1.0, MinLongitude =~= -2.0, MaxLatitude =~= 1.0, MaxLongitude =~= 1.0))) :-
		polygon_bounding_box(
			[geographic(1.0, -2.0), geographic(0.0, 1.0), geographic(-1.0, -1.0)],
			bbox(geographic(MinLatitude, MinLongitude), geographic(MaxLatitude, MaxLongitude))
		).

	test(geospatial_close_polygon_2_01, deterministic(ClosedPolygon == [geographic(0.0, 0.0), geographic(0.0, 1.0), geographic(1.0, 0.0), geographic(0.0, 0.0)])) :-
		close_polygon([geographic(0.0, 0.0), geographic(0.0, 1.0), geographic(1.0, 0.0)], ClosedPolygon).

	test(geospatial_polygon_orientation_2_01, deterministic(Orientation == clockwise)) :-
		polygon_orientation([geographic(0.0, 0.0), geographic(1.0, 0.0), geographic(1.0, 1.0), geographic(0.0, 1.0)], Orientation).

	test(geospatial_polygon_orientation_2_02, deterministic(Orientation == counterclockwise)) :-
		polygon_orientation([geographic(0.0, 0.0), geographic(0.0, 1.0), geographic(1.0, 1.0), geographic(1.0, 0.0)], Orientation).

	test(geospatial_is_clockwise_polygon_1_01, deterministic) :-
		is_clockwise_polygon([geographic(0.0, 0.0), geographic(1.0, 0.0), geographic(1.0, 1.0), geographic(0.0, 1.0)]).

	test(geospatial_is_clockwise_polygon_1_02, fail) :-
		is_clockwise_polygon([geographic(0.0, 0.0), geographic(0.0, 1.0), geographic(1.0, 1.0), geographic(1.0, 0.0)]).

	test(geospatial_normalize_polygon_orientation_3_01, deterministic(is_clockwise_polygon(Oriented))) :-
		normalize_polygon_orientation([geographic(0.0, 0.0), geographic(0.0, 1.0), geographic(1.0, 1.0), geographic(1.0, 0.0)], clockwise, Oriented).

	test(geospatial_normalize_polygon_orientation_3_02, fail) :-
		normalize_polygon_orientation([geographic(0.0, 0.0), geographic(0.0, 1.0), geographic(1.0, 1.0), geographic(1.0, 0.0)], unsupported, _).

	test(geospatial_clockwise_polygon_2_01, deterministic(is_clockwise_polygon(Clockwise))) :-
		clockwise_polygon([geographic(0.0, 0.0), geographic(0.0, 1.0), geographic(1.0, 1.0), geographic(1.0, 0.0)], Clockwise).

	test(geospatial_counterclockwise_polygon_2_01, deterministic(Orientation == counterclockwise)) :-
		counterclockwise_polygon([geographic(0.0, 0.0), geographic(1.0, 0.0), geographic(1.0, 1.0), geographic(0.0, 1.0)], Counterclockwise),
		polygon_orientation(Counterclockwise, Orientation).

	test(geospatial_is_valid_polygon_1_01, deterministic) :-
		is_valid_polygon([geographic(0.0, 0.0), geographic(0.0, 1.0), geographic(1.0, 0.0)]).

	test(geospatial_is_valid_polygon_1_02, fail) :-
		is_valid_polygon([geographic(0.0, 0.0), geographic(0.0, 1.0)]).

	test(geospatial_bbox_contains_2_01, deterministic) :-
		bbox_contains(bbox(geographic(-1.0, -1.0), geographic(1.0, 1.0)), geographic(0.5, 0.5)).
	...
	test(geospatial_polyline_length_3_01, deterministic((Length > 111.31, Length < 111.33))) :-
		polyline_length([geographic(0.0, 0.0), geographic(0.0, 1.0)], vincenty, Length).

	test(geospatial_polyline_simplify_3_01, deterministic(Simplified == [geographic(0.0, 0.0), geographic(1.0, 0.0)])) :-
		polyline_simplify(
			[geographic(0.0, 0.0), geographic(0.5, 0.1), geographic(1.0, 0.0)],
			20.0,
			Simplified
		).

	test(geospatial_polyline_simplify_3_02, deterministic(Simplified == [geographic(0.0, 0.0), geographic(0.5, 0.1), geographic(1.0, 0.0)])) :-
		polyline_simplify(
			[geographic(0.0, 0.0), geographic(0.5, 0.1), geographic(1.0, 0.0)],
			5.0,
			Simplified
	...
	test(geospatial_polyline_simplify_3_03, fail) :-
		polyline_simplify([geographic(0.0, 0.0)], 5.0, _).

	test(geospatial_polyline_split_at_distance_4_01, deterministic((coordinates_close(Left, [geographic(0.0, 0.0), geographic(0.0, 1.0)]), coordinates_close(Right, [geographic(0.0, 1.0), geographic(0.0, 2.0)])))) :-
		haversine_distance(geographic(0.0, 0.0), geographic(0.0, 1.0), Step),
		polyline_split_at_distance([geographic(0.0, 0.0), geographic(0.0, 1.0), geographic(0.0, 2.0)], Step, Left, Right).

	test(geospatial_polyline_split_at_distance_4_02, deterministic((Left == [geographic(0.0, 0.0)], Right == [geographic(0.0, 0.0), geographic(0.0, 1.0)]))) :-
		polyline_split_at_distance([geographic(0.0, 0.0), geographic(0.0, 1.0)], 0.0, Left, Right).

	test(geospatial_polyline_split_at_distance_4_03, deterministic((Left == [geographic(0.0, 0.0), geographic(0.0, 1.0)], Right == [geographic(0.0, 1.0)]))) :-
		polyline_split_at_distance([geographic(0.0, 0.0), geographic(0.0, 1.0)], 300.0, Left, Right).

	test(geospatial_polyline_resample_3_01, deterministic(coordinates_close(Resampled, [geographic(0.0, 0.0), geographic(0.0, 1.0), geographic(0.0, 2.0)]))) :-
		haversine_distance(geographic(0.0, 0.0), geographic(0.0, 1.0), Step),
		polyline_resample([geographic(0.0, 0.0), geographic(0.0, 1.0), geographic(0.0, 2.0)], Step, Resampled).

	test(geospatial_polyline_resample_3_02, deterministic(coordinates_close(Resampled, [geographic(0.0, 0.0), geographic(0.0, 1.0)]))) :-
		polyline_resample([geographic(0.0, 0.0), geographic(0.0, 1.0)], 500.0, Resampled).

	test(geospatial_polygon_perimeter_2_01, deterministic((Perimeter > 444.75, Perimeter < 444.80))) :-
		polygon_perimeter([geographic(0.0, 0.0), geographic(0.0, 1.0), geographic(1.0, 1.0), geographic(1.0, 0.0)], Perimeter).

	test(geospatial_polygon_perimeter_3_01, deterministic((Perimeter > 443.7, Perimeter < 444.1))) :-
		polygon_perimeter([geographic(0.0, 0.0), geographic(0.0, 1.0), geographic(1.0, 1.0), geographic(1.0, 0.0)], vincenty, Perimeter).

	test(geospatial_polygons_intersect_2_01, deterministic) :-
		polygons_intersect(
			[geographic(0.0, 0.0), geographic(0.0, 2.0), geographic(2.0, 2.0), geographic(2.0, 0.0)],
			[geographic(1.0, 1.0), geographic(1.0, 3.0), geographic(3.0, 3.0), geographic(3.0, 1.0)]
		).

	test(geospatial_polygons_intersect_2_02, deterministic) :-
		polygons_intersect(
			[geographic(0.0, 0.0), geographic(0.0, 4.0), geographic(4.0, 4.0), geographic(4.0, 0.0)],
			[geographic(1.0, 1.0), geographic(1.0, 2.0), geographic(2.0, 2.0), geographic(2.0, 1.0)]
		).
	...
	test(geospatial_route_distance_3_01, deterministic((Distance > 111.31, Distance < 111.33))) :-
		route_distance([geographic(0.0, 0.0), geographic(0.0, 1.0)], vincenty, Distance).

	test(geospatial_route_distance_3_02, fail) :-
		route_distance([geographic(0.0, 0.0)], haversine, _).

	test(geospatial_route_distance_3_03, fail) :-
		route_distance([geographic(0.0, 0.0), geographic(100.0, 0.0)], haversine, _).

	test(geospatial_route_distance_4_01, deterministic((Distance > 60.03, Distance < 60.05))) :-
		route_distance([geographic(0.0, 0.0), geographic(0.0, 1.0)], haversine, nautical_miles, Distance).

	test(geospatial_route_distance_4_02, fail) :-
		route_distance([geographic(0.0, 0.0), geographic(0.0, 1.0)], haversine, unsupported_unit, _).

	test(geospatial_route_distance_4_03, fail) :-
		route_distance([geographic(0.0, 0.0), geographic(0.0, 1.0)], haversine, nm, _).

	% auxiliary predicates

	coordinate_close(geographic(Latitude1, Longitude1), geographic(Latitude2, Longitude2)) :-
		Latitude1 =~= Latitude2,
