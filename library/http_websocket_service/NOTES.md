...
`http_socket` WebSocket connections. It adds connection lifecycle ownership,
...
subset of the library is usable; see the "Backends without thread support"
...
- `run_session/3-4` for higher-level callback-driven loops over upgraded
  ...
  connection automatically, with optional auto-pong, keepalive, idle-timeout,
  and maximum payload length policies.
...
The `run_session/3-4` predicates accept an upgraded `http_socket` WebSocket
connection handle together with a handler object that implements the
