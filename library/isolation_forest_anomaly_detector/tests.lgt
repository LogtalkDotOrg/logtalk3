...
:- object(tests,
	...
	cover(isolation_forest_anomaly_detector).

	cleanup :-
		^^clean_file('test_output.pl').

	learn_filtered(Dataset, Model) :-
		isolation_forest_anomaly_detector::learn(Dataset, Model, [baseline_selection_policy(filter)]).

	learn_filtered(Dataset, ExtraOptions, Model) :-
		isolation_forest_anomaly_detector::learn(Dataset, Model, [baseline_selection_policy(filter)| ExtraOptions]).
	...
	test(isolation_forest_diagnostic_2, deterministic(Diagnostics == Enumerated)) :-
		learn_filtered(gaussian_anomalies, [number_of_trees(50)], Model),
		isolation_forest_anomaly_detector::diagnostics(Model, Diagnostics),
		findall(Diagnostic, isolation_forest_anomaly_detector::diagnostic(Model, Diagnostic), Enumerated).
	...
	test(isolation_forest_export_to_file_4_loadable, deterministic(Prediction == anomaly)) :-
		...
		learn_filtered(gaussian_anomalies, [number_of_trees(10)], Model),
		isolation_forest_anomaly_detector::export_to_file(gaussian_anomalies, Model, iforest, File),
		logtalk_load(File),
		{iforest(LoadedModel)},
	...
	test(isolation_forest_missing_split_branches_both_ways, true((MissingScore > NormalScore, MissingScore < AnomalyScore))) :-
		...
		isolation_forest_anomaly_detector::score(Model, [x-5.10], AnomalyScore).
