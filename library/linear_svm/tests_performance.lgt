...
:- object(tests_performance,
	...
	:- info([
		...
		date is 2026-04-17,
		comment is 'Performance and reference-fit benchmarks for the "linear_svm" library.'
	]).
	...
	reference_fit(Dataset, MinimumAccuracy, TrainTime, Accuracy) :-
		benchmark(linear_svm::learn(Dataset, _), TrainTime),
		linear_svm::learn(Dataset, Classifier),
	...
	evaluate_examples([Class-AttributeValues| Examples], Classifier, Correct0, Correct) :-
		linear_svm::predict(Classifier, AttributeValues, Prediction),
		(   Prediction == Class ->
