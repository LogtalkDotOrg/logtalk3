...
:- object(tests,
	...
	:- info([
		...
		date is 2026-05-01,
		comment is 'Unit tests for the "iqr" library.'
	]).
	...
	test(iqr_invalid_anomaly_detector_1, error(domain_error(anomaly_detector, iqr_detector(gaussian_anomalies, attribute_schema([x], [x-1]), [iqr(x, 0.0, 1.0, 1.0)], [model(iqr), training_dataset(gaussian_anomalies), attribute_names([x]), feature_count(1), example_count(0), options([anomaly_threshold(0.5), baseline_class_values([normal]), baseline_selection_policy(reject), fence_multiplier(1.5), score_mode(root_mean_square)])])))) :-
		check_anomaly_detector(iqr_detector(gaussian_anomalies, attribute_schema([x], [x-1]), [iqr(x, 0.0, 1.0, 1.0)], [model(iqr), training_dataset(gaussian_anomalies), attribute_names([x]), feature_count(1), example_count(0), options([anomaly_threshold(0.5), baseline_class_values([normal]), baseline_selection_policy(reject), fence_multiplier(1.5), score_mode(root_mean_square)])])).

	test(iqr_diagnostics_2, deterministic((memberchk(model(iqr), Diagnostics), memberchk(training_dataset(gaussian_anomalies), Diagnostics), memberchk(feature_count(2), Diagnostics), memberchk(example_count(40), Diagnostics)))) :-
		learn_filtered_gaussian_anomalies(Detector),
