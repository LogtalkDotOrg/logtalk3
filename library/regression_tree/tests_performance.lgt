...
:- object(tests_performance,
	...
	:- info([
		...
		date is 2026-04-21,
		comment is 'Performance and reference-fit benchmarks for the "regression_tree" library.'
	]).
	...
	test(step_signal_reference_fit, true, [note(metrics(train_seconds-TrainTime, rmse-RMSE, mae-MAE))]) :-
		reference_fit(step_signal, [], 0.01, 0.01, TrainTime, RMSE, MAE).

	test(mixed_signal_reference_fit, true, [note(metrics(train_seconds-TrainTime, rmse-RMSE, mae-MAE))]) :-
		reference_fit(mixed_signal, [feature_scaling(off)], 0.01, 0.01, TrainTime, RMSE, MAE).
	...
	reference_fit(Dataset, Options, MaximumRMSE, MaximumMAE, TrainTime, RMSE, MAE) :-
		benchmark(regression_tree::learn(Dataset, _Regressor, Options), TrainTime),
		regression_tree::learn(Dataset, Regressor, Options),
	...
	evaluate_examples([Target-AttributeValues| Examples], Regressor, SumSquaredError0, SumSquaredError, SumAbsoluteError0, SumAbsoluteError) :-
		regression_tree::predict(Regressor, AttributeValues, Prediction),
