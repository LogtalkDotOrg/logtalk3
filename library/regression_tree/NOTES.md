...
`regression_protocols` library and learns a binary regression tree using
recursive variance-reduction splits.
...
- **`feature_scaling/1`**: Controls z-score standardization of continuous attributes before tree induction. Accepted values are `on` and `off`. The default is `off`.
