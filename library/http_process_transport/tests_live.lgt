...
:- object(echo_http_process_transport_live_handler,
	...
	:- info([
		...
		date is 2026-07-08,
		comment is 'Echo handler used by the http_process_transport live smoke tests.'
	]).
...
:- object(websocket_http_process_transport_live_handler,
	implements(http_handler_protocol)).
	...
	:- info([
		...
		author is 'Paulo Moura',
		date is 2026-07-08,
		comment is 'WebSocket handshake handler used by the http_process_transport live smoke tests.'
	]).
...
:- object(tests_live,
	...
	:- info([
		...
		author is 'Paulo Moura',
		date is 2026-07-08,
		comment is 'Opt-in live smoke tests for the "http_process_transport" library covering external HTTPS client paths and local plain/TLS/WSS server paths.'
	]).
	...
	setup :-
		cleanup,
		http_process_transport::temporary_tls_credentials('logtalk_http_process_transport_live_', CertificateFile, KeyFile),
	...
	test(http_process_transport_exchange_4_live_01, deterministic) :-
		live_endpoint(Host, Port, Path),
		live_request(Host, Path, Request),
		http_process_transport::open_connection(Host, Port, Connection, [type(binary), connection_transport(tls)]),
		setup_call_cleanup(
			true,
			(	http_process_transport::exchange(Connection, Request, Response),
				valid_live_client_response(Response)
			),
			http_process_transport::close_connection(Connection)
		).

	test(http_process_transport_open_connection_4_live_01, deterministic) :-
		live_endpoint(Host, Port, Path),
		live_request(Host, Path, Request),
		http_process_transport::open_connection(Host, Port, Connection, [type(binary), connection_transport(tls)]),
		setup_call_cleanup(
	...
	test(http_process_transport_exchange_sequence_3_live_01, deterministic) :-
		live_endpoint(Host, Port, Path),
		live_request(Host, Path, Request),
		http_process_transport::open_connection(Host, Port, Connection, [type(binary), connection_transport(tls)]),
		setup_call_cleanup(
			true,
			(	http_process_transport::exchange_sequence(Connection, [Request, Request], [Response1, Response2]),
	...
	test(http_process_transport_open_connection_pool_4_live_01, deterministic) :-
		live_endpoint(Host, Port, Path),
		live_request(Host, Path, Request),
		http_process_transport::open_connection_pool(Host, Port, Pool, [min_size(1), max_size(1), connection_options([type(binary), connection_transport(tls)])]),
		setup_call_cleanup(
			true,
			(	http_process_transport::connection_pool_stats(Pool, stats(1, 0, 1, 1, 1)),
				http_process_transport::exchange(Pool, Request, Response1),
				valid_live_client_response(Response1),
				http_process_transport::connection_pool_stats(Pool, stats(1, 0, 1, 1, 1)),
				http_process_transport::exchange(Pool, Request, Response2),
				valid_live_client_response(Response2),
				http_process_transport::connection_pool_stats(Pool, stats(1, 0, 1, 1, 1))
			),
	...
	:- if(current_logtalk_flag(threads, supported)).
		...
		test(http_process_transport_serve_once_3_live_01, deterministic, [condition(executable_available(ncat))]) :-
			Host = '127.0.0.1',
			...
			http_process_transport::serve_once(Listener, echo_http_process_transport_live_handler, ClientInfo),
			threaded_exit(local_plain_exchange(Host, Port, Request, Response), Tag),
			http_process_transport::close_listener(Listener),
			compound(ClientInfo),
			status(Response, status(200, 'OK')).

		test(http_process_transport_serve_once_3_live_02, deterministic, [condition(executable_available(ncat))]) :-
			with_temporary_tls_credentials(run_tls_serve_once_smoke_test).

		test(http_process_transport_serve_websocket_once_5_live_01, deterministic, [condition(executable_available(ncat))]) :-
		...
		run_tls_serve_once_smoke_test(CertificateFile, KeyFile) :-
			Host = '127.0.0.1',
			Request = request(get, origin('/ping'), http(1, 1), [host-host('example.com')], empty, [connection([close])]),
			http_process_transport::open_listener(Host, Port, Listener, [listener_transport(tls), tls_certificate_file(CertificateFile), tls_key_file(KeyFile)]),
		...
		run_wss_serve_websocket_smoke_test(CertificateFile, KeyFile) :-
			...
			http_process_transport::serve_websocket_once(Listener, websocket_http_process_transport_live_handler, ServerConnection, ServerResponse, ClientInfo),
			...
			http_process_transport::connection_streams(ClientConnection, ClientInput, ClientOutput),
			...
			http_process_transport::close_connection(ClientConnection),
			http_process_transport::close_listener(Listener),
			compound(ClientInfo),
		...
		local_tls_exchange(Host, Port, CertificateFile, Request, Response) :-
			setup_call_cleanup(
				http_process_transport::open_connection(Host, Port, Connection, [type(binary), connection_transport(tls), server_name(none), openssl_arguments(['-CAfile', CertificateFile, '-verify_return_error'])]),
				http_process_transport::exchange(Connection, Request, Response),
				http_process_transport::close_connection(Connection)
			).
		...
		websocket_tls_client_exchange(Host, Port, Path, Protocols, Key, CertificateFile, Connection, Response) :-
			...
			http_process_transport::open_connection(Host, Port, Connection, [type(binary), connection_transport(tls), server_name(none), openssl_arguments(['-CAfile', CertificateFile, '-verify_return_error'])]),
				...
				http_process_transport::exchange(Connection, Request, Response),
				Error,
				(	catch(http_process_transport::close_connection(Connection), _, true),
					throw(Error)
				)
			).
