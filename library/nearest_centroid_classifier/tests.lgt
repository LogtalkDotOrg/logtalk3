...
:- object(tests,
	...
	:- info([
		...
		date is 2026-05-06,
		comment is 'Unit tests for the "nearest_centroid_classifier" library.'
	]).
	...
	cover(nearest_centroid_classifier).
	...
	test(nearest_centroid_valid_classifier_1, deterministic(nearest_centroid_classifier::valid_classifier(Classifier))) :-
		nearest_centroid_classifier::learn(iris_small, Classifier).

	test(nearest_centroid_invalid_classifier_1, fail) :-
		nearest_centroid_classifier::valid_classifier(nc_classifier([sepal_length], [numeric], [setosa-[wide]])).

	% Test predict/3 with iris dataset (continuous features)
	test(nearest_centroid_predict_3_iris_setosa, true(Prediction == setosa)) :-
		nearest_centroid_classifier::learn(iris_small, Classifier),
		nearest_centroid_classifier::predict(Classifier, [sepal_length-5.0, sepal_width-3.3, petal_length-1.4, petal_width-0.2], Prediction).

	test(nearest_centroid_predict_3_iris_versicolor, true(Prediction == versicolor)) :-
		nearest_centroid_classifier::learn(iris_small, Classifier),
		nearest_centroid_classifier::predict(Classifier, [sepal_length-6.5, sepal_width-3.0, petal_length-4.6, petal_width-1.4], Prediction).

	test(nearest_centroid_predict_3_iris_virginica, true(Prediction == virginica)) :-
		nearest_centroid_classifier::learn(iris_small, Classifier),
		nearest_centroid_classifier::predict(Classifier, [sepal_length-6.7, sepal_width-3.1, petal_length-5.6, petal_width-2.4], Prediction).

	% Test predict/4 with different options
	test(nearest_centroid_predict_4_different_k, true(ground(Prediction))) :-
		nearest_centroid_classifier::learn(iris_small, Classifier),
		nearest_centroid_classifier::predict(Classifier, [sepal_length-5.0, sepal_width-3.3, petal_length-1.4, petal_width-0.2], Prediction, [distance_metric(cosine)]).

	test(nearest_centroid_predict_4_different_metric, true(ground(Prediction))) :-
		nearest_centroid_classifier::learn(iris_small, Classifier),
		nearest_centroid_classifier::predict(Classifier, [sepal_length-5.0, sepal_width-3.3, petal_length-1.4, petal_width-0.2], Prediction, [distance_metric(manhattan)]).
	...
	test(nearest_centroid_learn_2_weather, true(ground(Classifier))) :-
		nearest_centroid_classifier::learn(weather, Classifier).

	% Test predict/3 with weather dataset (categorical features)
	test(nearest_centroid_predict_3_weather_01, true(Prediction == yes)) :-
		nearest_centroid_classifier::learn(weather, Classifier),
		nearest_centroid_classifier::predict(Classifier, [outlook-overcast, temperature-hot, humidity-normal, wind-weak], Prediction).

	% Test learn/2 with mixed dataset (categorical + continuous)
	test(nearest_centroid_learn_2_mixed, true(ground(Classifier))) :-
		nearest_centroid_classifier::learn(mixed, Classifier).

	% Test predict/3 with mixed dataset
	test(nearest_centroid_predict_3_mixed_01, true(ground(Prediction))) :-
		nearest_centroid_classifier::learn(mixed, Classifier),
		nearest_centroid_classifier::predict(Classifier, [age-26, income-36000, student-yes, credit_rating-fair], Prediction).
	...
	test(nearest_centroid_export_to_clauses_4, true(ground(Prediction))) :-
		nearest_centroid_classifier::learn(iris_small, Classifier),
		nearest_centroid_classifier::export_to_clauses(_Dataset, Classifier, classifier, [classifier(ExportedClassifier)]),
		nearest_centroid_classifier::predict(ExportedClassifier, [sepal_length-5.0, sepal_width-3.3, petal_length-1.4, petal_width-0.2], Prediction).
	...
	test(nearest_centroid_export_to_file_4_written, deterministic(os::file_exists(File))) :-
		...
		nearest_centroid_classifier::learn(iris_small, Classifier),
		nearest_centroid_classifier::export_to_file(iris_small, Classifier, classify, File).
	...
	test(nearest_centroid_export_to_file_4_loaded, true(ground(Prediction))) :-
		^^file_path('test_output.pl', File),
		nearest_centroid_classifier::learn(iris_small, Classifier),
		nearest_centroid_classifier::export_to_file(iris_small, Classifier, classifier, File),
		logtalk_load(File),
	...
	test(nearest_centroid_print_classifier_1, true) :-
		...
		nearest_centroid_classifier::learn(iris_small, Classifier),
		nearest_centroid_classifier::print_classifier(Classifier).

	% Test different distance metrics
	test(nearest_centroid_distance_metric_euclidean, true(ground(Prediction))) :-
		nearest_centroid_classifier::learn(iris_small, Classifier),
		nearest_centroid_classifier::predict(Classifier, [sepal_length-5.0, sepal_width-3.3, petal_length-1.4, petal_width-0.2], Prediction, [distance_metric(euclidean)]).
