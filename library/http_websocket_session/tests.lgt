...
:- object(tests,
	...
	test(http_websocket_session_read_message_4_03, deterministic) :-
		...
		http_websocket_client_session::read_message(Input, State1, State2, Message2),
		close(Input),
		State1 == session_state(fragment(binary, [[1, 2]]), close_received(status(1000))),
		State2 == session_state(idle, close_received(status(1000))),
		Message1 == message(close, status(1000)),
		Message2 == message(binary, [1, 2, 3, 4]).
