...
Open the [../../apis/library_index.html#plackett_luce_last](../../apis/library_index.html#plackett_luce_last)
...
- **Grouped Last-Choice Learning**: Learns positive item strengths from
  grouped rankings by processing each group from lowest relevance to highest
  relevance.
...
order to admit a finite Plackett-Luce-last maximum-likelihood estimate, the
...
- `model(plackett_luce_last)` identifies the learning algorithm that produced
  the ranker.
...
- `Strengths`: List of normalized `Item-Strength` pairs.
