...
:- object(tests,
	...
	test(plackett_luce_last_singleton_missing_relevance_error, error(existence_error(relevance, ballot_one-alpha))) :-
		plackett_luce_last::learn(singleton_grouped_missing_relevance, _Ranker, [missing_relevance(error)]).

	test(plackett_luce_last_learn_3_custom_options, deterministic((memberchk(options(Options), Diagnostics), memberchk(maximum_iterations(50), Options), memberchk(tolerance(1.0e-7), Options), memberchk(missing_relevance(error), Options)))) :-
		plackett_luce_last::learn(ranked_ballots, plackett_luce_last_ranker(_Items, _Strengths, Diagnostics), [maximum_iterations(50), tolerance(1.0e-7), missing_relevance(error)]).

	test(plackett_luce_last_rank_3, deterministic(true)) :-
		plackett_luce_last::learn(ranked_ballots, Ranker),
		plackett_luce_last::rank(Ranker, [gamma, alpha, beta], [beta, alpha, gamma]).
	...
	test(plackett_luce_last_diagnostic_2_enumerates, deterministic(Enumerated == Diagnostics)) :-
		plackett_luce_last::learn(ranked_ballots, Ranker),
		plackett_luce_last::diagnostics(Ranker, Diagnostics),
		findall(Diagnostic, plackett_luce_last::diagnostic(Ranker, Diagnostic), Enumerated).

	test(plackett_luce_last_ranker_options_2, deterministic((memberchk(maximum_iterations(5000), Options), memberchk(tolerance(1.0e-6), Options), memberchk(missing_relevance(zero), Options)))) :-
		plackett_luce_last::learn(ranked_ballots, Ranker),
		plackett_luce_last::ranker_options(Ranker, Options).

	test(plackett_luce_last_maximum_iterations_exhausted_diagnostics_2, deterministic((memberchk(convergence(maximum_iterations_exhausted), Diagnostics), memberchk(iterations(1), Diagnostics), memberchk(final_delta(FinalDelta), Diagnostics), FinalDelta > 0.0))) :-
		plackett_luce_last::learn(ranked_ballots, Ranker, [maximum_iterations(1), tolerance(1.0e-12)]),
		plackett_luce_last::diagnostics(Ranker, Diagnostics).

	test(plackett_luce_last_export_to_clauses_4, deterministic(ground(Clause))) :-
		plackett_luce_last::learn(ranked_ballots, Ranker),
		plackett_luce_last::export_to_clauses(ranked_ballots, Ranker, ranker, [Clause]).

	test(plackett_luce_last_export_to_file_4_loaded, deterministic(Ranking == [beta, alpha, gamma])) :-
		^^file_path('test_output.pl', File),
		plackett_luce_last::learn(ranked_ballots, Ranker),
		plackett_luce_last::export_to_file(ranked_ballots, Ranker, ranker, File),
		logtalk_load(File),
		{ranker(LoadedRanker)},
		plackett_luce_last::rank(LoadedRanker, [alpha, beta, gamma], Ranking).

	test(plackett_luce_last_tied_grouped_non_regular_error, error(domain_error(plackett_luce_last_regular_dataset, _))) :-
		plackett_luce_last::learn(tied_grouped, _Ranker).

	test(plackett_luce_last_sparse_grouped_missing_relevance_error, error(existence_error(relevance, ballot_one-gamma))) :-
		plackett_luce_last::learn(sparse_grouped_relevance, _Ranker, [missing_relevance(error)]).

	test(plackett_luce_last_search_results_non_regular_error, error(domain_error(plackett_luce_last_regular_dataset, _))) :-
		plackett_luce_last::learn(search_results, _Ranker).

	test(plackett_luce_last_malformed_grouped_error, error(domain_error(non_negative_integer, high))) :-
		plackett_luce_last::learn(malformed_grouped, _Ranker).

	test(plackett_luce_last_empty_group_error, error(domain_error(non_empty_group, ballot_one))) :-
		plackett_luce_last::learn(plackett_luce_last_empty_grouped, _Ranker).

	test(plackett_luce_last_rank_variable_candidate_error, error(instantiation_error)) :-
		plackett_luce_last::learn(ranked_ballots, Ranker),
		plackett_luce_last::rank(Ranker, [_Candidate, beta], _Ranking).

	test(plackett_luce_last_rank_unknown_candidate_error, error(existence_error(item, phantom))) :-
		plackett_luce_last::learn(ranked_ballots, Ranker),
		plackett_luce_last::rank(Ranker, [alpha, phantom], _Ranking).

	test(plackett_luce_last_rank_non_list_candidates_error, error(type_error(list, alpha))) :-
		plackett_luce_last::learn(ranked_ballots, Ranker),
		plackett_luce_last::rank(Ranker, alpha, _Ranking).

	test(plackett_luce_last_rank_duplicate_candidates_error, error(domain_error(unique_candidates, [alpha, alpha, beta]))) :-
		plackett_luce_last::learn(ranked_ballots, Ranker),
		plackett_luce_last::rank(Ranker, [alpha, alpha, beta], _Ranking).

	test(plackett_luce_last_valid_ranker_1, deterministic) :-
		plackett_luce_last::learn(ranked_ballots, Ranker),
		plackett_luce_last::valid_ranker(Ranker).

	test(plackett_luce_last_invalid_valid_ranker_1, fail) :-
		plackett_luce_last::valid_ranker(fake_ranker([alpha], [alpha-1.0], [model(fake)])).
	...
	test(plackett_luce_last_rank_invalid_ranker_error, error(domain_error(plackett_luce_last_ranker, fake_ranker([alpha], [alpha-1.0], [model(fake)])))) :-
		plackett_luce_last::rank(fake_ranker([alpha], [alpha-1.0], [model(fake)]), [alpha], _Ranking).

	test(plackett_luce_last_print_ranker_1, deterministic) :-
		^^suppress_text_output,
		plackett_luce_last::learn(ranked_ballots, Ranker),
		plackett_luce_last::print_ranker(Ranker).
