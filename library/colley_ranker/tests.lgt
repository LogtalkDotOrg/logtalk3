...
:- object(tests,
	...
	test(colley_learn_3_non_empty_options_error, error(domain_error(option, prior(flat)))) :-
		colley_ranker::learn(regular_head_to_head, _Ranker, [prior(flat)]).
	...
	test(colley_diagnostic_2, true) :-
		colley_ranker::learn(regular_head_to_head, Ranker),
		colley_ranker::diagnostic(Ranker, model(colley_ranker)).

	test(colley_ranker_options_2, deterministic(Options == [])) :-
		colley_ranker::learn(regular_head_to_head, Ranker),
		colley_ranker::ranker_options(Ranker, Options).

	test(colley_export_to_clauses_4, deterministic(ground(Clause))) :-
		colley_ranker::learn(regular_head_to_head, Ranker),
		colley_ranker::export_to_clauses(regular_head_to_head, Ranker, ranker, [Clause]).

	test(colley_export_to_file_4_loaded, deterministic(Ranking == [alpha, beta, gamma, delta])) :-
		^^file_path('test_output.pl', File),
		colley_ranker::learn(regular_head_to_head, Ranker),
		colley_ranker::export_to_file(regular_head_to_head, Ranker, ranker, File),
		logtalk_load(File),
		{ranker(LoadedRanker)},
		colley_ranker::rank(LoadedRanker, [alpha, beta, gamma, delta], Ranking).

	test(colley_malformed_pairwise_error, error(existence_error(item, phantom))) :-
		colley_ranker::learn(malformed_pairwise, _Ranker).

	test(colley_duplicate_items_error, error(domain_error(unique_items, [alpha, alpha, beta]))) :-
		colley_ranker::learn(malformed_duplicate_items, _Ranker).

	test(colley_self_preference_error, error(domain_error(distinct_items, alpha-alpha))) :-
		colley_ranker::learn(malformed_self_preference, _Ranker).

	test(colley_non_positive_weight_error, error(domain_error(positive_number, 0))) :-
		colley_ranker::learn(malformed_non_positive_weight, _Ranker).

	test(colley_disconnected_pairwise_error, error(domain_error(connected_pairwise_dataset, [[alpha, beta], [gamma, delta]]))) :-
		colley_ranker::learn(disconnected_pairwise, _Ranker).

	test(colley_rank_variable_candidate_error, error(instantiation_error)) :-
		colley_ranker::learn(regular_head_to_head, Ranker),
		colley_ranker::rank(Ranker, [_Candidate, beta], _Ranking).

	test(colley_rank_unknown_candidate_error, error(existence_error(item, phantom))) :-
		colley_ranker::learn(regular_head_to_head, Ranker),
		colley_ranker::rank(Ranker, [alpha, phantom], _Ranking).

	test(colley_rank_non_list_candidates_error, error(type_error(list, alpha))) :-
		colley_ranker::learn(regular_head_to_head, Ranker),
		colley_ranker::rank(Ranker, alpha, _Ranking).

	test(colley_rank_duplicate_candidates_error, error(domain_error(unique_candidates, [alpha, alpha, beta]))) :-
		colley_ranker::learn(regular_head_to_head, Ranker),
		colley_ranker::rank(Ranker, [alpha, alpha, beta], _Ranking).

	test(colley_valid_ranker_1, deterministic) :-
		colley_ranker::learn(regular_head_to_head, Ranker),
		colley_ranker::valid_ranker(Ranker).

	test(colley_invalid_valid_ranker_1, fail) :-
		colley_ranker::valid_ranker(fake_ranker([alpha], [alpha-0.5], [model(fake)])).

	test(colley_rank_invalid_ranker_error, error(domain_error(colley_ranker, fake_ranker([alpha], [alpha-0.5], [model(fake)])))) :-
		colley_ranker::rank(fake_ranker([alpha], [alpha-0.5], [model(fake)]), [alpha], _Ranking).

	test(colley_scores_invalid_ranker_error, error(domain_error(colley_ranker, colley_ranker([alpha, beta], [alpha-1.1, beta- -0.1], [model(colley_ranker), options([]), dataset_summary([items(2), preferences(1), connected_components(1), isolated_items([])])])))) :-
		colley_ranker::scores(colley_ranker([alpha, beta], [alpha-1.1, beta- -0.1], [model(colley_ranker), options([]), dataset_summary([items(2), preferences(1), connected_components(1), isolated_items([])])]), _Scores).

	test(colley_print_ranker_1, deterministic) :-
		^^suppress_text_output,
		colley_ranker::learn(regular_head_to_head, Ranker),
		colley_ranker::print_ranker(Ranker).
