...
:- object(tests,
	...
	test(knn_distance_anomaly_detector_to_clauses_4, true(ground(Clauses))) :-
		learn(gaussian_anomalies, Detector, [k(5)]),
		anomaly_detector_to_clauses(gaussian_anomalies, Detector, detect, Clauses).

	test(knn_distance_anomaly_detector_to_file_4, deterministic(os::file_exists(File))) :-
		^^file_path('test_output.pl', File),
		learn(gaussian_anomalies, Detector, [k(5)]),
		anomaly_detector_to_file(gaussian_anomalies, Detector, detect, File).

	test(knn_distance_anomaly_detector_to_file_4_loadable, deterministic(Prediction == anomaly)) :-
		^^file_path('test_output.pl', File),
		learn(gaussian_anomalies, Detector, [k(5)]),
