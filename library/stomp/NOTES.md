...
SICStus Prolog, SWI-Prolog, and Trealla Prolog.
...
Open the [../../apis/library_index.html#stomp](../../apis/library_index.html#stomp)
...
Note: Integration tests require a running STOMP server (e.g., RabbitMQ with
the STOMP plugin, ActiveMQ, or Apache Apollo). They are usually run using
RabbitMQ with the STOMP plugin. As RabbitMQ uses `/` as its default virtual
host, the tests assume this and use the `host('/')` option when calling the
`connect/4` predicate (otherwise the connection would fail with a
...
Most predicates throw `error(stomp_error(Reason), Context)` on failure.
