	...
	:- if((
		...
		:- if(current_logtalk_flag(threads, supported)).
			...
			test(http_client_digest_session_07, deterministic) :-
				...
				threaded_once(http_socket::serve_listener(Listener, http_server_digest_handler(http_digest_test_verifier, http_digest_request_echo_handler, DigestOptions, []), 10, _ClientInfos, [shutdown(close)]), Tag),
				...
				http_client_digest_session::close(Session),
				once(threaded_exit(http_socket::serve_listener(Listener, http_server_digest_handler(http_digest_test_verifier, http_digest_request_echo_handler, DigestOptions, []), 10, _ClientInfos, [shutdown(close)]), Tag)),
			...
			test(http_client_digest_session_08, deterministic(Cookies == [session-'jar'])) :-
				protect_options(sha256, 1700000000, DigestOptions),
				http_socket::open_listener('127.0.0.1', Port, Listener, []),
				threaded_once(http_socket::serve_listener(Listener, http_server_digest_handler(http_digest_test_verifier, http_digest_request_echo_handler, DigestOptions, []), 2, _ClientInfos, [shutdown(close)]), Tag),
				...
				http_client_digest_session::close(Session),
				once(threaded_exit(http_socket::serve_listener(Listener, http_server_digest_handler(http_digest_test_verifier, http_digest_request_echo_handler, DigestOptions, []), 2, _ClientInfos, [shutdown(close)]), Tag)),
				catch(http_socket::close_listener(Listener), _, true),
