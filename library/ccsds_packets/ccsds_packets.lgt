...
:- object(ccsds_packets(_SecondaryHeaderLength_)).
	...
	:- public(secondary_header_time/3).
	:- mode(secondary_header_time(+compound, ?compound, -compound), zero_or_one).
	:- info(secondary_header_time/3, [
		comment is 'Parses time from a packet secondary header. When ``Descriptor`` is a variable, the secondary header is parsed as a self-describing CCSDS time field. When ``Descriptor`` is instantiated, it is used to parse the raw T-field bytes.',
		argnames is ['Packet', 'Descriptor', 'TimeCode']
	]).
...
:- object(ccsds_packets,
	extends(ccsds_packets(0))).

	:- info([
		version is 1:0:0,
		author is 'Paulo Moura',
		date is 2025-12-05,
		comment is 'CCSDS Space Packet parser with no secondary header parsing. For secondary header support, use ccsds_packets(Length) where Length is the secondary header size in bytes.'
	]).
