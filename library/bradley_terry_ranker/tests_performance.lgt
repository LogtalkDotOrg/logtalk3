...
:- object(tests_performance,
	...
	:- info([
		...
		date is 2026-05-06,
		comment is 'Performance and sparse-graph regression benchmarks for the "bradley_terry_ranker" library.'
	]).
	...
	test(sparse_cycle_256_reference_fit, true, [note(metrics(train_seconds-TrainTime, items-Count, iterations-Iterations, strength_sum-TotalStrength))]) :-
		benchmark(bradley_terry_ranker::learn(sparse_cycle_256, _), TrainTime),
		bradley_terry_ranker::learn(sparse_cycle_256, Ranker),
		bradley_terry_ranker::scores(Ranker, Strengths),
