...
- **`feature_scaling/1`**: Controls z-score standardization of continuous attributes inside each regression-tree base learner. Accepted values are `on` and `off`. The default is `off`.
