...
:- protocol(supervised_dimension_reduction_dataset_protocol,
	...
	:- info([
		...
		date is 2026-04-18,
		comment is 'Protocol for labeled datasets used with supervised dimension reduction algorithms such as LDA.'
	]).
	...
	:- info(example/3, [
		comment is 'Enumerates by backtracking the labeled examples in the dataset. Each example has an ``Id``, a ``Class`` value, and a list of ``Attribute-Value`` pairs.',
