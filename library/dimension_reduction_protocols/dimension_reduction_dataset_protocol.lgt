...
:- protocol(dimension_reduction_dataset_protocol).

	:- info([
		version is 1:0:0,
		author is 'Paulo Moura',
		date is 2026-04-20,
		comment is 'Protocol for datasets used with unsupervised dimension reduction algorithms.'
	]).
	...
	:- public(example/2).
	:- mode(example(-integer, -list(pair)), zero_or_more).
	:- info(example/2, [
		comment is 'Enumerates by backtracking the examples in the dataset. Each example has an ``Id`` and a list of ``Attribute-Value`` pairs.',
		argnames is ['Id', 'AttributeValues']
	]).
