...
:- object(tests,
	...
	:- info([
		...
		date is 2026-04-28,
		comment is 'Unit tests for the "truncated_svd" library.'
	]).
	...
	test(truncated_svd_learn_3_custom_options, deterministic((length(Components, 1), SingularValues = [SingularValue], SingularValue > 0.0, memberchk(options(Options), Diagnostics), memberchk(center(false), Options), memberchk(feature_scaling(true), Options), memberchk(model(truncated_svd), Diagnostics)))) :-
		truncated_svd::learn(low_rank_rectangular, truncated_svd_reducer(_Encoders, Components, SingularValues, Diagnostics), [n_components(1), center(false), feature_scaling(true), maximum_iterations(200), tolerance(1.0e-7)]).
	...
	test(truncated_svd_check_dimension_reducer_1, deterministic) :-
		...
		truncated_svd::check_dimension_reducer(DimensionReducer).

	test(truncated_svd_valid_dimension_reducer_1_invalid_singular_values_length, fail) :-
		truncated_svd::learn(low_rank_rectangular, truncated_svd_reducer(Encoders, Components, _SingularValues, Diagnostics), [n_components(1)]),
		truncated_svd::valid_dimension_reducer(truncated_svd_reducer(Encoders, Components, [], Diagnostics)).

	test(truncated_svd_valid_dimension_reducer_1_invalid_singular_values_order, fail) :-
		truncated_svd::learn(low_rank_rectangular, truncated_svd_reducer(Encoders, Components, [First, Second], Diagnostics), [n_components(2)]),
		Ascending is First + Second,
		truncated_svd::valid_dimension_reducer(truncated_svd_reducer(Encoders, Components, [Second, Ascending], Diagnostics)).

	test(truncated_svd_diagnostics_2, deterministic((memberchk(model(truncated_svd), Diagnostics), memberchk(component_count(1), Diagnostics), memberchk(singular_values([_]), Diagnostics), memberchk(convergence([_]), Diagnostics), memberchk(iterations([Iterations]), Diagnostics), Iterations >= 0, memberchk(final_delta([FinalDelta]), Diagnostics), FinalDelta >= 0.0))) :-
		truncated_svd::learn(low_rank_rectangular, DimensionReducer, [n_components(1)]),
		truncated_svd::diagnostics(DimensionReducer, Diagnostics).
	...
	test(truncated_svd_diagnostics_2_maximum_iterations, deterministic((memberchk(convergence([maximum_iterations_exhausted]), Diagnostics), memberchk(iterations([1]), Diagnostics), memberchk(final_delta([FinalDelta]), Diagnostics), FinalDelta > 0.0, memberchk(options(Options), Diagnostics), memberchk(maximum_iterations(1), Options), memberchk(tolerance(1.0e-12), Options)))) :-
		truncated_svd::learn(high_dimensional_measurements, DimensionReducer, [n_components(1), maximum_iterations(1), tolerance(1.0e-12)]),
		truncated_svd::diagnostics(DimensionReducer, Diagnostics).
	...
	test(truncated_svd_learn_2_exact_rank_request, deterministic((length(Components, 2), SingularValues = [First, Second], First >= Second, Second > 0.0))) :-
		truncated_svd::learn(low_rank_rectangular, truncated_svd_reducer(_Encoders, Components, SingularValues, _Diagnostics), [n_components(2)]).

	test(truncated_svd_learn_2_rank_deficient_request, error(domain_error(component_count, 3-2))) :-
		truncated_svd::learn(low_rank_rectangular, _DimensionReducer, [n_components(3)]).
	...
	test(truncated_svd_transform_3_component_names, deterministic((length(ReducedInstance, 2), memberchk(component_1-_, ReducedInstance), memberchk(component_2-_, ReducedInstance)))) :-
		truncated_svd::learn(high_dimensional_measurements, DimensionReducer, [n_components(2)]),
		truncated_svd::transform(DimensionReducer, [f1-0.9, f2-1.1, f3-1.0, f4-2.0, f5-2.2, f6-2.1], ReducedInstance).

	test(truncated_svd_transform_3_distinguishes_rank_rows, deterministic(Score1 =\= Score2)) :-
		truncated_svd::learn(low_rank_rectangular, DimensionReducer, [n_components(1), center(false)]),
		truncated_svd::transform(DimensionReducer, [f1-1.0, f2-0.0, f3-1.0], [component_1-Score1]),
		truncated_svd::transform(DimensionReducer, [f1-0.0, f2-1.0, f3-1.0], [component_1-Score2]).

	test(truncated_svd_learn_2_leading_zero_feature, deterministic((SingularValues = [SingularValue], SingularValue > 0.0))) :-
		truncated_svd::learn(leading_zero_feature_measurements, truncated_svd_reducer(_Encoders, _Components, SingularValues, _Diagnostics), [n_components(1), center(false)]).

	test(truncated_svd_learn_2_singleton_dataset, deterministic((SingularValues = [SingularValue], SingularValue > 0.0))) :-
		truncated_svd::learn(singleton_measurement, truncated_svd_reducer(_Encoders, _Components, SingularValues, _Diagnostics), [n_components(1), center(false)]).
	...
	test(truncated_svd_diagnostics_2_singleton_tolerance, deterministic((memberchk(convergence([tolerance]), Diagnostics), memberchk(iterations([1]), Diagnostics), memberchk(final_delta([0.0]), Diagnostics)))) :-
		truncated_svd::learn(singleton_measurement, DimensionReducer, [n_components(1), center(false)]),
		truncated_svd::diagnostics(DimensionReducer, Diagnostics).

	test(truncated_svd_learn_2_centered_singleton_dataset, error(domain_error(component_count, 1-0))) :-
		truncated_svd::learn(singleton_measurement, _DimensionReducer, [n_components(1), center(true)]).

	test(truncated_svd_valid_dimension_reducer_1_empty_components, deterministic) :-
		truncated_svd::learn(low_rank_rectangular, truncated_svd_reducer(Encoders, _Components, _SingularValues, _Diagnostics), [n_components(1)]),
		Diagnostics = [model(truncated_svd), options([n_components(1), center(false), feature_scaling(false), maximum_iterations(1000), tolerance(1.0e-8)])],
		truncated_svd::valid_dimension_reducer(truncated_svd_reducer(Encoders, [], [], Diagnostics)).

	test(truncated_svd_export_to_clauses_4, deterministic(Clause == reduced(DimensionReducer))) :-
		truncated_svd::learn(low_rank_rectangular, DimensionReducer, [n_components(1)]),
		truncated_svd::export_to_clauses(low_rank_rectangular, DimensionReducer, reduced, [Clause]).
	...
	test(truncated_svd_export_to_file_4, deterministic((Reducer = truncated_svd_reducer(_Encoders, Components, SingularValues, _Diagnostics), length(Components, 1), SingularValues = [SingularValue], SingularValue > 0.0))) :-
		...
		truncated_svd::learn(low_rank_rectangular, DimensionReducer, [n_components(1)]),
		truncated_svd::export_to_file(low_rank_rectangular, DimensionReducer, reducer, File),
		logtalk_load(File),
		{reducer(Reducer)}.
