...
:- object(http_socket_transport,
	...
	serve_until_shutdown(_Listener, _Handler, _Control, _Options, _Ready) :-
		throw(error(resource_error(threads), http_socket_transport::serve_until_shutdown(_, _, _, _, _))).

	request_listener_shutdown_control(_Listener) :-
		fail.

	request_shutdown_impl(_Control) :-
		throw(error(resource_error(threads), http_socket_transport::request_shutdown(_))).
	...
	serve_listener_with_workers_impl(per_connection, _Listener, _Handler, _Count, _ClientInfos) :-
		throw(error(resource_error(threads), http_socket_transport::serve_listener(_, _, _, _, [workers(per_connection)]) )).
	serve_listener_with_workers_impl(pool(_Size), _Listener, _Handler, _Count, _ClientInfos) :-
		throw(error(resource_error(threads), http_socket_transport::serve_listener(_, _, _, _, [workers(pool(_))]) )).
	serve_listener_with_workers_impl(pool(_Size, rolling), _Listener, _Handler, _Count, _ClientInfos) :-
		throw(error(resource_error(threads), http_socket_transport::serve_listener(_, _, _, _, [workers(pool(_, rolling))]) )).
