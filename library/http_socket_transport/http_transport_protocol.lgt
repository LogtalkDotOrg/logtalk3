...
:- protocol(http_transport_protocol).
	...
	:- info(open_listener/4, [
		...
		exceptions is [
			...
			'``Options`` contains an invalid listener option' - domain_error(http_process_transport_listener_option, 'Option'),
			'``Options`` contains an invalid listener helper executable' - domain_error(http_process_transport_listener_helper_executable, 'Executable'),
			'``Options`` contains incompatible TCP listener options' - domain_error(http_process_transport_listener_tcp_options, [listener_transport(tcp), temporary_tls_credentials('TemporaryTLSCredentialsPrefix')]),
			'``Options`` contains incompatible TLS listener options' - domain_error(http_process_transport_listener_tls_options, [tls_certificate_file('CertificateFile'), tls_key_file('KeyFile')]),
			'Temporary TLS credentials cannot be created' - resource_error(http_process_transport_temporary_tls_credentials),
			'The process-backed listener helper cannot be started' - resource_error(http_process_transport_listener)
		]
	]).
	...
	:- info(close_listener/1, [
		...
		exceptions is [
			...
			'``Listener`` is not an open listener handle for the transport implementation' - domain_error(http_socket_transport_listener, 'Listener'),
			'``Listener`` no longer exists for the transport implementation' - existence_error(http_socket_transport_listener, 'Listener')
		]
	]).
	...
	:- info(open_connection/4, [
		...
		exceptions is [
			...
			'An element ``Option`` of the list ``Options`` is neither a variable nor a compound term' - type_error(compound, 'Option'),
			'An element ``Option`` of the list ``Options`` is a compound term but not a valid option' - domain_error(option, 'Option'),
			'``Options`` contains an invalid connection option' - domain_error(http_process_transport_connection_option, 'Option'),
			'The process-backed client helper cannot be started' - resource_error(http_process_transport_connection)
		]
	]).
	...
	:- info(close_connection/1, [
		...
		exceptions is [
			...
			'``Connection`` is not an open reusable client connection handle' - domain_error(http_socket_transport_connection, 'Connection')
		]
	]).
	...
	:- info(connection_streams/3, [
		...
		exceptions is [
			'``Connection`` is a variable' - instantiation_error,
			'``Connection`` is not an open reusable client or upgraded WebSocket connection handle' - domain_error(http_socket_transport_connection, 'Connection')
		]
	]).
	...
	:- info(open_connection_pool/4, [
		...
		exceptions is [
			...
			'An element ``Option`` of the list ``Options`` is neither a variable nor a compound term' - type_error(compound, 'Option'),
			'An element ``Option`` of the list ``Options`` is a compound term but not a valid option' - domain_error(option, 'Option'),
			'``Options`` contains an invalid connection-pool option' - domain_error(http_socket_transport_connection_pool_option, 'Option'),
			'``Options`` contains an invalid process-backed connection-pool option' - domain_error(http_process_transport_connection_pool_option, 'Option'),
			'``Options`` contains incompatible connection-pool bounds' - domain_error(http_socket_transport_connection_pool_options, [min_size('MinSize'), max_size('MaxSize')]),
			'``Options`` contains incompatible process-backed connection-pool bounds' - domain_error(http_process_transport_connection_pool_options, [min_size('MinSize'), max_size('MaxSize')]),
			'No managed connection is available in the connection pool' - resource_error(http_socket_transport_connection_pool),
			'The process-backed client helper cannot be started' - resource_error(http_process_transport_connection)
		],
		remarks is [
	...
	:- info(close_connection_pool/1, [
		...
		exceptions is [
			...
			'``Pool`` is not a valid managed connection-pool handle' - domain_error(http_socket_transport_connection_pool, 'Pool'),
			'``Pool`` no longer exists' - existence_error(http_socket_transport_connection_pool, 'Pool'),
			'``Pool`` cannot be closed while pooled exchanges remain active' - permission_error(close, http_socket_transport_connection_pool, 'Pool')
		]
	]).
	...
	:- info(exchange/3, [
		...
		exceptions is [
			...
			'``ConnectionOrPool`` is neither an open connection nor a managed connection-pool handle' - domain_error(http_socket_transport_connection, 'ConnectionOrPool'),
			'``ConnectionOrPool`` is not a valid managed connection-pool handle' - domain_error(http_socket_transport_connection_pool, 'ConnectionOrPool'),
			'``ConnectionOrPool`` no longer exists as a managed connection pool' - existence_error(http_socket_transport_connection_pool, 'ConnectionOrPool'),
			'No managed connection is available in the connection pool' - resource_error(http_socket_transport_connection_pool),
	...
	:- info(serve_once/3, [
		comment is 'Accepts one incoming socket connection on the given server socket, serves that connection using the http_server_core library, closes the streams, and returns client information.',
	...
	:- info(serve_listener/5, [
		...
		exceptions is [
			...
			'An element ``Option`` of the list ``Options`` is neither a variable nor a compound term' - type_error(compound, 'Option'),
			'An element ``Option`` of the list ``Options`` is a compound term but not a valid option' - domain_error(option, 'Option'),
			'``Options`` contains an invalid listener-serving option' - domain_error(http_socket_transport_listener_option, 'Option'),
			'``Handler`` does not conform to the HTTP handler protocol' - domain_error(http_handler_protocol, 'Handler'),
			'The delegated HTTP server rejects the response stream' - domain_error(http_response_stream, 'Error'),
			'Thread workers are not available on this backend' - resource_error(threads)
		],
	...
	:- info(serve_until_shutdown/4, [
		...
		exceptions is [
			...
			'``Control`` is already registered for another open-ended listener loop' - permission_error(reuse, http_socket_transport_shutdown_control, 'Control'),
			'``Options`` is a variable or a partial list' - instantiation_error,
			'``Options`` is neither a variable nor a list' - type_error(list, 'Options'),
			'``Options`` contains an invalid listener-serving option' - domain_error(http_socket_transport_listener_option, 'Option'),
			'``Handler`` does not conform to the HTTP handler protocol' - domain_error(http_handler_protocol, 'Handler'),
	...
	:- info(request_shutdown/1, [
		...
		exceptions is [
			'``Control`` is a variable' - instantiation_error,
			'``Control`` is not registered for an open-ended listener loop' - existence_error(http_socket_transport_shutdown_control, 'Control'),
