...
:- object(tests,
	...
	cover(http_socket_transport).

	test(http_socket_transport_open_close_listener_4_01, deterministic) :-
	...
	test(http_socket_transport_open_close_connection_4_01, deterministic) :-
		http_socket_transport::open_listener('127.0.0.1', Port, Listener, []),
		threaded_once(server_accept_and_close_once(Listener), Tag),
		http_socket_transport::open_connection('127.0.0.1', Port, Connection, []),
		http_socket_transport::close_connection(Connection),
		threaded_exit(server_accept_and_close_once(Listener), Tag),
		http_socket_transport::close_listener(Listener).

	test(http_socket_transport_open_close_connection_pool_4_01, deterministic) :-
		http_socket_transport::open_listener('127.0.0.1', Port, Listener, []),
		threaded_once(server_accept_and_close_once(Listener), Tag),
		http_socket_transport::open_connection_pool('127.0.0.1', Port, Pool, [min_size(1), max_size(2)]),
		http_socket_transport::connection_pool_stats(Pool, stats(1, 0, 1, 1, 2)),
		http_socket_transport::close_connection_pool(Pool),
		threaded_exit(server_accept_and_close_once(Listener), Tag),
		http_socket_transport::close_listener(Listener).

	test(http_socket_transport_serve_once_3_01, true(compound(ClientInfo))) :-
	...
	test(http_socket_transport_serve_websocket_once_5_01, deterministic) :-
		...
		http_socket_transport::open_listener(Host, Port, Listener, []),
		...
		http_socket_transport::serve_websocket_once(Listener, websocket_http_socket_transport_handler, ServerConnection, ServerResponse, ClientInfo),
		await_websocket_client_exchange(Control, ClientConnection, ClientResponse),
		http_socket_transport::connection_streams(ServerConnection, ServerInput, ServerOutput),
		http_socket_transport::connection_streams(ClientConnection, ClientInput, ClientOutput),
		...
		http_socket_transport::close_connection(ServerConnection),
		http_socket_transport::close_connection(ClientConnection),
		http_socket_transport::close_listener(Listener),
		compound(ClientInfo),
	...
	test(http_socket_transport_serve_websocket_once_5_02, error(domain_error(http_socket_transport_websocket_response, _))) :-
		http_socket_transport::open_listener('127.0.0.1', Port, Listener, []),
		...
		catch(http_socket_transport::serve_websocket_once(Listener, echo_http_socket_transport_handler, _Connection, _HandshakeResponse, _ClientInfo), Error, (threaded_exit(client_exchange_response('127.0.0.1', Port, Request, _Response), ClientTag), http_socket_transport::close_listener(Listener), throw(Error))),
		threaded_exit(client_exchange_response('127.0.0.1', Port, Request, _Response), ClientTag),
		http_socket_transport::close_listener(Listener).

	test(http_socket_transport_exchange_4_01, deterministic) :-
		Request = request(post, origin('/echo'), http(1, 1), [host-host('example.com')], content('text/plain', text(hello)), []),
		http_socket_transport::open_listener('127.0.0.1', Port, Listener, []),
		threaded_ignore(server_serve_once(Listener, echo_http_socket_transport_handler)),
		http_socket_transport::exchange('127.0.0.1', Port, Request, Response),
		http_socket_transport::close_listener(Listener),
		status(Response, status(200, 'OK')),
		body(Response, content('text/plain', text(hello))).

	test(http_socket_transport_exchange_3_01, deterministic) :-
		...
		http_socket_transport::open_listener('127.0.0.1', Port, Listener, []),
		threaded_once(server_serve_requests_once(Listener, echo_http_socket_transport_handler, 2), Tag),
		http_socket_transport::open_connection('127.0.0.1', Port, Connection, []),
		http_socket_transport::exchange(Connection, Request1, Response1),
		http_socket_transport::exchange(Connection, Request2, Response2),
		http_socket_transport::close_connection(Connection),
		threaded_exit(server_serve_requests_once(Listener, echo_http_socket_transport_handler, 2), Tag),
		http_socket_transport::close_listener(Listener),
	...
	test(http_socket_transport_exchange_pool_3_01, deterministic) :-
		...
		http_socket_transport::open_listener('127.0.0.1', Port, Listener, []),
		threaded_once(server_serve_requests_once(Listener, echo_http_socket_transport_handler, 2), Tag),
		http_socket_transport::open_connection_pool('127.0.0.1', Port, Pool, [max_size(1)]),
		http_socket_transport::exchange(Pool, Request1, Response1),
		http_socket_transport::exchange(Pool, Request2, Response2),
		http_socket_transport::connection_pool_stats(Pool, stats(1, 0, 1, 0, 1)),
		http_socket_transport::close_connection_pool(Pool),
		threaded_exit(server_serve_requests_once(Listener, echo_http_socket_transport_handler, 2), Tag),
		http_socket_transport::close_listener(Listener),
		status(Response1, status(200, 'OK')),
	...
	test(http_socket_transport_exchange_sequence_4_01, deterministic) :-
		Requests = [
		...
		http_socket_transport::open_listener('127.0.0.1', Port, Listener, []),
		threaded_once(server_serve_once(Listener, echo_http_socket_transport_handler), Tag),
		http_socket_transport::exchange_sequence('127.0.0.1', Port, Requests, [Response1, Response2]),
		threaded_exit(server_serve_once(Listener, echo_http_socket_transport_handler), Tag),
		http_socket_transport::close_listener(Listener),
		status(Response1, status(200, 'OK')),
		body(Response1, content('text/plain', text(one))),
		status(Response2, status(200, 'OK')),
	...
	test(http_socket_transport_exchange_sequence_4_02, deterministic) :-
		Requests = [
		...
		http_socket_transport::open_listener('127.0.0.1', Port, Listener, []),
		threaded_once(server_serve_requests_once(Listener, keep_alive_close_http_socket_transport_handler, 2), Tag),
		http_socket_transport::exchange_sequence('127.0.0.1', Port, Requests, [Response1, Response2]),
		threaded_exit(server_serve_requests_once(Listener, keep_alive_close_http_socket_transport_handler, 2), Tag),
		http_socket_transport::close_listener(Listener),
		status(Response1, status(200, 'OK')),
	...
	test(http_socket_transport_exchange_sequence_3_01, deterministic) :-
		Requests = [
			request(post, origin('/one'), http(1, 1), [host-host('example.com')], content('text/plain', text(one)), []),
			request(post, origin('/two'), http(1, 1), [host-host('example.com')], content('text/plain', text(two)), [])
		],
		...
		threaded_once(server_serve_requests_once(Listener, echo_http_socket_transport_handler, 2), Tag),
		http_socket_transport::open_connection('127.0.0.1', Port, Connection, []),
		http_socket_transport::exchange_sequence(Connection, Requests, [Response1, Response2]),
		http_socket_transport::close_connection(Connection),
		threaded_exit(server_serve_requests_once(Listener, echo_http_socket_transport_handler, 2), Tag),
		http_socket_transport::close_listener(Listener),
		status(Response1, status(200, 'OK')),
	...
	test(http_socket_transport_exchange_sequence_pool_3_01, deterministic) :-
		Requests = [
		...
		threaded_once(server_serve_requests_once(Listener, echo_http_socket_transport_handler, 2), Tag),
		http_socket_transport::open_connection_pool('127.0.0.1', Port, Pool, [max_size(1)]),
		http_socket_transport::exchange_sequence(Pool, Requests, [Response1, Response2]),
		http_socket_transport::connection_pool_stats(Pool, stats(1, 0, 1, 0, 1)),
		http_socket_transport::close_connection_pool(Pool),
		threaded_exit(server_serve_requests_once(Listener, echo_http_socket_transport_handler, 2), Tag),
		http_socket_transport::close_listener(Listener),
	...
	test(http_socket_transport_exchange_pool_3_02, deterministic) :-
		Control = closing_exchange_pool_control,
		Request = request(get, origin('/pool-close'), http(1, 1), [host-host('example.com')], empty, []),
		http_socket_transport::open_listener('127.0.0.1', Port, Listener, []),
		threaded_once(http_socket_transport::serve_until_shutdown(Listener, closing_http_socket_transport_handler, Control, [workers(per_connection)]), ServeTag),
		http_socket_transport::open_connection_pool('127.0.0.1', Port, Pool, [max_size(1)]),
		http_socket_transport::exchange(Pool, Request, Response),
		http_socket_transport::connection_pool_stats(Pool, stats(0, 0, 0, 0, 1)),
		http_socket_transport::close_connection_pool(Pool),
		http_socket_transport::request_shutdown(Control),
		threaded_exit(http_socket_transport::serve_until_shutdown(Listener, closing_http_socket_transport_handler, Control, [workers(per_connection)]), ServeTag),
		status(Response, status(200, 'OK')),
		body(Response, empty),
		client_connection_state('127.0.0.1', Port, closed).

	test(http_socket_transport_serve_listener_4_01, deterministic) :-
		...
		http_socket_transport::serve_listener(Listener, echo_http_socket_transport_handler, 2, [ClientInfo1, ClientInfo2]),
		http_socket_transport::close_listener(Listener),
		compound(ClientInfo1),
		compound(ClientInfo2).

	test(http_socket_transport_serve_listener_5_01, deterministic) :-
		Request1 = request(post, origin('/one'), http(1, 1), [host-host('example.com')], content('text/plain', text(one)), []),
		Request2 = request(post, origin('/two'), http(1, 1), [host-host('example.com')], content('text/plain', text(two)), []),
		http_socket_transport::open_listener('127.0.0.1', Port, Listener, []),
		threaded_once(client_exchange_response('127.0.0.1', Port, Request1, Response1), Tag1),
		http_socket_transport::serve_listener(Listener, echo_http_socket_transport_handler, 1, [ClientInfo1], [shutdown(keep_open)]),
		...
		http_socket_transport::serve_once(Listener, echo_http_socket_transport_handler, ClientInfo2),
		http_socket_transport::close_listener(Listener),
	...
	test(http_socket_transport_serve_listener_5_02, deterministic) :-
		...
		http_socket_transport::serve_listener(Listener, echo_http_socket_transport_handler, 1, [ClientInfo], [shutdown(close)]),
	...
	test(http_socket_transport_serve_listener_5_03, deterministic) :-
		...
		threaded_once(client_exchange_response('127.0.0.1', Port, Request1, Response1), Tag1),
		threaded_once(client_exchange_response('127.0.0.1', Port, Request2, Response2), Tag2),
		http_socket_transport::serve_listener(Listener, echo_http_socket_transport_handler, 2, [ClientInfo1, ClientInfo2], [workers(per_connection), shutdown(close)]),
		threaded_exit(client_exchange_response('127.0.0.1', Port, Request1, Response1), Tag1),
		threaded_exit(client_exchange_response('127.0.0.1', Port, Request2, Response2), Tag2),
		compound(ClientInfo1),
		compound(ClientInfo2),
	...
	test(http_socket_transport_serve_listener_5_04, deterministic) :-
		...
		threaded_once(client_exchange_response('127.0.0.1', Port, Request1, Response1), Tag1),
		threaded_once(client_exchange_response('127.0.0.1', Port, Request2, Response2), Tag2),
		http_socket_transport::serve_listener(Listener, echo_http_socket_transport_handler, 2, [ClientInfo1, ClientInfo2], [workers(pool(1)), shutdown(close)]),
		threaded_exit(client_exchange_response('127.0.0.1', Port, Request1, Response1), Tag1),
		threaded_exit(client_exchange_response('127.0.0.1', Port, Request2, Response2), Tag2),
		compound(ClientInfo1),
		compound(ClientInfo2),
	...
	test(http_socket_transport_serve_listener_5_05, deterministic) :-
		...
		http_socket_transport::serve_listener(Listener, echo_http_socket_transport_handler, 3, [ClientInfo1, ClientInfo2, ClientInfo3], [workers(pool(2, rolling)), shutdown(close)]),
		threaded_exit(client_exchange_response('127.0.0.1', Port, Request1, Response1), Tag1),
		threaded_exit(client_exchange_response('127.0.0.1', Port, Request2, Response2), Tag2),
	...
	test(http_socket_transport_serve_listener_5_06, deterministic) :-
		Request = request(post, origin('/cancel-bounded'), http(1, 1), [host-host('example.com')], content('text/plain', text(cancel)), []),
		http_socket_transport::open_listener('127.0.0.1', Port, Listener, []),
		threaded_once(http_socket_transport::serve_listener(Listener, echo_http_socket_transport_handler, 2, ClientInfos, [shutdown(close)]), ServeTag),
		...
		threaded_exit(http_socket_transport::serve_listener(Listener, echo_http_socket_transport_handler, 2, ClientInfos, [shutdown(close)]), ServeTag),
		ClientInfos = [_ClientInfo],
		status(Response, status(200, 'OK')),
		body(Response, content('text/plain', text(cancel))),
		client_connection_state('127.0.0.1', Port, closed).

	test(http_socket_transport_serve_listener_5_07, deterministic) :-
		...
		http_socket_transport::open_listener('127.0.0.1', Port, Listener, []),
		threaded_once(http_socket_transport::serve_listener(Listener, echo_http_socket_transport_handler, 2, ClientInfos, [shutdown(keep_open)]), ServeTag),
		...
		http_socket_transport::request_listener_shutdown(Listener),
		threaded_exit(http_socket_transport::serve_listener(Listener, echo_http_socket_transport_handler, 2, ClientInfos, [shutdown(keep_open)]), ServeTag),
	...
	test(http_socket_transport_serve_until_shutdown_4_01, deterministic) :-
		...
		http_socket_transport::open_listener('127.0.0.1', Port, Listener, []),
		threaded_once(http_socket_transport::serve_until_shutdown(Listener, echo_http_socket_transport_handler, Control, []), ServeTag),
		threaded_once(client_exchange_response('127.0.0.1', Port, Request, Response), ClientTag),
		threaded_exit(client_exchange_response('127.0.0.1', Port, Request, Response), ClientTag),
		http_socket_transport::request_shutdown(Control),
		threaded_exit(http_socket_transport::serve_until_shutdown(Listener, echo_http_socket_transport_handler, Control, []), ServeTag),
		status(Response, status(200, 'OK')),
		body(Response, content('text/plain', text(serial))),
		client_connection_state('127.0.0.1', Port, closed).

	test(http_socket_transport_serve_until_shutdown_4_02, deterministic) :-
		...
		http_socket_transport::open_listener('127.0.0.1', Port, Listener, []),
		threaded_once(http_socket_transport::serve_until_shutdown(Listener, echo_http_socket_transport_handler, Control, [workers(per_connection)]), ServeTag),
		threaded_once(client_exchange_response('127.0.0.1', Port, Request1, Response1), ClientTag1),
		threaded_once(client_exchange_response('127.0.0.1', Port, Request2, Response2), ClientTag2),
		threaded_exit(client_exchange_response('127.0.0.1', Port, Request1, Response1), ClientTag1),
		threaded_exit(client_exchange_response('127.0.0.1', Port, Request2, Response2), ClientTag2),
		http_socket_transport::request_shutdown(Control),
		threaded_exit(http_socket_transport::serve_until_shutdown(Listener, echo_http_socket_transport_handler, Control, [workers(per_connection)]), ServeTag),
		status(Response1, status(200, 'OK')),
		body(Response1, content('text/plain', text(alpha))),
		status(Response2, status(200, 'OK')),
		body(Response2, content('text/plain', text(beta))),
	...
	test(http_socket_transport_serve_until_shutdown_4_03, deterministic) :-
		...
		http_socket_transport::open_listener('127.0.0.1', Port, Listener, []),
		threaded_once(http_socket_transport::serve_until_shutdown(Listener, echo_http_socket_transport_handler, Control, [workers(pool(1))]), ServeTag),
		threaded_once(client_exchange_response('127.0.0.1', Port, Request1, Response1), ClientTag1),
		threaded_once(client_exchange_response('127.0.0.1', Port, Request2, Response2), ClientTag2),
		threaded_exit(client_exchange_response('127.0.0.1', Port, Request1, Response1), ClientTag1),
		threaded_exit(client_exchange_response('127.0.0.1', Port, Request2, Response2), ClientTag2),
		http_socket_transport::request_shutdown(Control),
		threaded_exit(http_socket_transport::serve_until_shutdown(Listener, echo_http_socket_transport_handler, Control, [workers(pool(1))]), ServeTag),
		status(Response1, status(200, 'OK')),
		body(Response1, content('text/plain', text(alpha))),
		status(Response2, status(200, 'OK')),
		body(Response2, content('text/plain', text(beta))),
	...
	test(http_socket_transport_serve_until_shutdown_4_04, deterministic) :-
		...
		http_socket_transport::open_listener('127.0.0.1', Port, Listener, []),
		threaded_once(http_socket_transport::serve_until_shutdown(Listener, echo_http_socket_transport_handler, Control, [workers(pool(2, rolling))]), ServeTag),
		threaded_once(client_exchange_response('127.0.0.1', Port, Request1, Response1), ClientTag1),
		threaded_once(client_exchange_response('127.0.0.1', Port, Request2, Response2), ClientTag2),
		threaded_exit(client_exchange_response('127.0.0.1', Port, Request1, Response1), ClientTag1),
		threaded_exit(client_exchange_response('127.0.0.1', Port, Request2, Response2), ClientTag2),
		http_socket_transport::request_shutdown(Control),
		threaded_exit(http_socket_transport::serve_until_shutdown(Listener, echo_http_socket_transport_handler, Control, [workers(pool(2, rolling))]), ServeTag),
		status(Response1, status(200, 'OK')),
		body(Response1, content('text/plain', text(alpha))),
		status(Response2, status(200, 'OK')),
		body(Response2, content('text/plain', text(beta))),
	...
	test(http_socket_transport_serve_until_shutdown_5_01, deterministic) :-
		...
		http_socket_transport::open_listener('127.0.0.1', Port, Listener, []),
		http_socket_transport::serve_until_shutdown(Listener, echo_http_socket_transport_handler, Control, [], request_ready_shutdown(Control)),
		client_connection_state('127.0.0.1', Port, closed).
	...
	client_exchange_ignore(Host, Port, Request) :-
		catch(http_socket_transport::exchange(Host, Port, Request, _Response), _, true).

	client_exchange_response(Host, Port, Request, Response) :-
		once(http_socket_transport::exchange(Host, Port, Request, Response)).
	...
	server_serve_once(Listener, Handler) :-
		catch(http_socket_transport::serve_once(Listener, Handler, _ClientInfo), _, true).
	...
	websocket_client_exchange(Host, Port, Path, Protocols, Key, Connection, Response) :-
		...
		http_socket_transport::open_connection(Host, Port, Connection, []),
		catch(
			http_socket_transport::exchange(Connection, Request, Response),
			Error,
			(	catch(http_socket_transport::close_connection(Connection), _, true),
				throw(Error)
			)
		).
