...
The current library provides one public object and one companion category:
...
The companion router category provides these middleware handler predicates:
...
HTMX response headers are only added for non-`3xx` responses. When a request
is an HTMX request and the selected options would emit one or more `HX-*`
response headers, attempting to combine those options with a `3xx` response
...
HTMX-specific validation of the top-level shape:
...
The router companion category expects HTMX response options to be provided as
`htmx_response_options(Options)` request metadata or response properties.
