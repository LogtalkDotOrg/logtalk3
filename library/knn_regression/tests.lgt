...
:- object(tests,
	...
	test(knn_regression_predict_3_step_signal_left_band, deterministic(Prediction =~= 10.0)) :-
		knn_regression::learn(step_signal, Regressor, [k(1), feature_scaling(off)]),
	...
	test(knn_regression_learn_3_custom_options, deterministic((memberchk(k(5), Options), memberchk(distance_metric(manhattan), Options), memberchk(weight_scheme(distance), Options), memberchk(minkowski_power(4.0), Options), memberchk(feature_scaling(off), Options)))) :-
		knn_regression::learn(step_signal, knn_regressor(_Encoders, _Rows, Options), [k(5), distance_metric(manhattan), weight_scheme(distance), minkowski_power(4.0), feature_scaling(off)]).
