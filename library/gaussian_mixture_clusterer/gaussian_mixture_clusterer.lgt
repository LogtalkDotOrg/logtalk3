...
:- object(gaussian_mixture_clusterer,
	...
	:- info([
		...
		see_also is [clusterer_protocol, clustering_dataset_protocol, kmeans_clusterer]
	]).
	...
	build_diagnostics(Components, Count, Options, Convergence, Iterations, AverageLogLikelihood, FinalDelta, Diagnostics) :-
		...
		Diagnostics = [
			model(gaussian_mixture_clusterer),
	...
	check_clusterer(Clusterer) :-
		(	clusterer_data(Clusterer, Encoders, Components, Weights, Options, Diagnostics),
			...
			^^valid_clusterer_metadata(gaussian_mixture_clusterer, Options, Diagnostics),
			length(Components, ComponentCount),
