...
:- object(tests(_HTTPTransport_),
	...
	test(http_server_core_session_11, deterministic) :-
		...
		http_server_core_session::close(Manager),
		Visits == 1,
		memberchk(path-('/app'), Attributes),
		memberchk(secure-true, Attributes),
		\+ http_core::property(Response2, set_cookies(_)).

	test(http_server_core_session_12, deterministic) :-
		...
		http_server_core_session_event_logger::reset,
		memberchk(created(SessionId), Events),
		memberchk(touched(SessionId), Events),
		memberchk(resumed(SessionId), Events),
		memberchk(renewed(SessionId, NewId), Events),
		memberchk(touched(NewId), Events).
	...
	test(http_server_core_session_17, deterministic) :-
		date::format_date_time(date_time(2099, 1, 1, 0, 0, 0), 0, http_date, Expires),
		...
		http_server_core_session::close(Manager),
		memberchk(max_age-TTL, Attributes),
		TTL >= 0, TTL =< 5,
		memberchk(path-('/app'), Attributes),
		memberchk(secure-true, Attributes),
		\+ member(expires-_, Attributes),
		\+ member(max_age-99, Attributes).

	test(http_server_core_session_18, deterministic) :-
