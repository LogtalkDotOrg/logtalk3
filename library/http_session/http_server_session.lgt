...
:- object(http_server_session,
	...
	:- mode(http_server_session_event(+compound, +compound), zero_or_one).
	:- info(http_server_session_event/2, [
	...
	:- uses(list, [
		append/2, append/3, valid/1 as proper_list/1
	]).
	...
	open_manager(Manager, State) :-
		...
		Manager = http_server_session(ManagerId),
	...
	begin_request(Manager, Request, AnnotatedRequest) :-
		...
		Session = http_server_session_handle(ManagerId, ContextId),
	...
	manager_identifier(Manager, _ManagerId) :-
		var(Manager),
		instantiation_error.
	manager_identifier(http_server_session(ManagerId), ManagerId) :-
		integer(ManagerId),
	...
	session_handle_context(Session, _ContextId, _Context) :-
		...
		!,
		instantiation_error.
	session_handle_context(http_server_session_handle(ManagerId, ContextId), ContextId, Context) :-
		integer(ManagerId),
		integer(ContextId),
	...
	session_handle_manager_id(http_server_session_handle(ManagerId, _ContextId), ManagerId).
	...
	request_session_properties(Session, SessionState, SessionId, [http_server_session(Session), http_server_session_state(SessionState)| Tail]) :-
		(	SessionId == none ->
			Tail = []
		;	Tail = [http_server_session_id(SessionId)]
		).
	...
	ensure_session_handle(Session) :-
		...
		(	SessionId0 == none ->
			...
			update_context_state(ContextId, Context),
			notify_event(http_server_session(ManagerId), created(SessionId))
	...
	destroy_session_handle(Session) :-
		...
		;	retract(stored_session_(ManagerId, SessionId, _Data, _CreatedAt, _LastSeenAt, _AbsoluteExpiryAt)),
			...
			update_context_state(ContextId, Context),
			notify_event(http_server_session(ManagerId), destroyed(SessionId))
		).
	...
	maybe_gc_manager(ManagerId, CurrentTime) :-
		...
		;	CurrentTime - LastGcTime >= GcInterval ->
			collect_expired_sessions(http_server_session(ManagerId), ManagerId, IdleTimeout, AbsoluteTimeout, CurrentTime, _Collected),
			update_manager_last_gc(ManagerId, GcInterval, CurrentTime)
		;	true
		).
	...
	finalize_touched_session(ManagerId, SessionId, false, CurrentTime) :-
		...
		notify_event(http_server_session(ManagerId), touched(SessionId)).
