...
gradient descent with L2 regularization via the shared linear-model
training core in `regressor_common`.
...
- **Ridge Penalty**: Applies L2 regularization to the learned weights using a ridge-specific `regularization/1` option.
...
- `regularization/1`: Ridge penalty coefficient applied to the weight vector during optimization. Higher values increase shrinkage and can reduce overfitting. The default is `0.01`.
