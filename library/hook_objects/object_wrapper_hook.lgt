...
:- object(object_wrapper_hook,
	...
	:- info([
		...
		see_also is [
			object_wrapper_hook(_), object_wrapper_hook(_, _),
...
:- object(object_wrapper_hook(_Protocol_),
	...
	:- info([
		...
		see_also is [
			object_wrapper_hook, object_wrapper_hook(_, _),
			backend_adapter_hook, default_workflow_hook,
			grammar_rules_hook, prolog_module_hook(_),
			write_to_stream_hook(_, _), write_to_stream_hook(_),
			print_goal_hook, suppress_goal_hook
...
:- object(object_wrapper_hook(_Name_, _Relations_),
	implements(expanding)).
	...
	:- info([
		...
		author is 'Paulo Moura',
		date is 2022-02-03,
		comment is 'Use this object to wrap the contents of a plain Prolog file in an object with the given name and object entity relations (a list).',
		parnames is ['Name', 'Relations'],
		see_also is [
			object_wrapper_hook, object_wrapper_hook(_),
			backend_adapter_hook, default_workflow_hook,
			grammar_rules_hook, prolog_module_hook(_),
			write_to_stream_hook(_, _), write_to_stream_hook(_),
			print_goal_hook, suppress_goal_hook
	...
	term_expansion(
			begin_of_file,
			[begin_of_file, (:- Directive)]
	) :-
		Directive =.. [object, _Name_| _Relations_].
