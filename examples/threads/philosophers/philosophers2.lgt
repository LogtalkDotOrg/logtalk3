...
:- category(philosopher).
	...
	eat(Count, MaxTime):-
		...
		(	(Count mod 2 =:= 0, ::even; Count mod 2 =\= 0, ::odd) ->		
			LeftStick::pick_up,
			RightStick::pick_up
		;	RightStick::pick_up,
			LeftStick::pick_up
...
:- object(p5,
	...
	odd.
	left_chopstick(cs5).	% change order so that the chopsticks are picked
	right_chopstick(cs1).	% in different order from the other philosophers
