...
:- if((
	...
	:- initialization((
		...
		logtalk_load([routes, routes_demo], [optimize(on)])
	)).

:- else.

	:- initialization((write('(http_routes example requires a backend supporting sockets, threads, and unbounded integers)'), nl)).
