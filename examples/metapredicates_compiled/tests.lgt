...
:- object(tests,
	...
	test(metapredicates_compiled_01, true(Sorted == [1, 2, 3, 4, 9])) :-
		^^suppress_text_output,
		sort(user)::sort([3,1,4,2,9], Sorted).

	test(metapredicates_compiled_02, true(Included-Excluded == [2, 4]-[1, 3, 5])) :-
		meta::partition(user::even_integer, [1,2,3,4,5], Included, Excluded).

	test(metapredicates_compiled_03, true(Result == 34)) :-
		meta::fold_left(user::sum_squares, 0, [1,2,3], Result).

	test(metapredicates_compiled_04, true(Result == 'PREFIXabcdefghi')) :-
		meta::fold_left(atom_concat, 'PREFIX', [abc,def,ghi], Result).

	test(metapredicates_compiled_05, true(Result == abcdefghiSUFIX)) :-
		meta::fold_right(atom_concat, 'SUFIX', [abc,def,ghi], Result).

	test(metapredicates_compiled_06, true(Result == 15)) :-
		meta::fold_left(predicates::sum, 0, [1,2,3,4,5], Result).

	test(metapredicates_compiled_07, true(Result == 120)) :-
		meta::fold_left(predicates::product, 1, [1,2,3,4,5], Result).

	test(metapredicates_compiled_08, true(Result == (10, 10))) :-
		meta::fold_left(predicates::tuple, (0,0), [(1,2),(3,4),(6,4)], Result).

	test(metapredicates_compiled_09, true(Result == [0, 1, 5, 34])) :-
		meta::scan_left(user::sum_squares, 0, [1,2,3], Result).

	test(metapredicates_compiled_10, true(Result == [1, 5, 34])) :-
		meta::scan_left_1(user::sum_squares, [1,2,3], Result).

	test(metapredicates_compiled_11, true(Result == [15, 14, 12, 9, 5])) :-
		meta::scan_right(predicates::sum, 5, [1,2,3,4], Result).

	test(metapredicates_compiled_12, true(Result == [10, 9, 7, 4])) :-
		meta::scan_right_1(predicates::sum, [1,2,3,4], Result).

	test(metapredicates_compiled_13, true(Result == 15)) :-
		meta::fold_left_1([X,Y,Z]>>(Z is X+Y), [1,2,3,4,5], Result).

	test(metapredicates_compiled_14, true(Result == 3)) :-
		meta::fold_right_1([X,Y,Z]>>(Z is X-Y), [1,2,3,4,5], Result).

	test(metapredicates_compiled_15, true(Result == 120)) :-
		meta::fold_right_1([X,Y,Z]>>(Z is X*Y), [1,2,3,4,5], Result).

	test(metapredicates_compiled_16, true) :-
		meta::map(integer, [1,2,3,4,5]).

	test(metapredicates_compiled_17, true(Codes == [97, 98, 99, 100, 101])) :-
		meta::map(char_code, [a,b,c,d,e], Codes).

	% tests for calling meta-predicates with other meta-predicates as meta-arguments

	test(metapredicates_compiled_18, true(L == [1, 2, 3])) :-
		client::test1(L).

	test(metapredicates_compiled_19, true(L == [1, 2, 3])) :-
		client::test2(L).

	test(metapredicates_compiled_20, true(L == [1, 2, 3])) :-
		client::test3(L).

	% tests for calling meta-predicates with closure corresponding to control constructs

	test(metapredicates_compiled_21, true(X == proto)) :-
		proto::self_closure(X).

	test(metapredicates_compiled_22, true(X == parent)) :-
		parent::self_closure(X).

	test(metapredicates_compiled_23, true(X == library)) :-
		library::self_closure(X).

	test(metapredicates_compiled_24, true(X == extended_library)) :-
		extended_library::self_closure(X).

	test(metapredicates_compiled_25, true(X == parent)) :-
		proto::super_closure(X).

	test(metapredicates_compiled_26, true(L == [2, 4, 6])) :-
		test_object::p(L).

	test(metapredicates_compiled_27, true(X-Y == m1-m2)) :-
		m1::r(X, Y).

	test(metapredicates_compiled_28, true(Nth == 55)) :-
		fibonacci::nth(10, Nth).

	test(metapredicates_compiled_29, true(S1 == 179998)) :-
		company::company(C1),
		company::get_salary(company(C1), S1).

	test(metapredicates_compiled_30, true(S2 == 89999)) :-
		company::company(C1),
		company::cut_salary(company(C1), C2),
		company::get_salary(C2, S2).

	test(metapredicates_compiled_31, true(L == [2, 4])) :-
		meta::findall_member(N, [1, 2, 3, 4, 5], (N mod 2 =:= 0), L).

	test(metapredicates_compiled_32, true(L == [2, 4, 6, 8])) :-
		meta::findall_member(N, [1, 2, 3, 4, 5], (N mod 2 =:= 0), L, [6, 8]).

	test(metapredicates_compiled_33) :-
		obj::op1d(L1d),
	...
	test(metapredicates_compiled_34, true(L == [1, 2, 3])) :-
		wrappers_client::p(L).

	test(metapredicates_compiled_35, true(L == [1, 2, 3])) :-
		wrappers_client::q(L).

	test(metapredicates_compiled_36, true(L == [2, 1, 3])) :-
		wrappers_client::r(L).
	...
	test(metapredicates_compiled_40, true(Right == '(1+(2+(3+(4+(5+(6+(7+(8+(9+0)))))))))')) :-
		folds::right(Right).

	test(metapredicates_compiled_41, true(R == [9, 8, 7, 6, 5, 4, 3, 2, 1])) :-
		meta::fold_left([Y, X, [X|Y]]>>true, [], [1,2,3,4,5,6,7,8,9], R).

	test(metapredicates_compiled_42, true(R == [1, 2, 3, 4, 5, 6, 7, 8, 9])) :-
		meta::fold_right([X, Y, [X|Y]]>>true, [], [1,2,3,4,5,6,7,8,9], R).

	test(metapredicates_compiled_43) :-
		simple_client::test_whatever.

	test(metapredicates_compiled_44) :-
		^^suppress_text_output,
