...
:- if((
	...
	:- initialization((
		...
		logtalk_load(http_static_site, [optimize(on)])
	)).

:- else.

	:- initialization((write('(http_static_site example requires a backend supporting sockets and unbounded integers)'), nl)).
