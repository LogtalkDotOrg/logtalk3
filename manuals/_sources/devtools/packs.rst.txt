...
expected to share the name of the archive and use a ``.asc`` extension.
For example, if the pack archive name is ``v1.0.0.tar.gz``, the
signature file must be named ``v1.0.0.tar.gz.asc``. When the
``checksig(true)`` option is used, the signature file is automatically
downloaded using a URL constructed from the pack archive URL. An example
of signing a pack (and creating the ``.asc`` file) assuming the default
key is:
