...
usual default value), the ``lgtdoc`` tool prints warnings on missing
ending periods (full stops) or exclamation marks in ``info/1-2``
directives (in comments, remarks, parameter descriptions, and argument
descriptions).

Set a flag value to ``silent`` to turn of the corresponding linter
warnings.
