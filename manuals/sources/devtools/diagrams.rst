...
using diagrams for navigating complex code by zooming into details. SVG
output can also easily link to both source code repositories and API
documentation. This allows diagrams to be used for source code
navigation.
...
`docs/library_index.html#diagrams <https://logtalk.org/docs/library_index.html#diagrams>`__
...
also generate the entity and predicate cross-referencing diagrams. This
feature allows using diagrams for navigating complex code by zooming
into details.
...
file system URLs, define the empty atom, ``''``, as a prefix. See the
``SCRIPT.txt`` file for some usage examples.
...
predicates are not supported. See the ``SCRIPT.txt`` file for some usage
examples. Note that support for diagrams with links to API documentation
