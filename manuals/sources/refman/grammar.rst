...
mode_directive ::=
   "``:- mode(``" predicate_mode_term \| non_terminal_mode_term "``,``"
   number_of_proofs "``).``"
...
info_directive ::=
   "``:- info(``" predicate_indicator \| non_terminal_indicator "``,``"
   predicate_info_list "``).``"
...
category_property ::=
   ...
   "``declares(``" predicate_indicator "``,``"
   predicate_declaration_property_list "``)``" \|
   "``defines(``" predicate_indicator "``,``"
   predicate_definition_property_list "``)``" \|
   "``includes(``" predicate_indicator "``,``" object_identifier \|
   category_identifier "``,``" predicate_definition_property_list
   "``)``" \|
   "``provides(``" predicate_indicator "``,``" object_identifier \|
   category_identifier "``,``" predicate_definition_property_list
   "``)``" \|
